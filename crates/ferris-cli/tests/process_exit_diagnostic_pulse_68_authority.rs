...
const CUTOFF: &str = "48c26aff381eb66459bf099559f0d44971d46f97";
const PULSE_59_HEAD: &str = "6945f5fc96868c97267a1635fbb5219cc398eeb4";
const DOMAIN: &str = "ferris.process-exit-diagnostic-pulse-68-authority/v1";
const DECLARATION_IDENTITY: &str =
    "sha256:d9f840c32413105e337312363812b8ecec706c1c95a69a8f85ea170ad42e2818";
const MUTATION_COUNT: usize = 28_830;
const PRIOR_REGISTRY_TOTAL: usize = 290_502;
const REGISTRY_TOTAL: usize = 319_332;
const HELD_OUT: &str = "docs/simulations/profile-diff-held-out";
const P68_PROBE_WORKER: &str =
    "docs/simulations/profile-diff-held-out/fixtures/p68_wsl_probe_worker.py";
const P68_PROBE_WORKER_SHA256: &str =
    "sha256:92b8781fff0a27e6d54798514f402ca01c0e5e6705b94ed73809cf4a3502dfe1";
const P68_PROBE_WORKER_SIZE: u64 = 13_063;
const P68_PROBE_DEPENDENCY: &str =
    "docs/simulations/profile-diff-held-out/fixtures/p68_wsl_probe_sealed_dependencies.py";
const P68_PROBE_DEPENDENCY_SHA256: &str =
    "sha256:82b35ca824347ea29ddc0801d832c1858382bd0979868d403aa40febc8f7fa16";
const P68_PROBE_DEPENDENCY_SIZE: u64 = 20_521;
const P68_PROBE_PROTOCOL_SCHEMA: &str =
    "docs/simulations/profile-diff-held-out/schemas/ferris.pulse-68-wsl-probe-session.v1.schema.json";
const P68_PROBE_PROTOCOL_SCHEMA_SHA256: &str =
    "sha256:bfd1f7f6fec044dc22356fb4a2691cb5f668cdb462688d31aab6df99dd198d68";
const P68_PROBE_PROTOCOL_SCHEMA_SIZE: u64 = 3_863;
...
fn assert_current_cutoff_fields(declaration: &Value) {
    for pointer in [
        "/immutable_ferris/cutoff",
        "/pre_call_public_prerequisites/authority_checkout_root/revision",
        "/pre_call_public_prerequisites/p39_checkout_root/head",
        "/pre_call_public_prerequisites/p39_checkout_root/revision",
        "/pre_call_public_prerequisites/repo_root/revision",
    ] {
        assert_eq!(
            declaration.pointer(pointer),
            Some(&Value::String(CUTOFF.to_owned())),
            "{pointer} must equal current cutoff"
        );
    }
}

fn python_toplevel_block(source: &str, marker: &str) -> String {
...
fn pulse_68_binds_cutoff_containing_pulse_67_withdrawal_and_probe_artifacts() {
    ...
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-68-authority.json"));
    let schema = read_json(
        held_out.join("schemas/ferris.process-exit-diagnostic-pulse-68-authority.v1.schema.json"),
    );
    assert_eq!(declaration["schema"], DOMAIN);
    assert_eq!(declaration["status"], "authorized-unexecuted");
    assert_eq!(
        declaration["program_id"],
        "FERRIS-P68-INDEPENDENT-PROCESS-EXIT-DIAGNOSTIC-WITNESS-PRESERVING-CAPABILITY-MATERIALIZATION-AUTHORITY"
    );
    ...
    assert_current_cutoff_fields(&declaration);
    for field in [
        "authority_p39_repo_checkout_fields_bound_to_current_cutoff",
        ...
        "pulse_67_withdrawal_present_at_cutoff",
        "pulse_68_probe_worker_present_at_cutoff",
        "pulse_68_probe_dependency_present_at_cutoff",
        "pulse_68_probe_protocol_schema_present_at_cutoff",
    ...
    for path in [
        ...
        "docs/simulations/profile-diff-held-out/schemas/ferris.process-exit-diagnostic-pulse-68-authority.v1.schema.json",
        "docs/simulations/profile-diff-held-out/PROCESS_EXIT_DIAGNOSTIC_PULSE_68_AUTHORITY.md",
        "context/waves/2026-08-12-platform-profile-conformance/pulses/pulse-68.md",
        "crates/ferris-cli/tests/process_exit_diagnostic_pulse_68_authority.rs",
    ] {
        assert_cutoff_absent(path);
    }
    let p67_wave = String::from_utf8(cutoff_blob(
        "context/waves/2026-08-12-platform-profile-conformance/pulses/pulse-67.md",
    ))
    .expect("UTF-8 Pulse 67 wave");
    assert_contains_all(
        &p67_wave,
        &[
        ...
        "Pulse 67 wave at cutoff",
    );
    let p67_record = String::from_utf8(cutoff_blob(
        "docs/simulations/profile-diff-held-out/PROCESS_EXIT_DIAGNOSTIC_PULSE_67_AUTHORITY.md",
    ))
    .expect("UTF-8 Pulse 67 closeout");
    assert_contains_all(
        &p67_record,
        &[
...
#[test]
fn pulse_68_binds_cutoff_blobs_supported_variants_and_exact_p59_surface() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration_text = String::from_utf8(read_lf(
        held_out.join("fixtures/process-exit-diagnostic-pulse-68-authority.json"),
    ))
...
#[test]
fn pulse_68_records_withdrawn_pulse_67_and_binds_exact_p56_loader_probe_preflight() {
    let declaration = read_json(
        repo_root().join("docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-68-authority.json"),
    );
    let predecessors = declaration["closed_predecessors"]
        .as_object()
        .expect("predecessors");
    let p67 = &predecessors["pulse_67"];
    assert_eq!(
        p67["declaration_identity"],
        "sha256:d295759eed523c7c4c5d9efcd2c5f424ef6db03309c544cc31718fbd66eb3f05"
    );
    assert_eq!(
        p67["authority_commit"],
    ...
    assert_eq!(p67["p59_callable_invocations"], 0);
    assert_eq!(p67["non_retryable"], true);
    assert_eq!(p67["non_resumable"], true);
    assert_eq!(
        p67["contract_review"]["current_cutoff_checkout_fields_required"],
        "every-authority-p39-repo-cutoff-head-revision-field-equals-the-current-p68-cutoff"
    );
    assert_eq!(
        p67["contract_review"]["p56_loader_leg_required"],
    ...
    assert_eq!(
        authority["pre_call_probe_worker_exact_p56_loader_leg_required"],
        true
    );
    assert_eq!(
        authority["pre_call_probe_worker_exact_p56_callable_identity_validation_without_launch_required"],
        true
    );
    ...
    assert_eq!(
        ...
        serde_json::json!([
            "P68-PRIVATE-RUNTIME-TOPOLOGY-PROBE",
            "P68-P41-ACTUAL-BASENAME-RENAME-PROBE",
            "P68-P59-TERMINAL-SIBLING-PROBE"
        ])
    );
    assert_eq!(
        platform_stops["native_linux"],
        serde_json::json!([
            "P68-EXACT-P57-WSL-STAGE-BUNDLE-SUBPROCESS-RUN-PREFLIGHT",
            "P68-SYSTEMROOT-SOURCE-PRECEDENCE-DERIVATION",
            "P68-WINDOWS-WSL-CMD-NONREPARSE-IDENTITY",
            "P68-EXACT-P57-WORKER-PROBE-BUNDLE",
            "P68-EXACT-P57-WORKER-BOOTSTRAP-POPEN-PREFLIGHT",
            "P68-P57-WSL-TWO-SPAWN-CARDINALITY",
            "P68-UBUNTU-NOEXEC-PREREQUISITE"
        ])
    );
    ...
    assert_eq!(
        ...
        serde_json::json!([
            ...
            "exact-_WSL_WORKER_BOOTSTRAP-descriptor-open-hash-compile-route",
            "spawn1-bundle-root-handoff-to-spawn2-without-restage",
            "exact-p56-root-equality-repo_root-parent-derivation-load_exact_p56-and-module-parent-check",
            "ready-one-harmless-probe-close-line-protocol-under-bounded-reads-writes-and-timeouts",
            "post-close-wait-terminate-kill-stdout-drain-stderr-drain-cleanup",
            "single-bundle-cleanup-and-absence-verification-after-spawn2"
        ])
    );
    assert_eq!(
        probe_scope["probe_only_semantics"],
        serde_json::json!([
            "worker-uses-p68-probe-sealed-dependencies-with-exact-load_exact_p56-route-and-harmless-probe-surface",
            "exact-staged-p56-module-import-validates-callable-identities-without-publish-build-launch",
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "exact_load_exact_p56_callable_name": "load_exact_p56",
            "exact_p56_loader_route_uses_production_source_block": true,
            "exact_repo_root_parent_derivation_formula": "p56_root.parents[3]",
            "loaded_p56_module_parent_must_equal_p56_root": true,
            "no_ambient_pythonpath": true,
            "no_mounted_import_route": true,
            "no_publish_build_launch_invocation": true,
            "no_sitecustomize": true,
            "no_worker_path_reopen": true,
            "no_wslenv": true,
            "probe_dependency_validates_bundle_root_parent": true,
            "probe_dependency_validates_exact_p56_callable_signatures": [
                "publish_retained_build_and_custody",
                "launch_verified",
                "close_custody"
            ],
            "probe_dependency_validates_exact_p56_default_launch_uses": 69,
            "probe_dependency_validates_exact_p56_release_tree_via_load_exact_p56": true,
    ...
    assert_eq!(
        ...
        "docs/simulations/profile-diff-held-out/pulse-68-public-result/"
    );
    assert_eq!(
        terminal["published_result"]["p47_destination"],
        "docs/simulations/profile-diff-held-out/pulse-68-publication-witness/"
    );
    assert_eq!(
    ...
    assert!(worker_source.contains("SCHEMA = \"ferris.pulse-68-wsl-probe-session/v1\""));
    assert!(worker_source.contains("REQUEST_COUNT = 1"));
    assert!(worker_source.contains("class ProbeProtocol:"));
    assert!(worker_source.contains("repo_root = p56_root.parents[3]"));
    assert!(worker_source.contains("p56 = dependencies.load_exact_p56(repo_root)"));
    assert!(worker_source.contains("if Path(p56.__file__).parent != p56_root:"));
    assert!(worker_source.contains("launch_harmless_probe"));
    assert!(dep_source.contains("PROBE_SCHEMA = \"ferris.pulse-68-wsl-probe-session/v1\""));
    assert!(dep_source.contains("PROBE_RESULT_SCHEMA = \"ferris.pulse-68-wsl-probe-result/v1\""));
    assert!(dep_source.contains("PRODUCTION_P57_WORKER_SHA256"));
    assert!(dep_source.contains("validated_p56_loader_route"));
    assert!(dep_source.contains("PROBE_VALIDATED_P56_CALLABLES"));
    assert_eq!(
        python_toplevel_block(&dep_source, "def load_exact_p56("),
        python_toplevel_block(&prod_dep, "def load_exact_p56(")
    );
    assert_eq!(
        python_toplevel_block(&dep_source, "def _expect_signature("),
        python_toplevel_block(&prod_dep, "def _expect_signature(")
    );
    assert_eq!(
        python_toplevel_block(&dep_source, "def _verified_release("),
        python_toplevel_block(&prod_dep, "def _verified_release(")
    );
    assert!(dep_source.contains("def launch_harmless_probe("));
}

#[test]
fn pulse_68_validator_is_checkout_only_and_rejects_mutations() {
    let root = repo_root();
    ...
    let p68 = fs::read_to_string(
        root.join("crates/ferris-cli/tests/process_exit_diagnostic_pulse_68_authority.rs"),
    )
    ...
    assert!(
        p68.contains("canonical_identity_by_path"),
        "P68 validates canonical identities"
    );
    assert!(
        p68.contains("assert_current_cutoff_fields("),
        "P68 enumerates current cutoff fields explicitly"
    );
    for pointer in [
        "/immutable_ferris/cutoff",
        "/pre_call_public_prerequisites/authority_checkout_root/revision",
        ...
        assert!(
            p68.contains(pointer),
            "P68 validator must enumerate {pointer}"
        );
    }
    assert!(
        !p68.contains(&legacy_raw_map),
        "P68 cannot compare working-tree-only raw identities"
    );
    ...
    assert!(
        !p68.contains(&python_helper)
            && !p68.contains(&pulse_59_python_tests)
            && !p68.contains(&pulse_58_python_tests),
        "P68 test must stay declaration/mutation/checkout only"
    );

    let declaration = read_json(
        root.join("docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-68-authority.json"),
    );
    let schema = read_json(
        root.join("docs/simulations/profile-diff-held-out/schemas/ferris.process-exit-diagnostic-pulse-68-authority.v1.schema.json"),
    );
    let mutations = read_json(
        root.join("docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-68-authority-mutations.json"),
    );
