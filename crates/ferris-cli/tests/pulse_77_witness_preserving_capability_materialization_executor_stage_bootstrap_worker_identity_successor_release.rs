...
const RELEASE: &str = "docs/simulations/profile-diff-held-out/pulse-77-witness-preserving-capability-materialization-executor-stage-bootstrap-worker-identity-successor-release";
const MANIFEST_RAW: &str =
    "sha256:46606ec021b29baadb010dac35990623778d60f304cf24126bd121192bc5ccae";
const MANIFEST_AGGREGATE: &str =
    "sha256:ea0421ffb8b3aaaf9068f938b2750531f0b9c9f7cda0f2f2756ee8d20c75f680";
const RECEIPT_RAW: &str = "sha256:0ae53b7ca386b5ac4296eb05f146275c33db4da3dc1159264c54eedf4d686055";
const RECEIPT_PAYLOAD: &str =
    "sha256:16b565a13107795e8eeeab7c06afd0bf308a5aef4a36242239629c630cb98092";
const SEAL_RAW: &str = "sha256:e566c5a4d0dd3a7a5056c0abd4d03bef01faa56cfc6aca8ab1604b0aaf449710";
const SEAL_PAYLOAD: &str =
    "sha256:778d50bb6d1993e3f0aea8183f34759e8c23cb13915dec805b4adadfb8e18eec";
const P76_COMMIT: &str = "95edeb9560748989b4c94c0becc3a466e1fbc216";
...
fn python_output(release: &Path, arguments: &[&str]) -> std::process::Output {
    Command::new(python())
        ...
        .output()
        .expect("run Pulse 77 Python validation")
}

#[test]
fn pulse_77_witness_preserving_capability_materialization_executor_stage_bootstrap_worker_identity_successor_is_sealed_and_fake_only(
) {
    let root = repo_root();
    ...
    let expected_paths = BTreeSet::from([
        ...
        "fixtures/p77-public-gate-catalog.json".to_owned(),
        ...
        "schemas/ferris.pulse-77-witness-preserving-capability-materialization-executor-stage-bootstrap-worker-identity-successor.v1.schema.json"
            .to_owned(),
    ...
    assert!(source.contains("p76.run_ordered_capability_materialization_executor("));
    assert!(source.contains("p76._run_qualification_executor("));
    ...
    assert!(source.contains("class _Pulse77LinuxLockManager"));
    ...
    assert!(sealed_source.contains(&format!("P76_COMMIT = \"{P76_COMMIT}\"")));
    ...
    assert!(sealed_source.contains("_KERNEL_LOCK_NAMESPACE_PREFIX = \"ferris-p77\""));
    ...
    assert!(sealed_source.contains("_P77_INTERNAL_LOCK_MANAGER"));
    assert!(sealed_source.contains("_bind_internal_lock_manager"));
    assert!(sealed_source.contains("P77-SEALED-LOCK-CROSS-INSTANCE-REENTRY"));
    assert!(sealed_source.contains("_WINDOWS_WAIT_ABANDONED"));
    assert!(sealed_source.contains("if current is not dependencies"));
    assert!(sealed_source.contains("load_pulse76"));
    assert!(!sealed_source.contains("register_at_fork"));
    assert!(!sealed_source.contains("pulse-77-sealed-loader-locks"));
    ...
    let (_, catalog) = read_json(release.join("fixtures/p77-public-gate-catalog.json"));
    assert_eq!(
        catalog["gate_ids"],
        serde_json::json!([
    ...
    assert_eq!(
        manifest["schema"],
        "ferris.pulse-77-witness-preserving-capability-materialization-executor-stage-bootstrap-worker-identity-successor-public-manifest/v1"
    );
    ...
    assert_eq!(manifest["predecessors"]["pulse_76"]["commit"], P76_COMMIT);
    assert_eq!(
        manifest["predecessors"]["pulse_76"]["source_sha256"],
        "sha256:422b52c0be55bcc1e4f4df5f4920ed9b151218182e2d4496a92d186f08ca0aba"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_76"]["stack"]["pulse_75"]["commit"],
        "47229cdc7c090e1c5eb2762ac82b411ec50f1e7d"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_76"]["stack"]["pulse_75"]["release_root"],
    ...
    assert_eq!(
        receipt["schema"],
        "ferris.pulse-77-witness-preserving-capability-materialization-executor-stage-bootstrap-worker-identity-successor-qualification-envelope/v1"
    );
    assert_eq!(
    ...
    assert_eq!(payload["p76_bound_commit"], P76_COMMIT);
    assert_eq!(payload["p76_publication_invocations"], 0);
    assert_eq!(payload["p77_publication_invocations"], 20);
    assert_eq!(
    ...
    assert_eq!(
        ...
        BTreeSet::from([
            ...
            "concurrent-p76-load-restores-foreign-sentinel".to_owned(),
            "exact-p76-binding-and-signature".to_owned(),
            ...
            "p76-import-exception-restores-generic-slot".to_owned(),
            "p76-prelaunch-failure-publication-not-attempted".to_owned(),
            ...
            "qualification-delegates-to-p76".to_owned(),
    ...
    assert_eq!(seal["payload"]["scope"]["exact_p76_execution"], true);
    assert_eq!(
        seal["payload"]["scope"]["exact_p75_stage_bootstrap_worker_identity_chain"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        seal["payload"]["limits"]["publication_not_attempted_before_p76_completion"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        schema["$id"],
        "urn:ferris:schema:pulse-77-witness-preserving-capability-materialization-executor-stage-bootstrap-worker-identity-successor:v1"
    );
    assert_eq!(
