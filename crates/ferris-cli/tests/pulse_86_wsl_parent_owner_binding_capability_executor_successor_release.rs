...
fn python_output(release: &Path, arguments: &[&str]) -> std::process::Output {
    Command::new(python())
        ...
        .output()
        .expect("run Pulse 86 Python validation")
}

#[test]
fn pulse_86_wsl_parent_owner_binding_capability_executor_successor_is_sealed_and_fake_only(
) {
    let root = repo_root();
    ...
    let expected_paths = BTreeSet::from([
        ...
        "fixtures/p86_fake_native_wsl.py".to_owned(),
        ...
        "schemas/ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor.v1.schema.json".to_owned(),
    ...
    assert!(source.contains("P86-INDETERMINATE-STAGE-CLEANUP"));
    assert!(source.contains("P86-WSL-OWNER"));
    assert!(source.contains("P57-INDETERMINATE-CLEANUP"));
    assert!(source.contains("\"--user\""));
    assert!(source.contains("os.geteuid()!=expected_owner_uid"));
    assert!(source.contains("parent_stat.st_uid!=expected_owner_uid"));
    assert!(source.contains("dependency_relative"));
    assert!(source.contains("sys.argv[13:]"));
    ...
    assert!(sealed_source.contains("_KERNEL_LOCK_NAMESPACE_PREFIX = \"ferris-p86\""));
    ...
    assert!(sealed_source.contains("P86-SEALED-LOCK-CROSS-INSTANCE-REENTRY"));
    ...
    assert_eq!(
        manifest["predecessors"]["pulse_78"]["source_sha256"],
    ...
    assert_eq!(
        receipt["schema"],
        "ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor-receipt/v1"
    );
    assert_eq!(canonical_payload_sha256(&receipt["payload"]), RECEIPT_PAYLOAD);
    let payload = &receipt["payload"];
    assert_eq!(payload["schema"], "ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor-qualification/v1");
    ...
    assert_eq!(payload["explicit_owner_command_binding_verified"], true);
    assert_eq!(payload["unknown_owner_probe_stderr_rejected"], true);
    assert_eq!(payload["parent_owner_uid_race_rejected"], true);
    assert_eq!(payload["effective_owner_uid_mismatch_rejected"], true);
    assert_eq!(payload["malformed_owner_probe_mapped"], true);
    ...
    assert_eq!(
        ...
        BTreeSet::from([
            ...
            "parent-owner-explicit-root-resolution".to_owned(),
            "parent-owner-stderr-rejected".to_owned(),
            "parent-owner-uid-race-rejected".to_owned(),
    ...
    assert_eq!(seal["payload"]["scope"]["exact_p78_binding"], true);
    assert_eq!(
        seal["payload"]["scope"]["parent_owner_derived_execution_identity"],
        true
    );
    assert_eq!(
        seal["payload"]["scope"]["fork_child_lock_state_reset_without_inherited_mutex"],
        true
    );
    assert_eq!(
        seal["payload"]["scope"]["explicit_wsl_user_for_all_operational_spawns"],
        true
    );
    assert_eq!(
        seal["payload"]["scope"]["effective_uid_matches_parent_owner"],
        true
    );
    assert_eq!(
        seal["payload"]["scope"]["unknown_owner_probe_stderr_rejected"],
        true
    );
    assert_eq!(
        seal["payload"]["scope"]["malformed_owner_probe_mapped_to_owner_failure"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        seal["payload"]["limits"]["ambient_wsl_default_user_selection"],
        false
    );
    assert_eq!(
        seal["payload"]["limits"]["caller_supplied_wsl_username"],
        false
    );
    assert_eq!(
        seal["payload"]["limits"]["stderr_filter_or_allowlist"],
        false
    );
    assert_eq!(
    ...
    let schema: Value = serde_json::from_slice(&read_lf(release.join(
        "schemas/ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor.v1.schema.json",
    )))
    .expect("parse qualification schema");
    assert_eq!(
        schema["$id"],
        "https://ferris.dev/schema/ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor.v1.schema.json"
    );
    assert_eq!(
        schema["properties"]["schema"]["const"],
        "ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor-qualification/v1"
    );
    assert_eq!(
