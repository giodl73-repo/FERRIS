...
const CUTOFF: &str = "96b2bda8bea455a2e5a4610c4ab1722e2e68fcb3";
const DOMAIN: &str = "ferris.process-exit-diagnostic-pulse-49-authority/v1";
const DECLARATION_IDENTITY: &str =
    "sha256:01101bb7d2a63b657940f82f80eb3edcd3ab7bba05cb8cd54e4dd0c87ce8a3ee";
const MUTATION_COUNT: usize = 9_657;
const PRIOR_TOTAL_MUTATION_COUNT: usize = 48_317;
...
const PREAUTHORIZATION_EVIDENCE_DOMAIN: &str =
    "ferris.pulse-49-public-catalog-preauthorization-evidence/v1";
...
#[test]
fn pulse_49_public_catalog_preauthorization_is_exact_nonadvancing_and_public_only() {
    let declaration =
        read_json(held_out_root().join("fixtures/process-exit-diagnostic-pulse-49-authority.json"));
    let proof = &declaration["public_catalog_preauthorization"];
    assert_eq!(
        proof["schema"],
        "ferris.pulse-49-public-catalog-preauthorization/v1"
    );
...
#[test]
fn pulse_49_authority_is_closed_unexecuted_and_witness_publication_ordered() {
    ...
    let schema = read_json(
        held_out.join("schemas/ferris.process-exit-diagnostic-pulse-49-authority.v1.schema.json"),
    );
    assert_eq!(
        schema["$schema"],
    ...
    let pulse_48_authority =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-48-authority.json"));
    assert_eq!(
        declaration["public_release_custody"], pulse_48_authority["public_release_custody"],
        "Pulse 49 reuses the exact public release custody"
    );
    assert_eq!(
        declaration["inherited_public_gates"], pulse_48_authority["inherited_public_gates"],
        "Pulse 49 retains the bounded diagnostic gates"
    );
    assert_eq!(
        declaration["pulse_49_bounded_process_exit_search"],
        pulse_48_authority["pulse_48_bounded_process_exit_search"],
        "Pulse 49 retains the exact bounded search contract"
    );
...
#[test]
fn pulse_49_mutation_controls_reject_each_authority_change() {
    let held_out = held_out_root();
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-49-authority.json"));
    let schema = read_json(
    ...
    let mutations = read_json(
        held_out.join("fixtures/process-exit-diagnostic-pulse-49-authority-mutations.json"),
    );
