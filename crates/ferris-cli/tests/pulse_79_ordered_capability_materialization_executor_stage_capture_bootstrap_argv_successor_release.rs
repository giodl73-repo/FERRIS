...
const RELEASE: &str = "docs/simulations/profile-diff-held-out/pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor-release";
...
const RECEIPT_RAW: &str = "sha256:931ff5628bccb8ad8b115e1301355ff870b48b6110049a9a5baaf01676f57e7d";
const RECEIPT_PAYLOAD: &str =
    "sha256:c267d12567f479d10e85a46ab5eb69166c9e0b3eec743b6c16e36186aafa1efb";
const SEAL_RAW: &str = "sha256:392a837ecdce40262e009575273869766c4475f8680f743391ca1c6caa528e8b";
...
fn python_output(release: &Path, arguments: &[&str]) -> std::process::Output {
    Command::new(python())
        ...
        .output()
        .expect("run Pulse 79 Python validation")
}

#[test]
fn pulse_79_ordered_capability_materialization_executor_stage_capture_bootstrap_argv_successor_is_sealed_and_fake_only(
) {
    let root = repo_root();
    ...
    let expected_paths = BTreeSet::from([
        ...
        "fixtures/p79-public-gate-catalog.json".to_owned(),
        ...
        "schemas/ferris.pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor.v1.schema.json"
            .to_owned(),
    ...
    assert!(sealed_source.contains("P79-SEALED-LOCK-CROSS-INSTANCE-REENTRY"));
    assert!(!sealed_source.contains("load_exact_p69_stack"));

    let (_, catalog) = read_json(release.join("fixtures/p79-public-gate-catalog.json"));
    assert_eq!(
        catalog["gate_ids"],
        serde_json::json!([
    ...
    assert_eq!(
        receipt["schema"],
        "ferris.pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor-qualification-envelope/v1"
    );
    assert_eq!(
    ...
    assert_eq!(
        schema["$id"],
        "https://ferris.dev/schema/ferris.pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor.v1.schema.json"
    );
    assert_eq!(
        schema["properties"]["schema"]["const"],
        "ferris.pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor-qualification/v1"
    );
    assert_eq!(
