...
const CUTOFF: &str = "6945f5fc96868c97267a1635fbb5219cc398eeb4";
const DOMAIN: &str = "ferris.process-exit-diagnostic-pulse-60-authority/v1";
const DECLARATION_IDENTITY: &str =
    "sha256:13ba3aaa5d61c536a9dd22b3a57816b1b7d93c2e11592c87117190709cbfb40c";
const MUTATION_COUNT: usize = 19_085;
const PRIOR_REGISTRY_TOTAL: usize = 100_582;
const REGISTRY_TOTAL: usize = 119_667;
...
fn assert_api_bindings(declaration: &Value) {
    ...
    assert_eq!(p59["role"], "sole-p60-production-runtime-callable");
    assert_eq!(
...
#[test]
fn pulse_60_authority_is_closed_self_excluding_and_unexecuted() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-60-authority.json"));
    let schema = read_json(
        held_out.join("schemas/ferris.process-exit-diagnostic-pulse-60-authority.v1.schema.json"),
    );
    assert_eq!(declaration["schema"], DOMAIN);
    assert_eq!(declaration["status"], "authorized-unexecuted");
    assert_eq!(
        declaration["program_id"],
        "FERRIS-P60-INDEPENDENT-PROCESS-EXIT-DIAGNOSTIC-WITNESS-PRESERVING-CAPABILITY-MATERIALIZATION-AUTHORITY"
    );
    ...
    assert_eq!(declaration["immutable_ferris"]["pulse_59_head"], CUTOFF);
    for field in [
    ...
    for path in [
        "docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-60-authority.json",
        "docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-60-authority-mutations.json",
        "docs/simulations/profile-diff-held-out/schemas/ferris.process-exit-diagnostic-pulse-60-authority.v1.schema.json",
        ...
        "crates/ferris-cli/tests/process_exit_diagnostic_pulse_60_authority.rs",
    ] {
...
#[test]
fn pulse_60_binds_cutoff_blobs_supported_variants_and_exact_p59_surface() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-60-authority.json"));
...
#[test]
fn pulse_60_preserves_permanent_closures_and_one_call_contract() {
    let root = repo_root();
    ...
    assert_eq!(
        pre_call["fresh_roots"]["private_runtime_root"],
        "fresh-absent"
    );
    assert_eq!(pre_call["fresh_roots"]["p27_cycle_root"], "fresh");
    assert_eq!(
        pre_call["fresh_roots"]["p41_final_copy_root"],
        "fresh-absent"
    );
    assert_eq!(
    ...
    for destination in [
        ...
        assert!(destination.contains("pulse-60-"), "Pulse 60 destination");
        assert!(
...
#[test]
fn pulse_60_validator_is_checkout_only_and_rejects_mutations() {
    let root = repo_root();
    ...
    let p60 = fs::read_to_string(
        root.join("crates/ferris-cli/tests/process_exit_diagnostic_pulse_60_authority.rs"),
    )
    .expect("P60 validator source");
    let legacy_raw_map = ["raw", "sha256", "by", "path"].join("_");
    assert!(
    ...
    assert!(
        p60.contains("cutoff_blob"),
        "P60 reads immutable cutoff blobs"
    );
    assert!(
        p60.contains("canonical_identity_by_path"),
        "P60 validates canonical identities"
    );
    assert!(
        !p60.contains(&legacy_raw_map),
        "P60 cannot compare working-tree-only raw identities"
    );
    ...
    assert!(
        !p60.contains(&python_helper)
            && !p60.contains(&pulse_59_python_tests)
            && !p60.contains(&pulse_58_python_tests),
        "P60 test must stay declaration/mutation/checkout only"
    );
    ...
    let mutations = read_json(
        held_out.join("fixtures/process-exit-diagnostic-pulse-60-authority-mutations.json"),
    );
    ...
    for (index, control) in controls.iter().enumerate() {
        ...
        assert_eq!(id, format!("P60-M{:05}", index + 1));
