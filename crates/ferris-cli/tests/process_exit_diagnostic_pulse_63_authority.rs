...
const CUTOFF: &str = "5ad78a0623611ad57797ec4e9da34345b40a6e38";
const PULSE_59_HEAD: &str = "6945f5fc96868c97267a1635fbb5219cc398eeb4";
const DOMAIN: &str = "ferris.process-exit-diagnostic-pulse-63-authority/v1";
const DECLARATION_IDENTITY: &str =
    "sha256:b8cfea5cc8cb6dc52a7974f4fee35f6351557158943cc92af388c534421915d5";
const MUTATION_COUNT: usize = 23_266;
const PRIOR_REGISTRY_TOTAL: usize = 161_369;
const REGISTRY_TOTAL: usize = 184_635;
...
const P57_BUNDLE_LONGEST: &str = "repository/docs/simulations/profile-diff-held-out/pulse-59-witness-preserving-capability-materialization-executor-release/schemas/ferris.pulse-59-witness-preserving-capability-materialization-executor.v1.schema.json";
...
fn assert_api_bindings(declaration: &Value) {
    ...
    assert_eq!(p59["role"], "sole-p63-production-runtime-callable");
    assert_eq!(
...
#[test]
fn pulse_63_binds_cutoff_containing_pulse_62_withdrawal_and_zero_execution() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-63-authority.json"));
    let schema = read_json(
        held_out.join("schemas/ferris.process-exit-diagnostic-pulse-63-authority.v1.schema.json"),
    );
    assert_eq!(declaration["schema"], DOMAIN);
    assert_eq!(declaration["status"], "authorized-unexecuted");
    assert_eq!(
        declaration["program_id"],
        "FERRIS-P63-INDEPENDENT-PROCESS-EXIT-DIAGNOSTIC-WITNESS-PRESERVING-CAPABILITY-MATERIALIZATION-AUTHORITY"
    );
    ...
    for path in [
        ...
        "docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-63-authority-mutations.json",
        "docs/simulations/profile-diff-held-out/schemas/ferris.process-exit-diagnostic-pulse-63-authority.v1.schema.json",
        ...
        "crates/ferris-cli/tests/process_exit_diagnostic_pulse_63_authority.rs",
    ] {
        assert_cutoff_absent(path);
    }
    let p62_wave = String::from_utf8(cutoff_blob(
        "context/waves/2026-08-12-platform-profile-conformance/pulses/pulse-62.md",
    ))
    .expect("UTF-8 Pulse 62 wave");
    assert!(p62_wave.contains("invalid-prelaunch-path-route-contract"));
    let p62_closeout = String::from_utf8(cutoff_blob(
        "docs/simulations/profile-diff-held-out/PROCESS_EXIT_DIAGNOSTIC_PULSE_62_AUTHORITY.md",
    ))
    .expect("UTF-8 Pulse 62 closeout");
    assert!(p62_closeout.contains("P62-REAL-PATH-WSL-ROUTE-CONTRACT"));
    assert_zero_execution_state(&declaration["execution_state"]);
}

#[test]
fn pulse_63_binds_cutoff_blobs_supported_variants_and_exact_p59_surface() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-63-authority.json"));
...
#[test]
fn pulse_63_records_withdrawn_pulse_62_and_binds_actual_path_and_wsl_route_contract() {
    let root = repo_root();
    ...
    assert_eq!(p62["closeout_commit"], CUTOFF);
    ...
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["repository_deepest_exact_relative_path"],
        P57_BUNDLE_LONGEST
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["repository_deepest_exact_relative_path_chars"],
        serde_json::json!(P57_BUNDLE_LONGEST.len())
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["repository_probe_path"],
        P57_BUNDLE_LONGEST
    );
    assert_eq!(
    ...
    assert_eq!(
        ubuntu_probe["exact_wsl_route_preflight_result_must_cover_both_probe_roots"],
        true
    );
    ...
    assert_eq!(
        wsl_preflight["wsl_command_prefix"],
        serde_json::json!([
            "%SystemRoot%\\System32\\wsl.exe",
            "--distribution",
            "Ubuntu-24.04",
    ...
    assert_eq!(
        wsl_preflight["result_json_fields"],
        serde_json::json!([
            "bundle_probe_root",
            "p56_probe_root",
            "platform",
    ...
    assert_eq!(wsl_preflight["input_json_max_bytes"], 16384);
    assert_eq!(wsl_preflight["result_json_max_bytes"], 65536);
    assert_eq!(
        wsl_preflight["native_ubuntu_runtime_parent_validation"]["no_mnt_prefix"],
        true
    );
    assert_eq!(
        wsl_preflight["script_verifications"]["platform_identity"],
        "ubuntu-24.04-x86_64"
    );
    assert_eq!(
        wsl_preflight["script_verifications"]["python_executable_exact"],
        "/usr/bin/python3"
    );
    assert_eq!(
        wsl_preflight["script_verifications"]["p57_bundle_topology_create_fsync_remove"],
        true
    );
    assert_eq!(
        wsl_preflight["script_verifications"]["p56_ubuntu_topology_create_fsync_remove"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        serde_json::json!([
            "P63-PRIVATE-RUNTIME-TOPOLOGY-PROBE",
            "P63-P41-ACTUAL-BASENAME-RENAME-PROBE",
            "P63-P59-TERMINAL-SIBLING-PROBE"
        ])
    );
    assert_eq!(
        platform_stops["native_linux"],
        serde_json::json!([
            "P63-EXACT-WSL-ROUTE-PREFLIGHT",
            "P63-UBUNTU-NOEXEC-PREREQUISITE"
        ])
    );
    ...
    assert_eq!(
        audit["p57_bundle_path_rules_worker_protocol_and_output_bounds_reaudited"],
        true
    );
    assert_eq!(
        audit["p57_native_linux_parent_wsl_executable_distribution_python_route_reaudited"],
        true
    );
    assert_eq!(
...
#[test]
fn pulse_63_validator_is_checkout_only_and_rejects_mutations() {
    ...
    let p63 = fs::read_to_string(
        root.join("crates/ferris-cli/tests/process_exit_diagnostic_pulse_63_authority.rs"),
    )
    ...
    let mutations = read_json(
        held_out.join("fixtures/process-exit-diagnostic-pulse-63-authority-mutations.json"),
    );
    ...
    for (index, control) in controls.iter().enumerate() {
        ...
        assert_eq!(id, format!("P63-M{:05}", index + 1));
