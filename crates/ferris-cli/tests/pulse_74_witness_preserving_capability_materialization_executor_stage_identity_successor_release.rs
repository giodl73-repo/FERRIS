...
const RELEASE: &str = "docs/simulations/profile-diff-held-out/pulse-74-witness-preserving-capability-materialization-executor-stage-identity-successor-release";
const MANIFEST_RAW: &str =
    "sha256:36cf64169c6f1e2d1e7835e135c4eac5006e10bde3a6c38836fc1e73150c8a9c";
const MANIFEST_AGGREGATE: &str =
    "sha256:50a2f35fba9ac9f47a6e2be82cf14482084ee85ff9d30a09e085feb31545b9f7";
const RECEIPT_RAW: &str = "sha256:0351316890a1983abd68decb1b7b9837e82e9c9cdf421ddf6039177b6fb0ff6f";
const RECEIPT_PAYLOAD: &str =
    "sha256:e20b457e5f386e2c23eb20aab6ea258acbdf9fac0fb7319661be15e299e1a8d9";
const SEAL_RAW: &str = "sha256:86900457b1827b6addac088acd989c3e1a45b793712d0611992c51d916be027b";
const SEAL_PAYLOAD: &str =
    "sha256:3c72cdeb1c2b107fa9ce031626f44e68c48824e62fb0c9dac348dfcecba4eacd";
const P73_COMMIT: &str = "450b4b4bd79b2217bdda368b1210b4ba7c519e0e";
...
fn python_output(release: &Path, arguments: &[&str]) -> std::process::Output {
    Command::new(python())
        ...
        .output()
        .expect("run Pulse 74 Python validation")
}

#[test]
fn pulse_74_witness_preserving_capability_materialization_executor_stage_identity_successor_is_sealed_and_fake_only(
) {
    let root = repo_root();
    ...
    let expected_paths = BTreeSet::from([
        ...
        "fixtures/p74-public-gate-catalog.json".to_owned(),
        ...
        "schemas/ferris.pulse-74-witness-preserving-capability-materialization-executor-stage-identity-successor.v1.schema.json"
            .to_owned(),
    ...
    assert!(source.contains("p73.run_ordered_capability_materialization_executor("));
    assert!(source.contains("p73._run_qualification_executor("));
    ...
    assert!(source.contains("class _Pulse74LinuxLockManager"));
    ...
    assert!(sealed_source.contains(&format!("P73_COMMIT = \"{P73_COMMIT}\"")));
    ...
    assert!(sealed_source.contains("_KERNEL_LOCK_NAMESPACE_PREFIX = \"ferris-p74\""));
    ...
    assert!(sealed_source.contains("_P74_INTERNAL_LOCK_MANAGER"));
    assert!(sealed_source.contains("_bind_internal_lock_manager"));
    assert!(sealed_source.contains("P74-SEALED-LOCK-CROSS-INSTANCE-REENTRY"));
    assert!(sealed_source.contains("_WINDOWS_WAIT_ABANDONED"));
    assert!(sealed_source.contains("if current is not dependencies"));
    assert!(sealed_source.contains("load_pulse73"));
    assert!(!sealed_source.contains("register_at_fork"));
    assert!(!sealed_source.contains("pulse-74-sealed-loader-locks"));
    ...
    let (_, catalog) = read_json(release.join("fixtures/p74-public-gate-catalog.json"));
    assert_eq!(
        catalog["gate_ids"],
        serde_json::json!([
    ...
    assert_eq!(
        manifest["schema"],
        "ferris.pulse-74-witness-preserving-capability-materialization-executor-stage-identity-successor-public-manifest/v1"
    );
    ...
    assert_eq!(manifest["predecessors"]["pulse_73"]["commit"], P73_COMMIT);
    assert_eq!(
        manifest["predecessors"]["pulse_73"]["source_sha256"],
    ...
    assert_eq!(
        receipt["schema"],
        "ferris.pulse-74-witness-preserving-capability-materialization-executor-stage-identity-successor-qualification-envelope/v1"
    );
    assert_eq!(
    ...
    assert_eq!(payload["p73_bound_commit"], P73_COMMIT);
    assert_eq!(payload["p73_publication_invocations"], 0);
    assert_eq!(payload["p74_publication_invocations"], 20);
    assert_eq!(
    ...
    assert_eq!(
        ...
        BTreeSet::from([
            ...
            "concurrent-p73-load-restores-foreign-sentinel".to_owned(),
            "exact-p73-binding-and-signature".to_owned(),
            ...
            "p73-import-exception-restores-generic-slot".to_owned(),
            "p73-prelaunch-failure-publication-not-attempted".to_owned(),
            ...
            "qualification-delegates-to-p73".to_owned(),
    ...
    assert_eq!(seal["payload"]["scope"]["exact_p73_execution"], true);
    assert_eq!(
        seal["payload"]["scope"]["exact_p72_stage_identity_chain"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        seal["payload"]["limits"]["publication_not_attempted_before_p73_completion"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        schema["$id"],
        "urn:ferris:schema:pulse-74-witness-preserving-capability-materialization-executor-stage-identity-successor:v1"
    );
    assert_eq!(
