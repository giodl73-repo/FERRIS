...
const CUTOFF: &str = "3a99e9e0f383a9821297ef47778fd586b447b7ba";
const PULSE_59_HEAD: &str = "6945f5fc96868c97267a1635fbb5219cc398eeb4";
const DOMAIN: &str = "ferris.process-exit-diagnostic-pulse-66-authority/v1";
const DECLARATION_IDENTITY: &str =
    "sha256:2cf44e16b0c61d79ed5ac889ab6fbfe46ee693ce6d9ccf2b4528bb877db45034";
const MUTATION_COUNT: usize = 27_156;
const PRIOR_REGISTRY_TOTAL: usize = 235_150;
const REGISTRY_TOTAL: usize = 262_306;
const HELD_OUT: &str = "docs/simulations/profile-diff-held-out";
const P39_DEEPEST: &str = "tests/test_checkout_verifier.py";
const REPO_LONGEST: &str = "docs/simulations/profile-diff-held-out/pulse-59-witness-preserving-capability-materialization-executor-release/schemas/ferris.pulse-59-witness-preserving-capability-materialization-executor.v1.schema.json";
const P57_BUNDLE_LONGEST: &str = "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/schemas/ferris.pulse-56-retained-build-receipt.v1.schema.json";
const P56_WINDOWS_ARTIFACT: &str =
    "ferris-windows-x86_64-29517d732db13cc2ffa304684b344f3538ab587d.exe";
const P56_WINDOWS_RECEIPT: &str =
    "ferris-windows-x86_64-29517d732db13cc2ffa304684b344f3538ab587d.exe.receipt.json";
const P56_UBUNTU_ARTIFACT: &str =
    "ferris-ubuntu-24.04-x86_64-29517d732db13cc2ffa304684b344f3538ab587d";
const P56_UBUNTU_RECEIPT: &str =
    "ferris-ubuntu-24.04-x86_64-29517d732db13cc2ffa304684b344f3538ab587d.receipt.json";
const P56_LAUNCH_ROOT: &str = "launches/l-0123456789abcdef01234567";
const P56_TARGET_A: &str = "work/target-a/release/deps/libaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-0123456789abcdef.rlib";
const P56_TARGET_B: &str = "work/target-b/release/build/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb-0123456789abcdef/out/build-script-probe.txt";
...
fn assert_api_bindings(declaration: &Value) {
    ...
    assert_eq!(p59["role"], "sole-p66-production-runtime-callable");
    assert_eq!(
...
fn pulse_66_binds_historical_cutoff_probe_dependency_and_zero_execution() {
    ...
    let schema = read_json(
        held_out.join("schemas/ferris.process-exit-diagnostic-pulse-66-authority.v1.schema.json"),
    );
    assert_eq!(declaration["schema"], DOMAIN);
    assert_eq!(declaration["status"], "authorized-unexecuted");
    assert_eq!(
        declaration["program_id"],
        "FERRIS-P66-INDEPENDENT-PROCESS-EXIT-DIAGNOSTIC-WITNESS-PRESERVING-CAPABILITY-MATERIALIZATION-AUTHORITY"
    );
    ...
    for path in [
        "docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-66-authority.json",
        "docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-66-authority-mutations.json",
        "docs/simulations/profile-diff-held-out/schemas/ferris.process-exit-diagnostic-pulse-66-authority.v1.schema.json",
        ...
        "crates/ferris-cli/tests/process_exit_diagnostic_pulse_66_authority.rs",
    ] {
        assert_cutoff_absent(path);
    }
    let p65_wave = String::from_utf8(cutoff_blob(
        "context/waves/2026-08-12-platform-profile-conformance/pulses/pulse-65.md",
    ))
    .expect("UTF-8 Pulse 65 wave");
    assert!(p65_wave.contains("invalid-prelaunch-wsl-spawn-cardinality-contract"));
    let p65_closeout = String::from_utf8(cutoff_blob(
        "docs/simulations/profile-diff-held-out/PROCESS_EXIT_DIAGNOSTIC_PULSE_65_AUTHORITY.md",
    ))
    .expect("UTF-8 Pulse 65 closeout");
    assert!(p65_closeout.contains("P65-P57-WSL-TWO-SPAWN-CONTRACT"));
...
fn pulse_66_binds_cutoff_blobs_supported_variants_and_exact_p59_surface() {
    ...
    let declaration_text = String::from_utf8(read_lf(
        held_out.join("fixtures/process-exit-diagnostic-pulse-66-authority.json"),
    ))
...
#[test]
fn pulse_66_records_withdrawn_pulse_65_and_binds_exact_p57_two_spawn_contract() {
    let root = repo_root();
    ...
    assert_eq!(p62["p59_callable_invocations"], 0);
    assert_eq!(p62["non_retryable"], true);
    assert_eq!(p62["non_resumable"], true);
    for field in ["retry", "resume", "reconstruction", "reseed", "reuse"] {
        assert_eq!(p62[field], false, "Pulse 62 closeout {field}");
    }
    for field in [
        "descriptors",
        "private_runtime_roots",
        ...
        assert_eq!(p62["artifacts"][field], 0, "Pulse 62 artifact {field}");
    }
    assert_eq!(
        p62["contract_review"]["path_qualification_declared"],
        "generic-synthetic-probe-topology"
    );
    assert_eq!(
        p62["contract_review"]["path_qualification_required"],
        "actual-caller-supplied-basenames-and-conservative-deepest-exact-paths"
    );
    assert_eq!(
        p62["contract_review"]["wsl_route_preflight_declared"],
        "implicit-ubuntu-parent-and-generic-p57-p56-child-creatability"
    );
    assert_eq!(
        p62["contract_review"]["wsl_route_preflight_required"],
        "exact-one-shot-windows-wsl-executable-distribution-python-route-preflight"
    );
    let p63 = &predecessors["pulse_63"];
    assert_eq!(
    ...
    assert_eq!(p63["withdrawn_as_recorded"], true);
    assert_eq!(p63["publication"], "not-attempted");
    assert_eq!(p63["authority_callable_invocations"], 0);
    assert_eq!(p63["p59_callable_invocations"], 0);
    assert_eq!(p63["non_retryable"], true);
    assert_eq!(p63["non_resumable"], true);
    for field in ["retry", "resume", "reconstruction", "reseed", "reuse"] {
        assert_eq!(p63[field], false, "Pulse 63 closeout {field}");
    }
    for field in [
        "descriptors",
        "private_runtime_roots",
        ...
        assert_eq!(p63["artifacts"][field], 0, "Pulse 63 artifact {field}");
    }
    assert_eq!(
        p63["contract_review"]["path_qualification_declared"],
        "actual-basenames-plus-generic-wsl-prefix-and-bounded-probe-json"
    );
    assert_eq!(
        p63["contract_review"]["path_qualification_required"],
        "actual-caller-supplied-basenames-and-conservative-deepest-exact-paths"
    );
    assert_eq!(
        p63["contract_review"]["wsl_route_preflight_declared"],
        "generic-prefix-separated-flags-smaller-bounds-p59-schema-path-and-public-probe-roots"
    );
    assert_eq!(
        p63["contract_review"]["wsl_route_preflight_required"],
        "exact-p57-stage-bundle-route-plus-byte-bound-worker-bootstrap-proof-and-private-parent-comparison"
    );
    let p64 = &predecessors["pulse_64"];
    assert_eq!(
    ...
    assert_eq!(
        p64["integrity_blocker"],
        "P64-WSL-QUALIFIER-SYSTEMROOT-DERIVATION"
    );
    assert_eq!(p64["withdrawn_as_recorded"], true);
    assert_eq!(p64["publication"], "not-attempted");
    assert_eq!(p64["authority_callable_invocations"], 0);
    assert_eq!(p64["p59_callable_invocations"], 0);
    assert_eq!(p64["non_retryable"], true);
    assert_eq!(p64["non_resumable"], true);
    for field in ["retry", "resume", "reconstruction", "reseed", "reuse"] {
        assert_eq!(p64[field], false, "Pulse 64 closeout {field}");
    }
    for field in [
        "descriptors",
        "private_runtime_roots",
        ...
        assert_eq!(p64["artifacts"][field], 0, "Pulse 64 artifact {field}");
    }
    assert_eq!(
        p64["contract_review"]["callable_surface_declared"],
        "optional-public-qualification-callable-branch"
    );
    assert_eq!(
        p64["contract_review"]["callable_surface_required"],
        "every-permitted-branch-callable-exists-and-is-byte-bound-at-cutoff"
    );
    assert_eq!(
        p64["contract_review"]["systemroot_derivation_declared"],
        "literal-percent-systemroot-placeholders"
    );
    assert_eq!(
        p64["contract_review"]["systemroot_derivation_required"],
        "source-precedence-systemroot-system32-wsl-cmd-derivation-plus-private-comparison-and-nonreparse-identity-proof"
    );
    assert_eq!(
        p64["contract_review"]["wsl_route_preflight_declared"],
        "same-stage-bundle-route-or-undefined-public-callable"
    );
    assert_eq!(
        p64["contract_review"]["wsl_route_preflight_required"],
        "only-implemented-exact-p57-stage-bundle-worker-bootstrap-route-with-systemroot-derivation"
    );
    let p65 = &predecessors["pulse_65"];
    assert_eq!(
        p65["declaration_identity"],
        "sha256:5bd7c876180a3bfb9f0bcb1518ef68921d1b28210d1f717c904753508e28abb0"
    );
    assert_eq!(
        p65["authority_commit"],
    ...
    assert_eq!(p65["integrity_blocker"], "P65-P57-WSL-TWO-SPAWN-CONTRACT");
    assert_eq!(p65["withdrawn_as_recorded"], true);
    assert_eq!(p65["publication"], "not-attempted");
    assert_eq!(p65["authority_callable_invocations"], 0);
    assert_eq!(p65["p59_callable_invocations"], 0);
    assert_eq!(p65["non_retryable"], true);
    assert_eq!(p65["non_resumable"], true);
    for field in ["retry", "resume", "reconstruction", "reseed", "reuse"] {
        assert_eq!(p65[field], false, "Pulse 65 closeout {field}");
    }
    for field in [
        "descriptors",
        "private_runtime_roots",
        ...
        assert_eq!(p65["artifacts"][field], 0, "Pulse 65 artifact {field}");
    }
    assert_eq!(
        p65["contract_review"]["wsl_spawn_cardinality_declared"],
        "single-wsl-process-spawn-for-stage-bundle-plus-worker-bootstrap"
    );
    assert_eq!(
        p65["contract_review"]["wsl_spawn_cardinality_required"],
        "exact-two-wsl-spawns-subprocess-run-then-popen-with-no-hidden-third-spawn"
    );
    assert_eq!(
        p65["contract_review"]["worker_preflight_declared"],
        "single-proof-without-distinct-worker-startup-close-lifecycle"
    );
    assert_eq!(
        p65["contract_review"]["worker_preflight_required"],
        "distinct-prestaged-probe-bundle-worker-ready-close-handshake-and-cleanup-proof"
    );
    assert_eq!(
    ...
    assert_eq!(
        predecessors["relationship_to_prior_permanent_dispositions"]["new_cutoff_containing_pulse_62_withdrawal"],
        true
    );
    assert_eq!(
        predecessors["relationship_to_prior_permanent_dispositions"]["new_cutoff_containing_pulse_63_withdrawal"],
        true
    );
    assert_eq!(
        predecessors["relationship_to_prior_permanent_dispositions"]["new_cutoff_containing_pulse_64_withdrawal"],
        true
    );
    assert_eq!(
        predecessors["relationship_to_prior_permanent_dispositions"]["new_cutoff_containing_pulse_65_withdrawal"],
        true
    );

    let authority = &declaration["authority"];
    assert_eq!(
        authority["pre_call_path_qualification_must_use_actual_basenames_or_conservative_exact_topology"],
        true
    );
    assert_eq!(
        authority["pre_call_exact_wsl_route_preflight_required"],
        true
    );
    assert_eq!(
        authority["pre_call_systemroot_derivation_and_identity_proof_required"],
        true
    );
    assert_eq!(
        authority["pre_call_undefined_callable_branches_forbidden"],
        true
    );
    assert_eq!(
        authority["pre_call_exact_wsl_two_spawn_cardinality_required"],
        true
    );
    assert_eq!(
        authority["pre_call_worker_probe_bundle_must_remain_fake_dependency_only"],
        true
    );
    ...
    assert_eq!(probes["actual_caller_supplied_root_basenames_only"], true);
    assert_eq!(
        probes["proof_must_fail_before_callable_if_any_exact_or_conservative_path_cannot_be_shown"],
        true
    );
    let stem = probes["probe_name_stem"].as_str().expect("probe stem");
    assert_eq!(stem, format!("{}-{}", &CUTOFF[..12], &PULSE_59_HEAD[..12]));
    assert_eq!(
    ...
    assert_eq!(
        private_runtime["parent_path_source"],
        "actual-caller-supplied-private_runtime_root"
    );
    assert_eq!(
        private_runtime["p58_namespace_probe_paths"],
        serde_json::json!([
            ".pulse58-private-launch",
            ".pulse58-private-launch/seed.bin",
            ".pulse58-private-launch/descriptors/case-manifest.json"
        ])
    );
    assert_eq!(
        private_runtime["p56_windows_probe_root_name"],
        ".p56-0123456789abcdef01234567"
    );
    assert_eq!(private_runtime["probe_file_name"], "probe.marker");
    assert_eq!(
        private_runtime["p56_windows_exact_topology"]["checkout_deepest_exact_relative_path"],
        REPO_LONGEST
    );
    assert_eq!(
        private_runtime["p56_windows_exact_topology"]["checkout_deepest_exact_relative_path_chars"],
        serde_json::json!(REPO_LONGEST.len())
    );
    assert_eq!(
        private_runtime["p56_windows_exact_topology"]["checkout_probe_paths"],
        serde_json::json!([
            format!("work/checkout-a/{REPO_LONGEST}"),
            format!("work/checkout-b/{REPO_LONGEST}")
        ])
    );
    assert_eq!(
        private_runtime["p56_windows_exact_topology"]["target_conservative_probe_paths"],
        serde_json::json!([P56_TARGET_A, P56_TARGET_B])
    );
    assert_eq!(
        private_runtime["p56_windows_exact_topology"]["custody_probe_paths"],
        serde_json::json!([
            format!("custody/{P56_WINDOWS_ARTIFACT}"),
            format!("custody/{P56_WINDOWS_RECEIPT}"),
            P56_LAUNCH_ROOT,
            "home",
            "tmp"
        ])
    ...
    assert_eq!(
        private_runtime["p56_windows_exact_topology"]["exact_shaped_topology_or_explicit_bound_required"],
        true
    );
    assert_eq!(
        private_runtime["p56_windows_exact_topology"]["fail_before_callable_if_unproven"],
        true
    );
    ...
    assert_eq!(
        p41_probes["probe_final_root_name_derivation"],
        "actual-caller-supplied-p41-final-root.name"
    );
    assert_eq!(
        p41_probes["probe_stage_root_name_derivation"],
        ".<actual-p41-final-root.name>.pulse-41-stage"
    );
    assert_eq!(
    ...
    assert_eq!(
        p41_probes["probe_copy_relative_paths"],
        serde_json::json!([P39_DEEPEST])
    );
    assert_eq!(
        p41_probes["exact_topology_proven"],
        serde_json::json!([
            "stage-create",
            "copy-deepest-p39-relative-path",
            "file-fsync",
            "stage-directory-fsync",
            "stage-to-final-rename",
            "final-verify",
            "rollback-delete",
            "parent-sync",
            "absence-verify"
    ...
    assert_eq!(
        terminal_probe["terminal_probe_child_name_derivation"],
        "<private_runtime_root.name>.pulse59-terminal-publication"
    );
    assert_eq!(
        terminal_probe["terminal_probe_child_name_uses_actual_runtime_basename"],
        true
    );
    assert_eq!(
        terminal_probe["reuses_actual_private_runtime_root_basename"],
        true
    );
    assert_eq!(terminal_probe["probe_file_name"], "probe.marker");
    assert_eq!(
        terminal_probe["exact_child_creation_proven"],
        serde_json::json!(["pulse59-terminal-sibling"])
    );
    let expected_p56_release_paths = serde_json::json!([
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/README.md",
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/generate_release.py",
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/public-manifest.json",
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/qualification-receipt.json",
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/qualify.py",
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/release-seal.json",
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/retained_build_custody.py",
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/root-cause-report.md",
        ...
        "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/tests/test_retained_build_custody.py"
    ]);
    let expected_staged_files = serde_json::json!([
    ...
    assert_eq!(ubuntu_probe["parent"], "ubuntu_runtime_parent");
    assert_eq!(
        ubuntu_probe["parent_path_source"],
        "actual-caller-supplied-ubuntu_runtime_parent"
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_probe_root_name"],
        ".p57-0123456789abcdef0123456789abcdef"
    );
    assert_eq!(
        ubuntu_probe["p56_ubuntu_probe_root_name"],
        ".p56-0123456789abcdef01234567"
    );
    assert_eq!(
    ...
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["deepest_exact_relative_path"],
        P57_BUNDLE_LONGEST
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["deepest_exact_relative_path_chars"],
        serde_json::json!(P57_BUNDLE_LONGEST.len())
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["p56_release_paths"],
        expected_p56_release_paths
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["staged_file_paths_in_order"],
        expected_staged_files
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["exact_file_count"],
        12
    );
    assert_eq!(
        ubuntu_probe["p57_bundle_exact_topology"]["canonical_json_only"],
        true
    );
    assert_eq!(
        ubuntu_probe["p56_ubuntu_exact_topology"]["checkout_deepest_exact_relative_path"],
        REPO_LONGEST
    );
    assert_eq!(
        ubuntu_probe["p56_ubuntu_exact_topology"]["checkout_probe_paths"],
        serde_json::json!([
            format!("work/checkout-a/{REPO_LONGEST}"),
            format!("work/checkout-b/{REPO_LONGEST}")
        ])
    ...
    assert_eq!(
        ubuntu_probe["p56_ubuntu_exact_topology"]["target_conservative_probe_paths"],
        serde_json::json!([P56_TARGET_A, P56_TARGET_B])
    );
    assert_eq!(
        ubuntu_probe["p56_ubuntu_exact_topology"]["custody_probe_paths"],
        serde_json::json!([
            format!("custody/{P56_UBUNTU_ARTIFACT}"),
            format!("custody/{P56_UBUNTU_RECEIPT}"),
            P56_LAUNCH_ROOT,
            "home",
            "tmp"
        ])
    ...
    assert_eq!(
        ubuntu_probe["p56_ubuntu_exact_topology"]["exact_shaped_topology_or_explicit_bound_required"],
        true
    );
    assert_eq!(
        ubuntu_probe["p56_ubuntu_exact_topology"]["fail_before_callable_if_unproven"],
        true
    );
    assert_eq!(
        ubuntu_probe["exact_stage_stdout_bundle_root_formula_required"],
        true
    );
    assert_eq!(
        ubuntu_probe["exact_worker_bootstrap_p56_root_formula_required"],
        true
    );
    assert_eq!(
        ubuntu_probe["private_parent_path_compare_only_record_permitted"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(wsl_preflight["invocations_exact"], 1);
    assert_eq!(wsl_preflight["no_retry"], true);
    assert_eq!(wsl_preflight["single_wsl_process_spawn"], false);
    assert_eq!(wsl_preflight["wsl_process_spawns_exact"], 2);
    assert_eq!(
        wsl_preflight["spawn_order_exact"],
        serde_json::json!([
            "stage-bundle-subprocess.run",
            "worker-bootstrap-subprocess.Popen"
        ])
    );
    assert_eq!(wsl_preflight["hidden_third_wsl_spawn_forbidden"], true);
    assert_eq!(
        wsl_preflight["separate_stage_bundle_and_worker_bootstrap_proofs_required"],
        true
    );
    assert_eq!(
        wsl_preflight["worker_bootstrap_proof_uses_prestaged_probe_bundle"],
        true
    );
    assert_eq!(
        wsl_preflight["worker_bootstrap_startup_ready_plus_close_handshake_only"],
        true
    );
    assert_eq!(
        wsl_preflight["worker_bootstrap_launch_request_forbidden_during_preflight"],
        true
    );
    assert_eq!(
        wsl_preflight["fake_dependency_only_publish_and_close_during_worker_preflight"],
        true
    );
    assert_eq!(wsl_preflight["distribution"], "Ubuntu-24.04");
    assert_eq!(wsl_preflight["python_executable"], "/usr/bin/python3");
    assert_eq!(
        wsl_preflight["resolved_windows_wsl_executable_formula"],
        "os.fspath(Path(system_root) / \"System32\" / \"wsl.exe\")"
    );
    assert_eq!(
        wsl_preflight["resolved_windows_wsl_executable_public_comparison_token"],
        "<derived-wsl-executable>"
    );
    assert_eq!(
        wsl_preflight["exact_stage_bundle_and_worker_bootstrap_route_only"],
        true
    );
    assert_eq!(
        wsl_preflight["optional_public_qualification_callable_branches_forbidden"],
        true
    );
    assert_eq!(
        wsl_preflight["isolated_python_flags"],
        serde_json::json!(["-I", "-S", "-B"])
    );
    assert_eq!(
        wsl_preflight["environment_allowlist"],
        serde_json::json!(["ComSpec", "PATH", "SystemRoot"])
    );
    assert_eq!(
        wsl_preflight["environment_exact_formulas"],
        serde_json::json!({
    ...
    assert_eq!(
        wsl_preflight["environment_public_comparison_tokens"],
        serde_json::json!({
            "ComSpec": "<derived-comspec>",
            "PATH": "<derived-system32>",
            "SystemRoot": "<resolved-system-root>"
        })
    );
    assert_eq!(wsl_preflight["harmless_bounded_script_only"], true);
    assert_eq!(wsl_preflight["no_ferris_or_p56_callable_execution"], true);
    assert_eq!(
        wsl_preflight["fails_before_callable_if_any_path_or_route_assertion_is_unproven"],
        true
    );
    assert_eq!(
        wsl_preflight["native_ubuntu_runtime_parent_validation"],
        serde_json::json!({
            "absolute_path_required": true,
            "no_mnt_prefix": true,
            "no_nul_cr_lf": true,
            "not_publicly_serialized": true
        })
    );
    assert!(wsl_preflight.get("public_qualification_callable").is_none());
    let derivation = &wsl_preflight["system_root_derivation"];
    assert_eq!(
        derivation["environment_lookup_precedence"],
        serde_json::json!(["SystemRoot", "SYSTEMROOT"])
    );
    assert_eq!(
        derivation["lookup_expression"],
        "os.environ.get(\"SystemRoot\") or os.environ.get(\"SYSTEMROOT\")"
    );
    assert_eq!(
        derivation["literal_percent_systemroot_placeholders_forbidden"],
        true
    );
    assert_eq!(
        derivation["public_contract_must_name_derivation_and_comparison_only"],
        true
    );
    assert_eq!(
        derivation["system32_formula"],
        "os.fspath(Path(system_root) / \"System32\")"
    );
    assert_eq!(
        derivation["cmd_executable_formula"],
        "os.fspath(Path(system32) / \"cmd.exe\")"
    );
    assert_eq!(
        derivation["wsl_executable_formula"],
        "os.fspath(Path(system_root) / \"System32\" / \"wsl.exe\")"
    );
    assert_eq!(
        derivation["system_root_validation"],
        serde_json::json!({
            "absolute_path_required": true,
            "directory_required": true,
            "drive_or_unc_root_required": true,
            "no_dot_or_dotdot_segments": true,
            "no_nul_cr_lf": true,
            "non_reparse_directory_required": true
        })
    );
    assert_eq!(
        derivation["derived_identity_requirements"],
        serde_json::json!({
            "system_root_directory_exists": true,
            "system_root_directory_non_reparse": true,
            "system32_directory_exists": true,
            "system32_directory_non_reparse": true,
            "wsl_executable_regular_file_exists": true,
            "wsl_executable_regular_file_non_reparse": true,
            "cmd_executable_regular_file_exists": true,
            "cmd_executable_regular_file_non_reparse": true
        })
    );
    assert_eq!(
        derivation["public_comparison_requirements"],
        serde_json::json!({
            "argv0_equals_derived_wsl_executable": true,
            "environment_comspec_equals_derived_cmd_executable": true,
            "environment_path_equals_derived_system32": true,
            "environment_systemroot_equals_resolved_system_root": true
        })
    );
    let stage_bundle = &wsl_preflight["stage_bundle_route"];
    assert_eq!(
        stage_bundle["argv_shape"],
        serde_json::json!([
            "<derived-wsl-executable>",
            "--distribution",
            "Ubuntu-24.04",
            ...
            "-B",
            "-c",
            "<exact-_WSL_BUNDLE_BOOTSTRAP>",
            "<ubuntu_runtime_parent>",
            "<name>"
        ])
    );
    assert_eq!(
        stage_bundle["argv0_public_comparison_token"],
        "<derived-wsl-executable>"
    );
    ...
    assert_eq!(stage_bundle["bundle_name_hex_suffix_chars"], 32);
    assert_eq!(stage_bundle["bundle_name_lowercase_hex_required"], true);
    assert_eq!(stage_bundle["bundle_name_slash_forbidden"], true);
    assert_eq!(
        stage_bundle["bundle_root_formula"],
        "ubuntu_runtime_parent.rstrip('/') + '/' + name"
    );
    assert_eq!(
        stage_bundle["post_bootstrap_arguments_exact"],
        serde_json::json!(["<ubuntu_runtime_parent>", "<name>"])
    );
    assert_eq!(stage_bundle["python_executable_exact"], "/usr/bin/python3");
    assert_eq!(
        stage_bundle["python_identity_fields"],
        serde_json::json!(["executable", "version"])
    );
    assert_eq!(stage_bundle["python_version_triplet_required"], true);
    assert_eq!(stage_bundle["stderr_must_be_empty"], true);
    assert_eq!(
        stage_bundle["stdout_fields"],
        serde_json::json!(["bundle_root", "python", "schema"])
    );
    assert_eq!(
        stage_bundle["stdout_schema"],
        "ferris.pulse-57-wsl-bundle-staged/v1"
    );
    assert_eq!(stage_bundle["stdout_single_canonical_json_line"], true);
    assert_eq!(stage_bundle["stdout_max_bytes"], 2_800_000);
    assert_eq!(stage_bundle["timeout_seconds_exact"], 15);
    ...
    assert_eq!(
        worker_route["argv0_public_comparison_token"],
        "<derived-wsl-executable>"
    );
    ...
    assert_eq!(
        ...
        P66_PROBE_DEPENDENCY_SHA256
    );
    assert_eq!(
        worker_route["expected_p56_root"],
        "<probe_bundle_root>/repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release"
    );
    ...
    assert_eq!(worker_route["kill_timeout_seconds_exact"], 5);
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "no_wslenv": true,
            "worker_checks_exact_p56_root": true,
            "worker_derives_repo_root_as_p56_root_parents_3": true
        })
    );
    let probe_bundle = &worker_route["probe_bundle"];
    assert_eq!(probe_bundle["host_side_prestaging_only"], true);
    assert_eq!(probe_bundle["separate_from_stage_bundle_spawn"], true);
    assert_eq!(probe_bundle["wsl_spawn_required_for_prestaging"], false);
    assert_eq!(probe_bundle["staged_file_count"], 3);
    assert_eq!(
        probe_bundle["staged_file_paths_in_order"],
        serde_json::json!([
            "worker/wsl_session_worker.py",
            "worker/sealed_dependencies.py",
            "repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/retained_build_custody.py"
    ...
    assert_eq!(
        probe_bundle["worker_source_cutoff_path"],
        "docs/simulations/profile-diff-held-out/pulse-57-capability-bound-diagnostic-executor-release/wsl_session_worker.py"
    );
    assert_eq!(
        probe_bundle["worker_source_cutoff_identity"],
        serde_json::json!({
            "sha256": "sha256:9b0d91f7c4e2aed57d7dc40b95f5860f017138717364d3399d132884047904cb",
            "size": 12297,
            "source": "immutable-git-blob-at-cutoff"
        })
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "sha256": P66_PROBE_DEPENDENCY_SHA256,
            "size": P66_PROBE_DEPENDENCY_SIZE,
            "source": "immutable-git-blob-at-cutoff"
        })
    );
    assert_eq!(probe_bundle["fake_dependency_callable"], "load_exact_p56");
    assert_eq!(
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "sha256": P56_SOURCE_SHA256,
            "size": 56548,
            "source": "immutable-git-blob-at-cutoff"
        })
    );
    assert_eq!(
        probe_bundle["placeholder_p56_stage_destination"],
        "<probe_bundle_root>/repository/docs/simulations/profile-diff-held-out/pulse-56-retained-build-custody-release/retained_build_custody.py"
    );
    let fake_scope = &worker_route["fake_dependency_scope"];
    assert_eq!(
        fake_scope["exact_route_semantics_proven"],
        serde_json::json!([
            "worker-bootstrap-argv-order-and-derived-wsl-executable-comparison",
            "exact-worker-source-sha256-argument-and-staged-worker-path",
            "descriptor-open-hash-compile-load-of-staged-sealed-dependencies",
            "exact-p56-root-equality-check-and-repo-root-parent-derivation",
            "ready-message-read-under-protocol-timeout-and-byte-bound",
            "exact-close-request-write-flush-stdin-close-wait-terminate-kill-drain-lifecycle",
            "residual-output-or-nonzero-exit-fails-cleanup"
        ])
    );
    assert_eq!(
        fake_scope["fake_dependency_semantics_only"],
        serde_json::json!([
            "worker-uses-p66-probe-sealed-dependencies-instead-of-production-sealed_dependencies",
            "load_exact_p56-returns-a-harmless-fake-module",
            "fake-publish-retained-build-and-custody-produces-no-real-capability",
            "fake-close-custody-confirms-close-only",
            "fake-launch-verified-is-forbidden-and-unreached-during-preflight"
        ])
    );
    assert_eq!(
        fake_scope["production_semantics_not_executed"],
        serde_json::json!([
            "production-sealed_dependencies.py",
            "real-pulse-56.publish_retained_build_and_custody",
            "real-pulse-56.launch_verified",
            "any-ferris-binary-launch"
        ])
    );
    ...
    assert_eq!(
        private_compare["actual_ubuntu_runtime_parent_may_be_recorded_privately"],
        true
    );
    assert_eq!(
        private_compare["private_record_must_compare_caller_argument_to_observed_path"],
        true
    );
    assert_eq!(
        private_compare["bundle_root_comparison_may_remain_private"],
        true
    );
    assert_eq!(
        private_compare["durable_public_record_must_not_serialize_actual_path"],
        true
    );
    assert_eq!(
        private_compare["durable_public_record_status_or_digest_only_if_needed"],
        true
    );
    assert_eq!(
        private_compare["concrete_windows_derivation_values_recorded_privately"],
        serde_json::json!([
            "system_root",
            "system32",
            "resolved_windows_wsl_executable",
            "resolved_windows_cmd_executable"
        ])
    );
    assert_eq!(
        private_compare["concrete_windows_derivation_digest_algorithm"],
        "sha256-utf8-path-bytes"
    );
    assert_eq!(
        private_compare["concrete_windows_derivation_sha256_digests_recorded_privately"],
        true
    );
    assert_eq!(
        private_compare["private_record_must_compare_argv0_and_environment_to_derived_values"],
        true
    );
    assert_eq!(
        private_compare["durable_public_record_must_not_serialize_concrete_windows_paths"],
        true
    );
    assert_eq!(
        private_compare["durable_public_record_names_only_formulas_comparisons_and_optional_digests"],
        true
    );
    assert_eq!(
        private_compare["durable_public_record_must_not_serialize_actual_path"],
    ...
    assert_eq!(
        declaration["execution_order"]["wsl_route_preflight_invocations_exact"],
        1
    );
    assert_eq!(
        declaration["execution_order"]["wsl_route_preflight_retry_permitted"],
        false
    );
    assert_eq!(
        declaration["execution_order"]["wsl_route_preflight_failures_stop_before_p59"],
        true
    );
    assert_eq!(
        declaration["execution_order"]["ordered_steps"][5],
        "ubuntu-runtime-parent-exact-p57-two-wsl-spawn-stage-bundle-and-worker-bootstrap-preflight"
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_exact_wsl_route_preflight_required"],
        true
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_exact_wsl_route_preflight_invocations_exact"],
        1
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_exact_wsl_route_preflight_retry_permitted"],
        false
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_systemroot_derivation_and_identity_proof_required"],
        true
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_exact_wsl_two_spawn_cardinality_required"],
        true
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_exact_wsl_route_preflight_wsl_process_spawns_exact"],
        2
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_worker_probe_bundle_must_remain_fake_dependency_only"],
        true
    );
    assert_eq!(
        declaration["runtime_binding"]["pre_call_undefined_callable_branches_forbidden"],
        true
    );
    ...
    assert_eq!(
        ...
        serde_json::json!([
            "P66-PRIVATE-RUNTIME-TOPOLOGY-PROBE",
            "P66-P41-ACTUAL-BASENAME-RENAME-PROBE",
            "P66-P59-TERMINAL-SIBLING-PROBE"
        ])
    );
    assert_eq!(
        platform_stops["native_linux"],
        serde_json::json!([
            "P66-EXACT-P57-WSL-STAGE-BUNDLE-SUBPROCESS-RUN-PREFLIGHT",
            "P66-SYSTEMROOT-SOURCE-PRECEDENCE-DERIVATION",
            "P66-WINDOWS-WSL-CMD-NONREPARSE-IDENTITY",
            "P66-EXACT-P57-WORKER-PROBE-BUNDLE",
            "P66-EXACT-P57-WORKER-BOOTSTRAP-POPEN-PREFLIGHT",
            "P66-P57-WSL-TWO-SPAWN-CARDINALITY",
            "P66-UBUNTU-NOEXEC-PREREQUISITE"
        ])
    );
    ...
    assert_eq!(audit["p56_checkout_and_target_path_floors_reaudited"], true);
    assert_eq!(
        audit["p56_restrictive_child_modes_directory_sync_custody_and_launch_paths_reaudited"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(audit["p57_exact_two_spawn_cardinality_reaudited"], true);
    assert_eq!(audit["p57_no_hidden_third_spawn_reaudited"], true);
    assert_eq!(
        audit["p57_worker_probe_bundle_fake_dependency_scope_reaudited"],
        true
    );
    assert_eq!(audit["p57_worker_ready_close_lifecycle_reaudited"], true);
    assert_eq!(
        audit["p58_namespace_absence_child_creation_and_seed_path_reaudited"],
        true
    );
    assert_eq!(
        audit["p59_terminal_parent_child_creation_and_suffix_derivation_reaudited"],
        true
    );
    assert_eq!(
        audit["systemroot_source_precedence_and_derived_windows_paths_reaudited"],
        true
    );
    assert_eq!(
        audit["systemroot_derived_windows_regular_non_reparse_identity_reaudited"],
        true
    );
    assert_eq!(
        audit["every_permitted_runtime_callable_byte_bound_at_cutoff_reaudited"],
        true
    );
    assert_eq!(
        pre["private_runtime_root"]["p58_namespace_child_proven_pre_call"],
        true
    );
    assert_eq!(
        pre["private_runtime_root"]["p56_windows_custody_child_proven_pre_call"],
        true
    );
    assert_eq!(
        pre["private_runtime_root"]["exact_p58_namespace_leaf_names_proven_pre_call"],
        true
    );
    assert_eq!(
        pre["private_runtime_root"]["exact_p56_windows_topology_proven_pre_call"],
        true
    );
    assert_eq!(
        pre["p41_public_custody_roots"]["actual_final_root_basename_required"],
        true
    );
    assert_eq!(
        pre["p41_public_custody_roots"]["actual_final_root_basename_derived_stage_root_required"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        pre["p59_terminal_root"]["actual_private_runtime_root_basename_required"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        pre["ubuntu_runtime_parent"]["exact_p56_ubuntu_topology_proven_pre_call"],
        true
    );
    assert_eq!(
        pre["ubuntu_runtime_parent"]["exact_wsl_route_preflight_required"],
        true
    );
    assert_eq!(
        pre["ubuntu_runtime_parent"]["absolute_native_linux_path_string"],
        true
    );
    assert_eq!(pre["ubuntu_runtime_parent"]["native_linux_required"], true);
    assert_eq!(pre["ubuntu_runtime_parent"]["not_mnt_mount"], true);
    assert_eq!(
        pre["ubuntu_runtime_parent"]["path_not_publicly_serialized"],
        true
    );
    assert_eq!(
        pre["ubuntu_runtime_parent"]["custodian_private_compare_only_record_required"],
        true
    );
    assert_eq!(
        pre["ubuntu_runtime_parent"]["durable_public_status_or_digest_only_if_path_sensitive"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(terminal["source_paths_never_publicly_serialized"], true);
    assert_eq!(terminal["private_record_or_root_path_disclosure"], false);
    assert_eq!(
        terminal["published_result"]["p43_destination"],
        "docs/simulations/profile-diff-held-out/pulse-66-public-result/"
    );
    assert_eq!(
    ...
    assert_eq!(
        terminal["not_attempted_prelaunch_or_runtime_failure"]["permanent_closeout"],
        true
    );
    assert_eq!(
        terminal["not_attempted_prelaunch_or_runtime_failure"]["retry_or_resume_permitted"],
        false
    );
    ...
    let worker = String::from_utf8(cutoff_blob(
        "docs/simulations/profile-diff-held-out/pulse-57-capability-bound-diagnostic-executor-release/wsl_session_worker.py",
    ))
    .expect("UTF-8 Pulse 57 worker");
    ...
    assert!(p57.contains("\"PATH\": system32"));
    assert!(p57.contains("\"SystemRoot\": system_root"));
    assert!(p57.contains("return os.fspath(Path(system_root) / \"System32\" / \"wsl.exe\")"));
    assert!(p57.contains("MAX_BUNDLE_BYTES = 1_048_576"));
    assert!(p57.contains("PROTOCOL_TIMEOUT_SECONDS = 15"));
    assert!(p57.contains("CLOSE_TIMEOUT_SECONDS = 5"));
    ...
    assert!(p57.contains(r#"set(response) != {"bundle_root", "python", "schema"}"#));
    assert!(p57.contains(r#"response["schema"] != "ferris.pulse-57-wsl-bundle-staged/v1""#));
    assert!(p57.contains(r#"response["bundle_root"] != expected_root"#));
    assert!(p57.contains(r#"staged.root + "/worker/wsl_session_worker.py""#));
    assert!(p57.contains("pulse-56-retained-build-custody-release"));
    assert!(p56.contains(
        r#"run = _fresh_child(parent, f".p56-{secrets.token_hex(12)}", "P56-RUNTIME-ROOT")"#
    ));
    assert!(p56.contains(r#"checkout = _fresh_child(work, f"checkout-{label}", "P56-WORK-ROOT")"#));
    ...
    assert!(worker.contains("expected_p56 = ("));
    assert!(worker.contains("pulse-56-retained-build-custody-release"));
    assert!(worker.contains("if p56_root != expected_p56:"));
    assert!(worker.contains("dependencies = _load_sealed_dependencies(bundle_root)"));
    assert!(worker.contains("repo_root = p56_root.parents[3]"));
    let probe = String::from_utf8(cutoff_blob(P66_PROBE_DEPENDENCY)).expect("UTF-8 P66 probe");
    assert!(probe.contains("def load_exact_p56(repo_root: Path) -> ModuleType:"));
    assert!(probe.contains("module.__file__ = str(root / \"retained_build_custody.py\")"));
    assert!(probe.contains("raise ReleaseFailure(\"P56-LAUNCH-FORBIDDEN\")"));
}
...
fn pulse_66_validator_is_checkout_only_and_rejects_mutations() {
    ...
    let p65 = fs::read_to_string(
        root.join("crates/ferris-cli/tests/process_exit_diagnostic_pulse_66_authority.rs"),
    )
    .expect("P66 validator source");
    let legacy_raw_map = ["raw", "sha256", "by", "path"].join("_");
    assert!(
    ...
    assert!(
        p65.contains("cutoff_blob"),
        "P66 reads immutable cutoff blobs"
    );
    assert!(
        p65.contains("canonical_identity_by_path"),
        "P66 validates canonical identities"
    );
    assert!(
        !p65.contains(&legacy_raw_map),
        "P66 cannot compare working-tree-only raw identities"
    );
    ...
    assert!(
        !p65.contains(&python_helper)
            && !p65.contains(&pulse_59_python_tests)
            && !p65.contains(&pulse_58_python_tests),
        "P66 test must stay declaration/mutation/checkout only"
    );
    ...
    let mutations = read_json(
        held_out.join("fixtures/process-exit-diagnostic-pulse-66-authority-mutations.json"),
    );
    ...
    for (index, control) in controls.iter().enumerate() {
        ...
        assert_eq!(id, format!("P66-M{:05}", index + 1));
