...
const RELEASE: &str = "docs/simulations/profile-diff-held-out/pulse-88-witness-preserving-capability-materialization-executor-wsl-parent-owner-binding-successor-release";
const MANIFEST_RAW: &str =
    "sha256:bce57c9e2f9ee3518be2be555309df67c7302819f67f35688fcfb9ae283f06bb";
const MANIFEST_AGGREGATE: &str =
    "sha256:dc3881fa07e5dafa5e3c25583adef7b3ced9e1cf8137df6a05ca37bd3015e65b";
const RECEIPT_RAW: &str = "sha256:9f2094f9eac5f2529cd3e92111edb72f301e24f0282840dcfab6f1642b68a344";
const RECEIPT_PAYLOAD: &str =
    "sha256:93b88d12916359d0dbf59b9aae1b81f12de53eda362a008ecf0fce82223ad024";
const SEAL_RAW: &str = "sha256:96964db2e843990ca9b9a27f60160bfff41155eb74aa1bb8aabbc213fc826aec";
const SEAL_PAYLOAD: &str =
    "sha256:dd279ed69465977e14209ee6a1ad7d91dc74368dcc342446a81e760d2cd2f5b6";
const P87_COMMIT: &str = "efa98a4e5b2fc138458c1ead45dbb7796cf00290";
...
fn python_output(release: &Path, arguments: &[&str]) -> std::process::Output {
    Command::new(python())
        ...
        .output()
        .expect("run Pulse 88 Python validation")
}

#[test]
fn pulse_88_witness_preserving_capability_materialization_executor_wsl_parent_owner_binding_successor_is_sealed_and_fake_only(
) {
    let root = repo_root();
    ...
    let expected_paths = BTreeSet::from([
        ...
        "fixtures/p88-public-gate-catalog.json".to_owned(),
        ...
        "schemas/ferris.pulse-88-witness-preserving-capability-materialization-executor-wsl-parent-owner-binding-successor.v1.schema.json"
            .to_owned(),
    ...
    assert!(source.contains("p87.run_ordered_capability_materialization_executor("));
    assert!(source.contains("p87._run_qualification_executor("));
    ...
    assert!(source.contains("class _Pulse88LinuxLockManager"));
    ...
    assert!(sealed_source.contains(&format!("P87_COMMIT = \"{P87_COMMIT}\"")));
    ...
    assert!(sealed_source.contains("_KERNEL_LOCK_NAMESPACE_PREFIX = \"ferris-p88\""));
    ...
    assert!(sealed_source.contains("_P88_INTERNAL_LOCK_MANAGER"));
    assert!(sealed_source.contains("_bind_internal_lock_manager"));
    assert!(sealed_source.contains("P88-SEALED-LOCK-CROSS-INSTANCE-REENTRY"));
    assert!(sealed_source.contains("_WINDOWS_WAIT_ABANDONED"));
    assert!(sealed_source.contains("if current is not dependencies"));
    assert!(sealed_source.contains("load_pulse87"));
    assert!(!sealed_source.contains("register_at_fork"));
    assert!(!sealed_source.contains("pulse-88-sealed-loader-locks"));
    ...
    let (_, catalog) = read_json(release.join("fixtures/p88-public-gate-catalog.json"));
    assert_eq!(
        catalog["gate_ids"],
        serde_json::json!([
    ...
    assert_eq!(
        manifest["schema"],
        "ferris.pulse-88-witness-preserving-capability-materialization-executor-wsl-parent-owner-binding-successor-public-manifest/v1"
    );
    ...
    assert_eq!(manifest["predecessors"]["pulse_87"]["commit"], P87_COMMIT);
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["source_sha256"],
        "sha256:33056b187769e12ee031d7c6e33a54e5edd5bbb1d7db566b7c7b7de7de4c16fb"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["manifest_raw_sha256"],
        "sha256:7e072766229d7f04eb1da5e99ec079b0fc1777cd5615dff15309298dcca32ccc"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["manifest_aggregate"],
        "sha256:2cfdadfa1295dfee6155bb877276e407acc15a3046db3236b98c24a33a11e8ec"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["receipt_raw_sha256"],
        "sha256:8ff1a07bcdf48d96bab5e41da0d7afff807d2f5a8ea0078a6e2c5d21f2c0ee40"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["receipt_payload_sha256"],
        "sha256:b7c9b232fd40bc566b3b2300581b7d1cdd03fd661930f905ad27b701134ddbeb"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["seal_raw_sha256"],
        "sha256:8c5614e02965d1696940771807ad45a937cc154dce568e7df5d2cd8aab668eb8"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["seal_payload_sha256"],
        "sha256:26f8a8746354cb37b02461f11fd0e01e7038a802ceee416c3b508aef5a0a035e"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["stack"]["pulse_86"]["commit"],
        "9f87d726d886ffd864c511268e5bdd0d021686d8"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["stack"]["pulse_86"]["release_root"],
        "docs/simulations/profile-diff-held-out/pulse-86-wsl-parent-owner-binding-capability-executor-successor-release"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["stack"]["pulse_35"]["receipt"],
        "sha256:4c4f4ad1d9fa437e23f655083eb74c754114c5bea43ae111d2127fc7f051a037"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["stack"]["pulse_35"]["receipt_payload"],
        "sha256:7f1154ca94009cef966ab2f43ba74a9f017989ed5dbbdbfd8c3ce8fe64fe5cee"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["stack"]["pulse_35"]["seal"],
        "sha256:17459123c674f2664d7d09ea03c00dcba72129bb1cf532cfe11f8cf4edeffd23"
    );
    assert_eq!(
        manifest["predecessors"]["pulse_87"]["stack"]["pulse_35"]["seal_payload"],
    ...
    assert_eq!(
        receipt["schema"],
        "ferris.pulse-88-witness-preserving-capability-materialization-executor-wsl-parent-owner-binding-successor-qualification-envelope/v1"
    );
    assert_eq!(
    ...
    assert_eq!(payload["behavioral_control_tests_run"], 40);
    assert_eq!(payload["behavioral_control_tests_passed"], 40);
    assert_eq!(payload["p87_bound_commit"], P87_COMMIT);
    assert_eq!(payload["p87_publication_invocations"], 0);
    assert_eq!(payload["p88_publication_invocations"], 20);
    assert_eq!(
    ...
    assert_eq!(
        ...
        BTreeSet::from([
            ...
            "concurrent-p87-load-restores-foreign-sentinel".to_owned(),
            "exact-p87-binding-and-signature".to_owned(),
            ...
            "p86-stage-indeterminate-publication-not-attempted".to_owned(),
            "p87-import-exception-restores-generic-slot".to_owned(),
            "p87-prelaunch-failure-publication-not-attempted".to_owned(),
            ...
            "qualification-delegates-to-p87".to_owned(),
    ...
    assert_eq!(seal["payload"]["scope"]["exact_p87_execution"], true);
    assert_eq!(
        seal["payload"]["scope"]["exact_p87_p35_release_tree_chain"],
        true
    );
    assert_eq!(
        seal["payload"]["scope"]["exact_p86_wsl_parent_owner_binding_chain"],
        true
    );
    assert_eq!(
        seal["payload"]["scope"]["ordered_failure_nonpublication"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        seal["payload"]["limits"]["publication_not_attempted_before_p87_completion"],
        true
    );
    assert_eq!(
    ...
    assert_eq!(
        schema["$id"],
        "urn:ferris:schema:pulse-88-witness-preserving-capability-materialization-executor-wsl-parent-owner-binding-successor:v1"
    );
    assert_eq!(
    ...
    assert_eq!(
        schema["properties"]["behavioral_control_tests_passed"]["const"],
        40
    );
    assert_eq!(
        schema["properties"]["p87_bound_commit"]["const"],
        P87_COMMIT
    );
    assert_eq!(
        schema["$defs"]["cycle"]["additionalProperties"],
        false
    );
    assert_eq!(
        schema["$defs"]["cycle"]["properties"]["publication_disposition"]["enum"],
        serde_json::json!(["published-result", "published-failure-witness"])
    );
