...
const CUTOFF: &str = "e3b0b62f6dd62b5071886d32a9eedca85c76b4ae";
const PULSE_59_HEAD: &str = "6945f5fc96868c97267a1635fbb5219cc398eeb4";
const DOMAIN: &str = "ferris.process-exit-diagnostic-pulse-65-authority/v1";
const DECLARATION_IDENTITY: &str =
    "sha256:5bd7c876180a3bfb9f0bcb1518ef68921d1b28210d1f717c904753508e28abb0";
const MUTATION_COUNT: usize = 25_815;
const PRIOR_REGISTRY_TOTAL: usize = 209_335;
const REGISTRY_TOTAL: usize = 235_150;
...
fn assert_api_bindings(declaration: &Value) {
    ...
    assert_eq!(p59["role"], "sole-p65-production-runtime-callable");
    assert_eq!(
...
#[test]
fn pulse_65_binds_cutoff_containing_pulse_64_withdrawal_and_zero_execution() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-65-authority.json"));
    let schema = read_json(
        held_out.join("schemas/ferris.process-exit-diagnostic-pulse-65-authority.v1.schema.json"),
    );
    assert_eq!(declaration["schema"], DOMAIN);
    assert_eq!(declaration["status"], "authorized-unexecuted");
    assert_eq!(
        declaration["program_id"],
        "FERRIS-P65-INDEPENDENT-PROCESS-EXIT-DIAGNOSTIC-WITNESS-PRESERVING-CAPABILITY-MATERIALIZATION-AUTHORITY"
    );
    ...
    for path in [
        ...
        "docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-65-authority-mutations.json",
        "docs/simulations/profile-diff-held-out/schemas/ferris.process-exit-diagnostic-pulse-65-authority.v1.schema.json",
        ...
        "crates/ferris-cli/tests/process_exit_diagnostic_pulse_65_authority.rs",
    ] {
        assert_cutoff_absent(path);
    }
    let p64_wave = String::from_utf8(cutoff_blob(
        "context/waves/2026-08-12-platform-profile-conformance/pulses/pulse-64.md",
    ))
    .expect("UTF-8 Pulse 64 wave");
    assert!(p64_wave.contains("invalid-prelaunch-unbound-wsl-qualification-contract"));
    let p64_closeout = String::from_utf8(cutoff_blob(
        "docs/simulations/profile-diff-held-out/PROCESS_EXIT_DIAGNOSTIC_PULSE_64_AUTHORITY.md",
    ))
    .expect("UTF-8 Pulse 64 closeout");
    assert!(p64_closeout.contains("P64-WSL-QUALIFIER-SYSTEMROOT-DERIVATION"));
    assert_zero_execution_state(&declaration["execution_state"]);
}

#[test]
fn pulse_65_binds_cutoff_blobs_supported_variants_and_exact_p59_surface() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration_text = String::from_utf8(read_lf(
        held_out.join("fixtures/process-exit-diagnostic-pulse-65-authority.json"),
    ))
...
#[test]
fn pulse_65_records_withdrawn_pulse_64_and_binds_exact_p57_wsl_bootstrap_contract() {
    let root = repo_root();
    ...
    assert_eq!(p64["closeout_commit"], CUTOFF);
    ...
    assert_eq!(wsl_preflight["single_wsl_process_spawn"], true);
    ...
    assert_eq!(cleanup["exact_probe_bundle_removed"], true);
    assert_eq!(cleanup["absence_verified_after_remove"], true);
    assert_eq!(cleanup["same_invocation_cleanup_required"], true);
    ...
    assert_eq!(
        ...
        "ubuntu-runtime-parent-exact-p57-wsl-bootstrap-systemroot-derivation-qualification"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        serde_json::json!([
            "P65-PRIVATE-RUNTIME-TOPOLOGY-PROBE",
            "P65-P41-ACTUAL-BASENAME-RENAME-PROBE",
            "P65-P59-TERMINAL-SIBLING-PROBE"
        ])
    );
    assert_eq!(
        platform_stops["native_linux"],
        serde_json::json!([
            "P65-EXACT-P57-WSL-STAGE-BUNDLE-PREFLIGHT",
            "P65-SYSTEMROOT-SOURCE-PRECEDENCE-DERIVATION",
            "P65-WINDOWS-WSL-CMD-NONREPARSE-IDENTITY",
            "P65-EXACT-P57-WORKER-BOOTSTRAP-PROOF",
            "P65-UBUNTU-NOEXEC-PREREQUISITE"
        ])
    );
    ...
    assert_eq!(
        ...
        "docs/simulations/profile-diff-held-out/pulse-65-public-result/"
    );
    assert_eq!(
...
#[test]
fn pulse_65_validator_is_checkout_only_and_rejects_mutations() {
    ...
    let p65 = fs::read_to_string(
        root.join("crates/ferris-cli/tests/process_exit_diagnostic_pulse_65_authority.rs"),
    )
    .expect("P65 validator source");
    let legacy_raw_map = ["raw", "sha256", "by", "path"].join("_");
    assert!(
    ...
    assert!(
        ...
        "P65 reads immutable cutoff blobs"
    );
    assert!(
        p65.contains("canonical_identity_by_path"),
        "P65 validates canonical identities"
    );
    assert!(
        !p65.contains(&legacy_raw_map),
        "P65 cannot compare working-tree-only raw identities"
    );
    ...
    assert!(
        ...
        "P65 test must stay declaration/mutation/checkout only"
    );
    ...
    let mutations = read_json(
        held_out.join("fixtures/process-exit-diagnostic-pulse-65-authority-mutations.json"),
    );
    ...
    for (index, control) in controls.iter().enumerate() {
        ...
        assert_eq!(id, format!("P65-M{:05}", index + 1));
