...
const CUTOFF: &str = "2388b7d9a5fda7f9cbf772e12d1b4c07d22f2161";
const PULSE_59_HEAD: &str = "6945f5fc96868c97267a1635fbb5219cc398eeb4";
const DOMAIN: &str = "ferris.process-exit-diagnostic-pulse-64-authority/v1";
const DECLARATION_IDENTITY: &str =
    "sha256:634e7b3197f5d550c6f3816dbf13770d44738c4f05de6956aa07966548a0be23";
const MUTATION_COUNT: usize = 24_700;
const PRIOR_REGISTRY_TOTAL: usize = 184_635;
const REGISTRY_TOTAL: usize = 209_335;
...
fn assert_api_bindings(declaration: &Value) {
    ...
    assert_eq!(p59["role"], "sole-p64-production-runtime-callable");
    assert_eq!(
...
#[test]
fn pulse_64_binds_cutoff_containing_pulse_63_withdrawal_and_zero_execution() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-64-authority.json"));
    let schema = read_json(
        held_out.join("schemas/ferris.process-exit-diagnostic-pulse-64-authority.v1.schema.json"),
    );
    assert_eq!(declaration["schema"], DOMAIN);
    assert_eq!(declaration["status"], "authorized-unexecuted");
    assert_eq!(
        declaration["program_id"],
        "FERRIS-P64-INDEPENDENT-PROCESS-EXIT-DIAGNOSTIC-WITNESS-PRESERVING-CAPABILITY-MATERIALIZATION-AUTHORITY"
    );
    ...
    for path in [
        ...
        "docs/simulations/profile-diff-held-out/fixtures/process-exit-diagnostic-pulse-64-authority-mutations.json",
        "docs/simulations/profile-diff-held-out/schemas/ferris.process-exit-diagnostic-pulse-64-authority.v1.schema.json",
        ...
        "crates/ferris-cli/tests/process_exit_diagnostic_pulse_64_authority.rs",
    ] {
        assert_cutoff_absent(path);
    }
    let p63_wave = String::from_utf8(cutoff_blob(
        "context/waves/2026-08-12-platform-profile-conformance/pulses/pulse-63.md",
    ))
    .expect("UTF-8 Pulse 63 wave");
    assert!(p63_wave.contains("invalid-prelaunch-wsl-bootstrap-contract"));
    let p63_closeout = String::from_utf8(cutoff_blob(
        "docs/simulations/profile-diff-held-out/PROCESS_EXIT_DIAGNOSTIC_PULSE_63_AUTHORITY.md",
    ))
    .expect("UTF-8 Pulse 63 closeout");
    assert!(p63_closeout.contains("P63-P57-EXACT-WSL-BOOTSTRAP-CONTRACT"));
    assert_zero_execution_state(&declaration["execution_state"]);
}

#[test]
fn pulse_64_binds_cutoff_blobs_supported_variants_and_exact_p59_surface() {
    let held_out = repo_root().join(HELD_OUT);
    let declaration =
        read_json(held_out.join("fixtures/process-exit-diagnostic-pulse-64-authority.json"));
...
#[test]
fn pulse_64_records_withdrawn_pulse_63_and_binds_exact_p57_wsl_bootstrap_contract() {
    let root = repo_root();
    ...
    assert_eq!(p63["closeout_commit"], CUTOFF);
    ...
    assert_eq!(
        authority["pre_call_public_qualification_callable_may_prove_exact_p57_route_invariants"],
        true
    );
    ...
    assert_eq!(
        wsl_preflight["resolved_windows_wsl_executable"],
    ...
    assert_eq!(
        wsl_preflight["environment_exact_values"],
        serde_json::json!({
    ...
    let public_qualification = &wsl_preflight["public_qualification_callable"];
    assert_eq!(public_qualification["allowed"], true);
    assert_eq!(
        public_qualification["callable_name"],
        "qualify_exact_p57_wsl_bootstrap_contract"
    );
    assert_eq!(public_qualification["single_invocation_exact"], 1);
    assert_eq!(public_qualification["retry_permitted"], false);
    assert_eq!(public_qualification["same_stage_bundle_route_required"], true);
    assert_eq!(
        public_qualification["must_prove_worker_bootstrap_and_source_loader_invariants_before_return"],
        true
    );
    assert_eq!(
        public_qualification["must_remove_exact_probe_bundle_and_verify_absence"],
        true
    );
    ...
    assert_eq!(
        ...
        "ubuntu-runtime-parent-exact-p57-wsl-bootstrap-qualification"
    );
    assert_eq!(
    ...
    assert_eq!(
        declaration["runtime_binding"]["pre_call_public_qualification_callable_may_prove_exact_p57_route_invariants"],
        true
    );
    ...
    assert_eq!(
        ...
        serde_json::json!([
            "P64-PRIVATE-RUNTIME-TOPOLOGY-PROBE",
            "P64-P41-ACTUAL-BASENAME-RENAME-PROBE",
            "P64-P59-TERMINAL-SIBLING-PROBE"
        ])
    );
    assert_eq!(
        platform_stops["native_linux"],
        serde_json::json!([
            "P64-EXACT-P57-WSL-STAGE-BUNDLE-PREFLIGHT",
            "P64-EXACT-P57-WORKER-BOOTSTRAP-PROOF",
            "P64-UBUNTU-NOEXEC-PREREQUISITE"
        ])
    );
    ...
    assert_eq!(
        ...
        "docs/simulations/profile-diff-held-out/pulse-64-public-result/"
    );
    assert_eq!(
...
#[test]
fn pulse_64_validator_is_checkout_only_and_rejects_mutations() {
    ...
    let p63 = fs::read_to_string(
        root.join("crates/ferris-cli/tests/process_exit_diagnostic_pulse_64_authority.rs"),
    )
    .expect("P63 validator source");
    let legacy_raw_map = ["raw", "sha256", "by", "path"].join("_");
    assert!(
    ...
    assert!(
        p63.contains("cutoff_blob"),
        "P63 reads immutable cutoff blobs"
    );
    assert!(
        p63.contains("canonical_identity_by_path"),
        "P63 validates canonical identities"
    );
    assert!(
        !p63.contains(&legacy_raw_map),
        "P63 cannot compare working-tree-only raw identities"
    );
    ...
    assert!(
        !p63.contains(&python_helper)
            && !p63.contains(&pulse_59_python_tests)
            && !p63.contains(&pulse_58_python_tests),
        "P63 test must stay declaration/mutation/checkout only"
    );
    ...
    let mutations = read_json(
        held_out.join("fixtures/process-exit-diagnostic-pulse-64-authority-mutations.json"),
    );
    ...
    for (index, control) in controls.iter().enumerate() {
        ...
        assert_eq!(id, format!("P64-M{:05}", index + 1));
