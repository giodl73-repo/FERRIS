...
Status: Implemented V1 with optional deterministic request binder
Implementation authority: Exhausted after the bounded binder implementation
...
Application and workspace IDs may preserve the Application Definition's
hierarchical lowercase slash-separated identity shape. Readiness binding does
not flatten, translate, or replace owner IDs.
...
`bind-application-readiness` is an optional preparation boundary for the
existing request record. It requires:

- one explicit `--application <APPLICATION_JSON>` naming a strict
  `ferris.application/v0` direct child of the application root;
- one repeated `--requirements <WORKSPACE_ID=REQUIREMENTS_JSON>` association
  for every application workspace, with no missing, duplicate, or extra IDs;
  and
- one explicit `--output <REQUEST_JSON>` whose existing parent is the
  application root and whose portable filename does not already exist.

The binder MUST fully validate the existing Application Definition,
requirements declarations, manifest paths, workspace coverage, and distinct
non-nested workspace roots before creating output. It MUST preserve manifest
paths, sort request entries by workspace ID, compute SHA-256 over exact bounded
owner-file bytes, and emit pretty UTF-8 JSON with one trailing newline. The
frozen fixture's generated bytes MUST equal `request-valid.json`.

Output creation MUST be atomic and no-clobber. The implementation writes and
synchronizes a unique sibling temporary file, atomically links it to the absent
destination, and removes the temporary name. Any ordinary failure MUST leave no
destination or temporary file; an existing or concurrently created destination
MUST remain unchanged. A process or host crash can leave the uniquely named
temporary sibling, but cannot expose a partial destination.

The binder MUST revalidate exact owner-file bytes and filesystem identities
immediately before output creation. A changed, replaced, linked, or unreadable
input returns `FERRIS-APPLICATION-READINESS-BINDING-INPUT-STALE`, result class
`stale`, without creating the destination.

Binder-specific output diagnostics are stable:

| Code | Condition |
|---|---|
| `FERRIS-APPLICATION-READINESS-OUTPUT-EXISTS` | The destination already exists or is created concurrently |
| `FERRIS-APPLICATION-READINESS-OUTPUT-UNAVAILABLE` | A temporary output cannot be created, synchronized, or atomically committed |
| `FERRIS-APPLICATION-READINESS-BINDING-INPUT-STALE` | An owner input changes or is replaced before commit |

Binding MUST NOT invoke Cargo, discover workspaces, infer or create
requirements, observe the environment, mutate owner files, create or approve
an Action Plan, or execute work. Successful JSON output is a normal command
envelope whose record is the request. Invalid and unsupported input returns a
typed envelope with a null record and no private absolute paths.
...
declaration MUST NOT be repurposed. V1 implements the optional `doctor`
consumer and deterministic request binder. It does not implement a cache,
parallel scheduler, source adapter, installer, repair action, or support policy.
...
- byte-identical deterministic request binding and no-clobber output;
