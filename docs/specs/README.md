...
| Path authority contract | Implemented V0; GO-WP-001 | Defines repository-root-qualified Cargo planning and portable path identity |
| Validation topology contract | Implemented V1; GO-WP-002 | Defines strict owner declarations, separate provider observations, and read-only reconciliation |
...
| [Application readiness composition contract](FERRIS_APPLICATION_READINESS_COMPOSITION_CONTRACT.md) | Implemented V1; APP-READINESS-001 | Composes explicit workspace READINESS-001 reports across one application without Cargo discovery or hidden ready states |

## Implemented bounded extensions

These reviewed plans record implemented extensions to existing public records.
They are evidence and design references, not additional normative
specifications.
...
| APP-READINESS-001 Pulse 07 | [Application readiness implementation review](../plans/reviews/FERRIS_APPLICATION_READINESS_IMPLEMENTATION_REVIEW.md) | Optional explicit-file V1 implemented; application-root requirements, adapters, execution, adoption, and support remain withheld |
