...
- Result: 40 successful plans, 40 matching repeated plan/change-set identity
  ...
  execution lanes, and three unavailable BISECT lanes.
...
| AI Assurance Skeptic | `pass-with-limitation` | All 40 repeated identities match and all unavailable outcomes remain non-success. The executable false-negative denominator is only one and cannot support promotion. |
...
| Value obtained | Determinism passed; current fallback and command-shape costs were quantified; affected-only promotion was rejected before owner CI changed. |
...
- BISECT `d9550b09`: selected and full unavailable with different denied-warning
  diagnostics;
- BISECT `453ca260`: collapsed selected-equals-full lane unavailable with a
  denied-warning diagnostic; and
