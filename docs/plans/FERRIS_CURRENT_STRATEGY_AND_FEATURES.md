...
| Passive Cargo failure diagnosis | `diagnose-cargo` | Implemented bounded classification of complete caller-supplied stderr into dependency, lockfile, offline-policy, or unclassified reports | Executes no work, retains no raw output, extracts no owner identifiers, and does not interpret rustc or test failures |
...
define a high-value product boundary: compare explicit owner requirements with
passive local observations before validation begins. Pulse 03 implements that
bounded explicit-file path without a support claim. The
[asdf adapter evaluation](../research/2026-09-19-asdf-tool-versions-readiness-adapter.md)
then found that `.tool-versions` cannot map losslessly to V1 because plugin IDs
do not identify executable leaves and its version-selection policy is outside
...
3. Validate a multi-release compatibility and support window for the smallest
   ...
   exact accepted/emitted schema set and can replay a checked-in adopter
   requirement set. The synthetic corpus supplies the first bounded set, but
   only one Ferris release has been observed, so no support duration or
   migration claim exists yet.
