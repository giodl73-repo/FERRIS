#!/usr/bin/env python3
"""Run Pulse 81's sealed fake-only qualification cycles."""
...
WORK_ROOT = REPO_ROOT / "target" / "pulse-81-qualification-runtime"
EXACT_REPO_ROOT = REPO_ROOT / "target" / "pulse-81-exact-p35-release-tree-root"
...
from sealed_dependencies import canonical_bytes, load_exact_p78_stack, sha256_bytes  # noqa: E402
...
CONTROL_METHODS = {
    ...
    "exact-p78-binding-and-local-loader": "test_exact_p78_binding_and_local_loader",
    ...
    "concurrent-100-complete-load-graph-serialized": "test_concurrent_100_exact_p78_loads_are_serialized",
...
def _normalize_exact_p35_release(root: Path) -> None:
    ...
    if manifest.get("file_count") != 8:
        raise RuntimeError("P81 exact Pulse 35 manifest file count mismatch")
    for entry in manifest["files"]:
        ...
        if (
            ...
            raise RuntimeError(
                f"P81 exact Pulse 35 normalization failed for {path.name}"
            )
        path.write_bytes(normalized)
    if sum(1 for path in root.rglob("*") if path.is_file()) != 10:
        raise RuntimeError("P81 exact Pulse 35 release tree file count mismatch")
...
def _run_cycle(root: Path, cycle: int) -> dict[str, object]:
    ...
    _p78, p51, _p56 = load_exact_p78_stack(repo_root)
    fake = FakeP56(
        artifact,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if WORK_ROOT.exists():
            cleanup_synthetic_runtime_root(WORK_ROOT)
    payload = {
        "schema": "ferris.pulse-81-ordered-capability-materialization-executor-pulse35-release-tree-successor-qualification/v1",
        "outcome": "pass",
        "cycles_required": 20,
    ...
    if args.write_receipt:
        ...
        envelope = {
            ...
            "receipt_id": digest,
            "schema": "ferris.pulse-81-ordered-capability-materialization-executor-pulse35-release-tree-successor-qualification-envelope/v1",
        }
