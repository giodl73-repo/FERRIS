"""Byte-bound exact imports for Pulse 81's ordered capability successor."""
...
P78_COMMIT = "7c60f6b384bec1494fbbabaa244b8f0f8eece355"
P78_RELEASE_ROOT = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p81"
_WINDOWS_WAIT_OBJECT_0 = 0x00000000
_WINDOWS_WAIT_ABANDONED = 0x00000080
_P81_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p81_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A Pulse 81 predecessor was not the exact sealed release."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P81-SEALED-LOCK-STATE")
    global _P81_INTERNAL_LOCK_MANAGER
    _P81_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P81_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P81-SEALED-LOCK-STATE")
    return _P81_INTERNAL_LOCK_MANAGER
...
P78 = ReleaseIdentity(
    ...
    14,
    16,
    "sha256:cc0d9fd1817318c07942f652a4438bcfee5c89b9144c8ca9f48da7d0b1f96a52",
    "sha256:e235dc8da9884bf67ebb2ed36791cf8894d42ed0a525dbeee1f854ece6e495e4",
    "sha256:0ea8020b9fc2cbd04a4a72861a897c78304c2505e125425d81173bddd74de0ae",
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P81-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(path: Path, code: str = "P81-SEALED-TREE", maximum: int = 4_194_304) -> bytes:
    try:
...
def _release_root(repo_root: Path, directory: str) -> Path:
    try:
        ...
        if not root.is_absolute():
            raise SealedDependencyFailure("P81-SEALED-ROOT")
        ...
        raise SealedDependencyFailure("P81-SEALED-ROOT") from error
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P81-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P81-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P81-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P81-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P81-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P81-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P81-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P81-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P81-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P81-SEALED-LOCK-STATE")
                else:
...
def _verified_release(
    ...
    *,
    ...
    code = code or f"P81-{identity.directory.upper().replace('-', '-')}-IDENTITY"
...
def load_exact_p39_and_p41(repo_root: Path) -> tuple[ModuleType, ModuleType]:
    with _sealed_loading_lock():
        ...
        p39_module = _exec_bound_module(
            "p81_exact_p39", p39.root / P39.source, p39.files[P39.source], "P81-P39-IMPORT"
        )
        p41_module = _exec_bound_module(
            "p81_exact_p41", p41.root / P41.source, p41.files[P41.source], "P81-P41-IMPORT"
        )
        for name, parameters in (
            ...
            _signature(p39_module, name, parameters, "P81-P39-API")
        if (
            ...
            raise SealedDependencyFailure("P81-P39-API")
        _signature(
            p41_module,
            "copy_release",
        ...
        _signature(p41_module, "verify_bound_tree", ("root", "phase"), "P81-P41-API")
        _signature(p41_module, "_safe_absolute", ("value",), "P81-P41-API")
        if (
            ...
            raise SealedDependencyFailure("P81-P41-API")
...
def load_exact_p52_stage_reader(repo_root: Path) -> ModuleType:
    with _sealed_loading_lock():
        ...
        module = _load_with_bound_dependencies("p81_exact_p52", bound, P52.source, "P81-P52-IMPORT")
        _signature(module, "_validate_materialization_summary", ("p51", "summary"), "P81-P52-API")
        _signature(module, "_validate_verification_summary", ("p51", "summary"), "P81-P52-API")
        _signature(module, "_validate_materialized_descriptor_root", ("p51", "descriptor_root", "runtime_root"), "P81-P52-API")
        _signature(module, "_verify_public_prelaunch_custody", ("p51", "p39", "p41", "runtime_root", "p39_checkout_root", "p41_final_root", "private_record"), "P81-P52-API")
        _signature(module, "_remove_private_tree", ("p51", "path", "code"), "P81-P52-API")
        _signature(module, "_remove_seed", ("p51", "seed_path", "private_record"), "P81-P52-API")
        return module


def load_exact_p78(repo_root: Path) -> ModuleType:
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P78)
        module = _load_with_bound_dependencies("p81_exact_p78", bound, P78.source, "P81-P78-IMPORT")
        _signature(
            module,
        ...
        for name, parameters in (
            ...
            _signature(module, name, parameters, "P81-P78-API")
        if (
    ...
    with _sealed_loading_lock():
        ...
        for module in (materializer, verifier):
            if (
                ...
                raise SealedDependencyFailure("P81-P35-API")
        _signature(materializer, "materialize", ("seed_path", "output", "case_count"), "P81-P35-API")
        _signature(materializer, "seed_commitment", ("seed",), "P81-P35-API")
        _signature(verifier, "verify", ("output", "seed_path"), "P81-P35-API")
        return materializer, verifier


def load_exact_p78_stack(repo_root: Path) -> tuple[ModuleType, ModuleType, ModuleType]:
    """Return exact Pulse 78 plus its own byte-bound P51/P56 modules."""
    with _sealed_loading_lock():
        p78 = load_exact_p78(repo_root)
        try:
            p51 = p78.load_exact_p51(repo_root)
            p56 = p78.load_exact_p56(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P81-P78-STACK") from error
        return p78, p51, p56
...
def release_identities() -> dict[str, dict[str, str]]:
    return {
        ...
        "pulse_78": {
            "commit": P78_COMMIT,
            "manifest": P78.manifest_raw_sha256,
            "release_root": P78_RELEASE_ROOT,
            "seal": P78.seal_payload_sha256,
        },
    }
