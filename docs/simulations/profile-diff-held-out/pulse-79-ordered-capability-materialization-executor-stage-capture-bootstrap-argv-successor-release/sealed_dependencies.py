"""Byte-bound exact imports for Pulse 79's ordered capability successor."""
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p79"
_WINDOWS_WAIT_OBJECT_0 = 0x00000000
_WINDOWS_WAIT_ABANDONED = 0x00000080
_P79_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p79_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A Pulse 79 predecessor was not the exact sealed release."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P79-SEALED-LOCK-STATE")
    global _P79_INTERNAL_LOCK_MANAGER
    _P79_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P79_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P79-SEALED-LOCK-STATE")
    return _P79_INTERNAL_LOCK_MANAGER
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P79-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(path: Path, code: str = "P79-SEALED-TREE", maximum: int = 4_194_304) -> bytes:
    try:
...
def _release_root(repo_root: Path, directory: str) -> Path:
    try:
        ...
        if not root.is_absolute():
            raise SealedDependencyFailure("P79-SEALED-ROOT")
        ...
        raise SealedDependencyFailure("P79-SEALED-ROOT") from error
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P79-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P79-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P79-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P79-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P79-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P79-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P79-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P79-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P79-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P79-SEALED-LOCK-STATE")
                else:
...
def _verified_release(repo_root: Path, identity: ReleaseIdentity) -> _VerifiedRelease:
    code = f"P79-{identity.directory.upper().replace('-', '-')}-IDENTITY"
...
def load_exact_p39_and_p41(repo_root: Path) -> tuple[ModuleType, ModuleType]:
    with _sealed_loading_lock():
        ...
        p39_module = _exec_bound_module(
            "p79_exact_p39", p39.root / P39.source, p39.files[P39.source], "P79-P39-IMPORT"
        )
        p41_module = _exec_bound_module(
            "p79_exact_p41", p41.root / P41.source, p41.files[P41.source], "P79-P41-IMPORT"
        )
        for name, parameters in (
            ...
            _signature(p39_module, name, parameters, "P79-P39-API")
        if (
            ...
            raise SealedDependencyFailure("P79-P39-API")
        _signature(
            p41_module,
            "copy_release",
        ...
        _signature(p41_module, "verify_bound_tree", ("root", "phase"), "P79-P41-API")
        _signature(p41_module, "_safe_absolute", ("value",), "P79-P41-API")
        if (
            ...
            raise SealedDependencyFailure("P79-P41-API")
...
def load_exact_p52_stage_reader(repo_root: Path) -> ModuleType:
    with _sealed_loading_lock():
        ...
        module = _load_with_bound_dependencies("p79_exact_p52", bound, P52.source, "P79-P52-IMPORT")
        _signature(module, "_validate_materialization_summary", ("p51", "summary"), "P79-P52-API")
        _signature(module, "_validate_verification_summary", ("p51", "summary"), "P79-P52-API")
        _signature(module, "_validate_materialized_descriptor_root", ("p51", "descriptor_root", "runtime_root"), "P79-P52-API")
        _signature(module, "_verify_public_prelaunch_custody", ("p51", "p39", "p41", "runtime_root", "p39_checkout_root", "p41_final_root", "private_record"), "P79-P52-API")
        _signature(module, "_remove_private_tree", ("p51", "path", "code"), "P79-P52-API")
        _signature(module, "_remove_seed", ("p51", "seed_path", "private_record"), "P79-P52-API")
        return module
...
def load_exact_p78(repo_root: Path) -> ModuleType:
    with _sealed_loading_lock():
        ...
        module = _load_with_bound_dependencies("p79_exact_p78", bound, P78.source, "P79-P78-IMPORT")
        _signature(
            module,
        ...
        for name, parameters in (
            ...
            _signature(module, name, parameters, "P79-P78-API")
        if (
            ...
            raise SealedDependencyFailure("P79-P78-API")
        return module
...
def load_exact_p35_materializer_and_verifier(repo_root: Path) -> tuple[ModuleType, ModuleType]:
    with _sealed_loading_lock():
        ...
        manifest_raw = _safe_regular(release / "public-manifest.json", "P79-P35-IDENTITY")
        materializer_raw = _safe_regular(release / "corpus_materializer.py", "P79-P35-IDENTITY", 131_072)
        verifier_raw = _safe_regular(release / "verify_materialization.py", "P79-P35-IDENTITY", 131_072)
        manifest = _sealed_json(manifest_raw, "P79-P35-IDENTITY")
        if (
            ...
            raise SealedDependencyFailure("P79-P35-IDENTITY")
        materializer = _exec_bound_module(
            "p79_exact_p35_materializer",
            release / "corpus_materializer.py",
            materializer_raw,
            "P79-P35-IMPORT",
        )
        verifier = _exec_bound_module(
            "p79_exact_p35_verifier",
        ...
        for module in (materializer, verifier):
            if (
                ...
                raise SealedDependencyFailure("P79-P35-API")
        _signature(materializer, "materialize", ("seed_path", "output", "case_count"), "P79-P35-API")
        _signature(materializer, "seed_commitment", ("seed",), "P79-P35-API")
        _signature(verifier, "verify", ("output", "seed_path"), "P79-P35-API")
...
def load_exact_p78_stack(repo_root: Path) -> tuple[ModuleType, ModuleType, ModuleType]:
    ...
    with _sealed_loading_lock():
        ...
        try:
            ...
            raise SealedDependencyFailure("P79-P78-STACK") from error
        return p78, p51, p56
