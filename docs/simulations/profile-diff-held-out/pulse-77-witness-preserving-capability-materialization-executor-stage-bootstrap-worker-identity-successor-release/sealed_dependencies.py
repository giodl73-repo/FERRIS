"""Byte-bound exact imports for Pulse 77's witness-preserving Pulse 76 wrapper."""
...
P76_COMMIT = "95edeb9560748989b4c94c0becc3a466e1fbc216"
P76_RELEASE_ROOT = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p77"
...
_P77_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p77_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 76 predecessor or callable binding failed."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P77-SEALED-LOCK-STATE")
    global _P77_INTERNAL_LOCK_MANAGER
    _P77_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P77_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P77-SEALED-LOCK-STATE")
    return _P77_INTERNAL_LOCK_MANAGER


P76 = ReleaseIdentity(
    "pulse-76-ordered-capability-materialization-executor-stage-bootstrap-worker-identity-successor-release",
    "ordered_capability_materialization_executor.py",
    "sha256:422b52c0be55bcc1e4f4df5f4920ed9b151218182e2d4496a92d186f08ca0aba",
    ...
    13,
    15,
    "sha256:d37c30d064f183ee5eaa6d9611aa41c076c5f0e7d608c86642fc5e3a38c491ac",
    "sha256:94e69bb56ec37701bff179afe2079f9788158d495d130c0d6f5c25dd51df8bc5",
    "sha256:cb782a06fe7e9bb24567fe58baee0d8d32ef37a92bdcb6717bf197da6227c07d",
...
P76_PREDECESSORS = {
    ...
    "pulse_75": {
        "commit": "47229cdc7c090e1c5eb2762ac82b411ec50f1e7d",
        "manifest": "sha256:b72b855f00f5482aba79c117aa00af9e47c9ebb440ae8a7d66ea02a5318b4888",
        "release_root": "docs/simulations/profile-diff-held-out/pulse-75-capability-bound-diagnostic-executor-stage-bootstrap-worker-identity-successor-release",
        "seal": "sha256:1401a52de74116fa429913ccb084db3381585cfb4c82e17a87221004b7afa6fa",
    },
}
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P77-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(
    path: Path, code: str = "P77-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P77-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P77-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P77-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P77-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P77-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P77-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P77-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P77-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P77-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P77-SEALED-LOCK-STATE")
                else:
...
def _patch_loaded_dependency_module(module: object, code: str) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise SealedDependencyFailure(code)
    if getattr(module, "__p77_bound_dependency_patch__", False):
        return module
    ...
    module.__p77_bound_dependency_patch__ = True
    return module
...
def load_pulse76(
    ...
    """Return fresh exact Pulse 76 modules after exact-tree verification."""
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P76, "P77-P76-IDENTITY")
        p76 = _load_with_bound_dependencies(
            "p77_exact_p76", bound, P76.source, "P77-P76-IMPORT"
        )
        _signature(
            p76,
        ...
        if (
            tuple(getattr(p76, "P58_GATE_IDS", ())) != P58_GATE_IDS
            or getattr(p76, "P39_CALLER_AUTHORITY_PRECONDITION", None)
            != "future-authority-supplied-fresh-anonymous-exact-cutoff-root"
            or not isinstance(
                getattr(p76, "OrderedCapabilityMaterializationResult", None), type
            )
            or not isinstance(getattr(p76, "P58Failure", None), type)
            or not isinstance(getattr(p76, "IndeterminateCleanup", None), type)
        ):
            raise SealedDependencyFailure("P77-P76-API")

        try:
            p52 = p76.load_exact_p52_stage_reader(repo_root)
            p75, p51, _p56 = p76.load_exact_p75_stack(repo_root)
            p43, _p45, p47 = p51.load_terminal_dependencies(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P77-P76-STACK") from error
        ...
        for module, name, parameters in (
            ...
            _signature(module, name, parameters, "P77-P76-STACK")
        if not isinstance(getattr(p51, "TerminalPulse47Once", None), type):
            raise SealedDependencyFailure("P77-P76-STACK")

        return p76, p52, p75, p51, p43, p47
...
def release_identities() -> dict[str, object]:
    return {
        ...
        "pulse_76": {
            "commit": P76_COMMIT,
            "manifest_aggregate": P76.manifest_aggregate,
            "manifest_raw_sha256": P76.manifest_raw_sha256,
            "receipt_payload_sha256": P76.receipt_payload_sha256,
            "receipt_raw_sha256": P76.receipt_raw_sha256,
            "release_root": P76_RELEASE_ROOT,
            "seal_payload_sha256": P76.seal_payload_sha256,
            "seal_raw_sha256": P76.seal_raw_sha256,
            "source_sha256": P76.source_sha256,
            "stack": json.loads(canonical_bytes(P76_PREDECESSORS)),
        },
    }
