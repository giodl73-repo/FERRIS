#!/usr/bin/env python3
"""Run deterministic fake-only Pulse 86 qualification cycles."""
...
WORK_ROOT = REPO_ROOT / "target" / "pulse-86-qualification-runtime"
...
CONTROL_METHODS = {
    ...
    "fork-child-inherited-lock-reset": "test_fork_child_reset_does_not_acquire_inherited_locks",
    "parent-owner-explicit-root-resolution": "test_parent_owner_resolution_uses_explicit_root_and_returns_owner",
    "parent-owner-stderr-rejected": "test_parent_owner_resolution_rejects_any_stderr",
    "parent-owner-malformed-output-mapped": "test_parent_owner_resolution_maps_malformed_output_to_owner_failure",
    "explicit-owner-command-binding": "test_wsl_command_rejects_invalid_owner_and_binds_valid_owner",
    "parent-owner-uid-race-rejected": "test_stage_rejects_parent_owner_uid_change_before_creation",
    "effective-owner-uid-mismatch-rejected": "test_stage_rejects_effective_uid_mismatch_before_creation",
    ...
    "host-stage-abnormal-completion-indeterminate": "test_host_stage_abnormal_completion_is_indeterminate",
    "public-terminal-preserves-stage-indeterminate": "test_public_terminal_preserves_indeterminate_stage_failure",
...
def _p27_success(path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError("P86 fake P27 root must be absent")
    path.mkdir()
    return {
...
def _run_negative_controls() -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("P86 qualification negative-control failure")
...
def _run_cycle(root: Path, cycle: int, p51: object) -> dict[str, object]:
    ...
    if (
        ...
        raise RuntimeError("P86 qualification topology or owner-binding failure")
    return {
        "cycle": cycle,
        "cycle_mode": mode,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if WORK_ROOT.exists():
            shutil.rmtree(WORK_ROOT)
    payload = {
        "schema": "ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor-qualification/v1",
        "outcome": "pass",
        "cycles_required": 20,
        ...
        "exact_p78_binding_verified": True,
        "parent_owner_resolution_verified": True,
        "explicit_owner_command_binding_verified": True,
        "unknown_owner_probe_stderr_rejected": True,
        "malformed_owner_probe_mapped": True,
        "parent_owner_uid_race_rejected": True,
        "effective_owner_uid_mismatch_rejected": True,
        "local_loader_explicit_binding_verified": True,
        "fresh_module_loading_verified": True,
        "fork_child_inherited_lock_reset_verified": True,
        "stage_identity_capture_verified": True,
        "stage_post_create_failure_cleanup_verified": True,
        "host_stage_abnormal_completion_indeterminate_verified": True,
    ...
    if args.write_receipt:
        receipt = {
            ...
            "schema": "ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor-receipt/v1",
        }
