"""Byte-bound exact imports for Pulse 86's stage-capture/bootstrap-argv stack."""
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p86"
_WINDOWS_WAIT_OBJECT_0 = 0x00000000
_WINDOWS_WAIT_ABANDONED = 0x00000080
_P86_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p86_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 75 predecessor or callable binding failed."""
...
P78 = ReleaseIdentity(
    "pulse-78-capability-bound-diagnostic-executor-stage-capture-bootstrap-argv-successor-release",
    "capability_bound_diagnostic_executor_successor.py",
    "sha256:1478e8698c4ef9ece625e0e4995f00a40bdca429d41119e2cc180e2ec9619f57",
...
P75_STACK = {
    "pulse_72": {
        "commit": "642285b1b7f45102718f31d1d3ce92336f78bf5c",
        "manifest_aggregate": "sha256:315f39c3eb03e9fe1d2171914d1f2d09d94c509e8cbb2b09e5911cffc2e83dfd",
        "manifest_raw_sha256": "sha256:ba65fa3e8b9363e3345b736d12152f9b9dd46bd5ac1ef8b718407868d9a749b9",
        "receipt_payload_sha256": "sha256:8c31c7d0e19d9298e6b072a137f6d10522b752d91ed8c8b2c5df8259c5a1fb26",
        "receipt_raw_sha256": "sha256:0e316975e76d47d2abc807c6c00e361fe2f22c9e57bdf0bb686364e48840205e",
        "release_root": "docs/simulations/profile-diff-held-out/pulse-72-capability-bound-diagnostic-executor-stage-identity-successor-release",
        "seal_payload_sha256": "sha256:f80fdcc6b3d7b23cd2e81004c7e5243c75512234adc049805cb025c4f93ec7a3",
        "seal_raw_sha256": "sha256:241fa954c14d002474120ce539894f881eeae9a4e2c38226f7a732807b76b8a7",
        "source_sha256": "sha256:408cc0013861c398e76e125d7e62ec6f24cfd0aec8e9083f2d699e3ad04b2901",
        "stack": {
            "pulse_69": {
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P86-SEALED-LOCK-STATE")
    global _P86_INTERNAL_LOCK_MANAGER
    _P86_INTERNAL_LOCK_MANAGER = manager



def _require_internal_lock_manager() -> object:
    if _P86_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P86-SEALED-LOCK-STATE")
    return _P86_INTERNAL_LOCK_MANAGER
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P86-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result
...
def _safe_regular(
    path: Path, code: str = "P86-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P86-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P86-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P86-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P86-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P86-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P86-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P86-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P86-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P86-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P86-SEALED-LOCK-STATE")
                else:
...
def _load_exact_p75(bound: _VerifiedRelease, code: str) -> ModuleType:
    return _exec_bound_module(
        "p86_exact_p75",
        bound.root / P75.source,
        bound.files[P75.source],
        code,
    )


def load_exact_p75_stack(repo_root: Path) -> tuple[ModuleType, ModuleType, ModuleType, ModuleType]:
    with _sealed_loading_lock():
        _verified_release(repo_root, P78, "P86-P78-IDENTITY")
        bound = _verified_release(repo_root, P75, "P86-P75-IDENTITY")
        p75 = _load_exact_p75(bound, "P86-P75-IMPORT")
        _signature(
            p75,
        ...
        for name in (
            ...
            if not callable(getattr(p75, name, None)):
                raise SealedDependencyFailure("P86-P75-API")
        if (
            not isinstance(getattr(p75, "_NativeWslSession", None), type)
            or not isinstance(getattr(p75, "ExecutorFailure", None), type)
            or getattr(p75, "REQUEST_COUNT", None) != 69
            or getattr(p75, "WSL_PLATFORM", None) != "ubuntu-24.04-x86_64"
            or getattr(p75, "WSL_SCHEMA", None)
            != "ferris.pulse-57-wsl-capability-session/v1"
        ):
            raise SealedDependencyFailure("P86-P75-API")
        try:
            p57 = getattr(p75, "_P57", None)
            p51 = p75.load_exact_p51(repo_root)
            p56 = p75.load_exact_p56(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P86-P75-STACK") from error
        dependencies = getattr(p75, "_SEALED", None)
        if (
            ...
            raise SealedDependencyFailure("P86-P75-API")
        p75._p86_bound_sealed_dependencies = dependencies
        return p75, p57, p51, p56


def load_p51_synthetic_fixture(repo_root: Path, p51: ModuleType) -> ModuleType:
    with _sealed_loading_lock():
        p75, _p57, _p51, _p56 = load_exact_p75_stack(repo_root)
        dependencies = getattr(p75, "_SEALED", None)
...
def release_identities() -> dict[str, object]:
    return {
        "pulse_78": {
            "commit": P78_COMMIT,
            "manifest_aggregate": P78.manifest_aggregate,
            "manifest_raw_sha256": P78.manifest_raw_sha256,
            "receipt_payload_sha256": P78.receipt_payload_sha256,
            "receipt_raw_sha256": P78.receipt_raw_sha256,
            "release_root": P78_RELEASE_ROOT,
            "seal_payload_sha256": P78.seal_payload_sha256,
            "seal_raw_sha256": P78.seal_raw_sha256,
            "source_sha256": P78.source_sha256,
        },
        "pulse_75": {
            "commit": P75_COMMIT,
            "manifest_aggregate": P75.manifest_aggregate,
            "manifest_raw_sha256": P75.manifest_raw_sha256,
            "receipt_payload_sha256": P75.receipt_payload_sha256,
            "receipt_raw_sha256": P75.receipt_raw_sha256,
            "release_root": P75_RELEASE_ROOT,
            "seal_payload_sha256": P75.seal_payload_sha256,
            "seal_raw_sha256": P75.seal_raw_sha256,
            "source_sha256": P75.source_sha256,
            "stack": json.loads(canonical_bytes(P75_STACK)),
        }
    }
