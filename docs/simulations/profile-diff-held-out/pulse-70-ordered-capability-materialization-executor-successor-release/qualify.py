#!/usr/bin/env python3
"""Run Pulse 70's sealed fake-only qualification cycles."""
...
P51_ROOT = REPO_ROOT / "docs" / "simulations" / "profile-diff-held-out" / "pulse-51-diagnostic-executor-release"
WORK_ROOT = REPO_ROOT / "target" / "pulse-70-qualification-runtime"
...
from sealed_dependencies import canonical_bytes, load_exact_p69_stack, sha256_bytes  # noqa: E402
...
def _fake_release(root: Path, mode: str) -> Path:
    ...
    artifact.write_bytes((P51_ROOT / "fixtures" / "fake_ferris.py").read_bytes() + f"\n# p58-{mode}\n".encode("ascii"))
    return artifact
...
def _run_cycle(root: Path, cycle: int) -> dict[str, object]:
    ...
    _p69, p51, _p56 = load_exact_p69_stack(REPO_ROOT)
    fake = FakeP56(
        artifact,
    ...
    def open_wsl(_repo: Path, _parent: str, p51: object) -> _InProcessWsl:
        session = _InProcessWsl(fake, p51)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if WORK_ROOT.exists():
            cleanup_synthetic_runtime_root(WORK_ROOT)
    payload = {
        "schema": "ferris.pulse-70-ordered-capability-materialization-executor-successor-qualification/v1",
        "outcome": "pass",
        "cycles_required": 20,
    ...
    if args.write_receipt:
        ...
        envelope = {"payload": payload, "payload_sha256": digest, "receipt_id": digest, "schema": "ferris.pulse-70-ordered-capability-materialization-executor-successor-qualification-envelope/v1"}
