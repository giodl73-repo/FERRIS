"""Byte-bound exact imports for Pulse 70's ordered capability successor."""
...
class SealedDependencyFailure(RuntimeError):
    """A Pulse 70 predecessor was not the exact sealed release."""
...
P69 = ReleaseIdentity(
    ...
    12,
    14,
    "sha256:5fe60dcabc27a3260439c302407079feb3f004e2463877c8ff133a055884cbd4",
    "sha256:244f02e9fb5a164a747d3407696f4ef4a7fa318fa7ee5fe33d8ce1baa533def9",
    "sha256:c25b7b9868825734c7d649c1a2f4d303cf0de8da2ae7acebde386031fd689363",
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P70-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(path: Path, code: str = "P70-SEALED-TREE", maximum: int = 4_194_304) -> bytes:
    try:
...
def _release_root(repo_root: Path, directory: str) -> Path:
    try:
        ...
        if not root.is_absolute():
            raise SealedDependencyFailure("P70-SEALED-ROOT")
        ...
        raise SealedDependencyFailure("P70-SEALED-ROOT") from error
...
def _verified_release(repo_root: Path, identity: ReleaseIdentity) -> _VerifiedRelease:
    code = f"P70-{identity.directory.upper().replace('-', '-')}-IDENTITY"
...
def load_exact_p39_and_p41(repo_root: Path) -> tuple[ModuleType, ModuleType]:
    ...
    p39_module = _exec_bound_module("p70_exact_p39", p39.root / P39.source, p39.files[P39.source], "P70-P39-IMPORT")
    p41_module = _exec_bound_module("p70_exact_p41", p41.root / P41.source, p41.files[P41.source], "P70-P41-IMPORT")
    for name, parameters in (("verify", ("checkout_root_value", "pulse_25_root", "pulse_27_root", "git")), ("resolve_checkout_root", ("value",)), ("enumerate_release_paths", ("checkout_root", "release_roots"))):
        _signature(p39_module, name, parameters, "P70-P39-API")
    if getattr(p39_module, "EXPECTED_CARDINALITY", None) != 36 or len(getattr(p39_module, "EXPECTED_PATHS", ())) != 36 or not isinstance(getattr(p39_module, "PublicFailure", None), type):
        raise SealedDependencyFailure("P70-P39-API")
    _signature(p41_module, "copy_release", ("source_root_value", "final_root_value", "synchronizer", "copier", "renamer", "remover", "post_rename"), "P70-P41-API")
    _signature(p41_module, "verify_bound_tree", ("root", "phase"), "P70-P41-API")
    _signature(p41_module, "_safe_absolute", ("value",), "P70-P41-API")
    if getattr(p41_module, "EXPECTED_COUNT", None) != 8 or len(getattr(p41_module, "EXPECTED_PATHS", ())) != 8 or not isinstance(getattr(p41_module, "PublicFailure", None), type):
        raise SealedDependencyFailure("P70-P41-API")
...
def load_exact_p52_stage_reader(repo_root: Path) -> ModuleType:
    ...
    module = _load_with_bound_dependencies("p70_exact_p52", bound, P52.source, "P70-P52-IMPORT")
    _signature(module, "_validate_materialization_summary", ("p51", "summary"), "P70-P52-API")
    _signature(module, "_validate_verification_summary", ("p51", "summary"), "P70-P52-API")
    _signature(module, "_validate_materialized_descriptor_root", ("p51", "descriptor_root", "runtime_root"), "P70-P52-API")
    _signature(module, "_verify_public_prelaunch_custody", ("p51", "p39", "p41", "runtime_root", "p39_checkout_root", "p41_final_root", "private_record"), "P70-P52-API")
    _signature(module, "_remove_private_tree", ("p51", "path", "code"), "P70-P52-API")
    _signature(module, "_remove_seed", ("p51", "seed_path", "private_record"), "P70-P52-API")
    return module


def load_exact_p69(repo_root: Path) -> ModuleType:
    bound = _verified_release(repo_root, P69)
    module = _load_with_bound_dependencies("p70_exact_p69", bound, P69.source, "P70-P69-IMPORT")
    _signature(module, "run_capability_bound_diagnostic_executor", ("repo_root", "descriptor_root", "private_runtime_root", "p27_cycle_root", "ubuntu_runtime_parent"), "P70-P69-API")
    for name, parameters in (("_freeze_descriptors", ("p51", "descriptor_root", "descriptors", "runtime_root")), ("_prelaunch_dispatch", ("p51", "frozen", "runtime_root")), ("_normalize_result", ("p51", "frozen", "capture")), ("_close_handles", ("controls", "windows_handle", "windows_launches", "wsl")), ("_known_failure", ("error", "controls")), ("_with_terminal_failure_types", ("controls", "modules"))):
        _signature(module, name, parameters, "P70-P69-API")
    if not isinstance(getattr(module, "_Controls", None), type) or not isinstance(getattr(module, "_NativeWslSession", None), type) or not isinstance(getattr(module, "ExecutorFailure", None), type) or getattr(module, "REQUEST_COUNT", None) != 69:
        raise SealedDependencyFailure("P70-P69-API")
    return module
...
def load_exact_p35_materializer_and_verifier(repo_root: Path) -> tuple[ModuleType, ModuleType]:
    ...
    manifest_raw = _safe_regular(release / "public-manifest.json", "P70-P35-IDENTITY")
    materializer_raw = _safe_regular(release / "corpus_materializer.py", "P70-P35-IDENTITY", 131_072)
    verifier_raw = _safe_regular(release / "verify_materialization.py", "P70-P35-IDENTITY", 131_072)
    manifest = _sealed_json(manifest_raw, "P70-P35-IDENTITY")
    if (sha256_bytes(manifest_raw) != P35_MANIFEST_RAW or sha256_bytes(materializer_raw) not in P35_MATERIALIZER_HASHES or sha256_bytes(verifier_raw) not in P35_VERIFIER_HASHES or manifest.get("schema") != "ferris.pulse-35-public-corpus-materializer-manifest/v1" or manifest.get("aggregate") != P35_MANIFEST_AGGREGATE or manifest.get("file_count") != 8 or manifest.get("total_bytes") != 403316):
        raise SealedDependencyFailure("P70-P35-IDENTITY")
    materializer = _exec_bound_module("p70_exact_p35_materializer", release / "corpus_materializer.py", materializer_raw, "P70-P35-IMPORT")
    verifier = _exec_bound_module("p70_exact_p35_verifier", release / "verify_materialization.py", verifier_raw, "P70-P35-IMPORT")
    for module in (materializer, verifier):
        if getattr(module, "REQUIRED_CASE_COUNT", None) != 70 or getattr(module, "MAX_LOGICAL_CASES", None) != 512 or getattr(module, "DERIVATION", None) != "hmac-sha256-seed-key-domain-purpose-counter-v1" or not isinstance(getattr(module, "MaterializationError", None), type):
            raise SealedDependencyFailure("P70-P35-API")
    _signature(materializer, "materialize", ("seed_path", "output", "case_count"), "P70-P35-API")
    _signature(materializer, "seed_commitment", ("seed",), "P70-P35-API")
    _signature(verifier, "verify", ("output", "seed_path"), "P70-P35-API")
    return materializer, verifier


def load_exact_p69_stack(repo_root: Path) -> tuple[ModuleType, ModuleType, ModuleType]:
    """Return exact P69 plus its own byte-bound P51/P56 modules."""
    p69 = load_exact_p69(repo_root)
    try:
        p51 = p69.load_exact_p51(repo_root)
        p56 = p69.load_exact_p56(repo_root)
    except SealedDependencyFailure:
        raise
    except BaseException as error:
        raise SealedDependencyFailure("P70-P69-STACK") from error
    return p69, p51, p56
...
def release_identities() -> dict[str, dict[str, str]]:
    return {
        ...
        "pulse_69": {"manifest": P69.manifest_raw_sha256, "seal": P69.seal_payload_sha256},
    }
