"""Byte-bound exact imports for Pulse 78's stage-capture/bootstrap-argv stack."""
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p78"
_WINDOWS_WAIT_OBJECT_0 = 0x00000000
_WINDOWS_WAIT_ABANDONED = 0x00000080
_P78_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p78_active_sealed_loading_lock", default=None)
)
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P78-SEALED-LOCK-STATE")
    global _P78_INTERNAL_LOCK_MANAGER
    _P78_INTERNAL_LOCK_MANAGER = manager



def _require_internal_lock_manager() -> object:
    if _P78_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P78-SEALED-LOCK-STATE")
    return _P78_INTERNAL_LOCK_MANAGER
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P78-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result
...
def _safe_regular(
    path: Path, code: str = "P78-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P78-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P78-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P78-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P78-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P78-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P78-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P78-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P78-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P78-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P78-SEALED-LOCK-STATE")
                else:
...
def load_exact_p75_stack(repo_root: Path) -> tuple[ModuleType, ModuleType, ModuleType, ModuleType]:
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P75, "P78-P75-IDENTITY")
        p75 = _load_exact_p75(bound, "P78-P75-IMPORT")
        _signature(
            p75,
        ...
        for name in (
            ...
            if not callable(getattr(p75, name, None)):
                raise SealedDependencyFailure("P78-P75-API")
        if (
        ...
        try:
            ...
            raise SealedDependencyFailure("P78-P75-STACK") from error
        ...
        if (
            ...
            raise SealedDependencyFailure("P78-P75-API")
        p75._p78_bound_sealed_dependencies = dependencies
