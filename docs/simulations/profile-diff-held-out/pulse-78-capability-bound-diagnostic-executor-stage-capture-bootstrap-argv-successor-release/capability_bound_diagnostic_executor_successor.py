"""Pulse 78 stage-capture/bootstrap-argv successor to exact Pulse 75."""
...
LOCAL_SEALED_DEPENDENCIES_SHA256 = (
    "sha256:b91d09f908323eff205586ffc9afcbcc9063f7e17947d763ee194e4a7cf7385d"
)
_LOCAL_SEALED_DEPENDENCIES_RUNTIME_PREFIX = (
    "ferris.pulse-78.local-sealed-dependencies.runtime"
)
_LOCAL_SLOT_MISSING = object()
_CROSS_INSTANCE_REENTRY_STATE_KEY = (
    "_ferris_p78_cross_instance_reentry_advisory_state_v1"
)


class _Pulse78LinuxLockManager:
    def __init__(self) -> None:
        ...
        if os.name == "posix" and sys.platform.startswith("linux"):
            ...
            if not callable(register_at_fork):
                raise RuntimeError("P78-LOCAL-SEALED-FORK-HOOK")
...
_P78_INTERNAL_LOCK_MANAGER = _Pulse78LinuxLockManager()
...
def _local_sealed_dependencies_path() -> Path:
    try:
        ...
        raise _LocalSealedBootstrapFailure("P78-LOCAL-SEALED-PATH") from error
...
def _verified_local_sealed_content(path: Path) -> bytes:
    content = _safe_local_regular(path, "P78-LOCAL-SEALED-IMPORT")
    if _local_sealed_source_digest(content) != LOCAL_SEALED_DEPENDENCIES_SHA256:
        raise _LocalSealedBootstrapFailure("P78-LOCAL-SEALED-IDENTITY")
    return content
...
def _bind_local_sealed_lock_manager_module(module: ModuleType) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise _LocalSealedBootstrapFailure("P78-LOCAL-SEALED-STATE")
    try:
        module_path = Path(module.__file__).resolve(strict=True)
    except (AttributeError, OSError, TypeError) as error:
        raise _LocalSealedBootstrapFailure("P78-LOCAL-SEALED-STATE") from error
    if module_path != _local_sealed_dependencies_path():
        raise _LocalSealedBootstrapFailure("P78-LOCAL-SEALED-STATE")
    binder = getattr(module, "_bind_internal_lock_manager", None)
    if not callable(binder):
        raise _LocalSealedBootstrapFailure("P78-LOCAL-SEALED-STATE")
    binder(_P78_INTERNAL_LOCK_MANAGER)
    return module
...
def _exec_local_sealed_module(path: Path, content: bytes) -> ModuleType:
    ...
    try:
        ...
        if isinstance(error, _LocalSealedBootstrapFailure):
            raise
        raise _LocalSealedBootstrapFailure("P78-LOCAL-SEALED-IMPORT") from error
    current = sys.modules.get(name)
...
_BUNDLE_STAGE_SCHEMA = "ferris.pulse-78-wsl-bundle-staged/v1"
_BUNDLE_REVALIDATION_SCHEMA = "ferris.pulse-78-staged-bundle-revalidation/v1"
_BUNDLE_CLEANUP_SCHEMA = "ferris.pulse-78-staged-bundle-cleanup/v1"
WSL_WORKER_SHA256 = "sha256:a1d9b3a31d59434300a63d36effe14382186ab1ba61c5f9a4247e260aa478f3c"
WORKER_SEALED_DEPENDENCIES_SHA256 = "sha256:a231b3b2a1c1799e5472a38d35eedd533561494282c4c45c2374cf5fc3750bb6"
...
def _stage_failure(root_path,failure_code,bundle_absent_verified,cleanup_posture):
 return {
  ...
  "schema":"ferris.pulse-78-wsl-bundle-staged/v1",
  "status":"failed",
 }
...
try:
 ...
 try:
  ...
  _emit(_stage_failure(root_path,"P78-INDETERMINATE-STAGE-CLEANUP",False,"indeterminate"))
  raise SystemExit(0)
 try:
  try:
...
if type(request) is not dict or set(request)!=required or request["schema"]!="ferris.pulse-78-staged-bundle-revalidation/v1": raise SystemExit(2)
...
try:
 ...
 try:
  ...
  os.close(root_fd)
 response={"bundle_root": expected_root, "schema": "ferris.pulse-78-staged-bundle-revalidation/v1", "verified": True}
...
if type(request) is not dict or set(request)!=required or request["schema"]!="ferris.pulse-78-staged-bundle-cleanup/v1": raise SystemExit(2)
...
try:
 ...
 response={
  ...
  "schema": "ferris.pulse-78-staged-bundle-cleanup/v1",
 }
 sys.stdout.buffer.write(json.dumps(response,allow_nan=False,ensure_ascii=True,separators=(",",":"),sort_keys=True).encode("ascii")+b"\n")
 sys.stdout.buffer.flush()
finally:
...
def _stage_owned_bundle(repo_root: Path, ubuntu_runtime_parent: str) -> _OwnedBundle:
    ...
    if (
        ...
        if response["failure_code"] == "P78-INDETERMINATE-STAGE-CLEANUP":
            raise ExecutorFailure("P78-INDETERMINATE-STAGE-CLEANUP")
...
dependency_relative=sys.argv[10]
expected_dependency_sha256=sys.argv[11]
flags=os.O_RDONLY|getattr(os,"O_DIRECTORY",0)|getattr(os,"O_NOFOLLOW",0)
if not parent.startswith("/") or parent.startswith("/mnt/") or "/" in name or not name.startswith(".p57-"): raise SystemExit(2)
if expected_root!=parent.rstrip("/")+"/"+name: raise SystemExit(2)
 ...
 try:
  ...
  try:
   ...
   try:
    ...
    sys.argv=[expected_root+"/"+worker_relative,*sys.argv[12:]]
...
def _run_qualification_executor(
    ...
    """Private fake-only seam used by Pulse 78 qualification and successors."""
