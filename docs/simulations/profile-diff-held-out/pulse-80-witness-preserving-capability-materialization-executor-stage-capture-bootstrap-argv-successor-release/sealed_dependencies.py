"""Byte-bound exact imports for Pulse 80's witness-preserving Pulse 79 wrapper."""
...
P79_COMMIT = "2be90a8c88cc96387ea8e6f7e599058d92bf5459"
P79_RELEASE_ROOT = (
    "docs/simulations/profile-diff-held-out/"
    "pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor-release"
)
P58_GATE_IDS = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p80"
...
_P80_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p80_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 79 predecessor or callable binding failed."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P80-SEALED-LOCK-STATE")
    global _P80_INTERNAL_LOCK_MANAGER
    _P80_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P80_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P80-SEALED-LOCK-STATE")
    return _P80_INTERNAL_LOCK_MANAGER
...
P79 = ReleaseIdentity(
    ...
    "ferris.pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor-manifest/v1",
    "sha256:7a876d0133563085182fac14651eef73757f73abd3fbf61f14336cd4592dfbb7",
    "sha256:5053f67811c4b3c3e4763a9acd4ba4cbfa6e9a2efa27184916796eeb122ba329",
    13,
    15,
    "sha256:931ff5628bccb8ad8b115e1301355ff870b48b6110049a9a5baaf01676f57e7d",
    "sha256:c267d12567f479d10e85a46ab5eb69166c9e0b3eec743b6c16e36186aafa1efb",
    "sha256:392a837ecdce40262e009575273869766c4475f8680f743391ca1c6caa528e8b",
    "sha256:1cffe94275e0a6d41298bf4b26bfd287800b2219ea507c78c0005a603af52366",
)
...
P79_PREDECESSORS = {
    ...
    "pulse_78": {
        "commit": "7c60f6b384bec1494fbbabaa244b8f0f8eece355",
        "manifest": "sha256:f5cd77f0f31474677bbc847a75e0d307b9a41e7b313dcf2be22a9135fb99b7fb",
        "release_root": "docs/simulations/profile-diff-held-out/pulse-78-capability-bound-diagnostic-executor-stage-capture-bootstrap-argv-successor-release",
        "seal": "sha256:a56612188fccec0c7f4cb7b0d40922f10d4128265d500878ca3ace6202e77a55",
    },
}
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P80-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(
    path: Path, code: str = "P80-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P80-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P80-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P80-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P80-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P80-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P80-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P80-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P80-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P80-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P80-SEALED-LOCK-STATE")
                else:
...
def _patch_loaded_dependency_module(module: object, code: str) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise SealedDependencyFailure(code)
    if getattr(module, "__p80_bound_dependency_patch__", False):
        return module
    ...
    module.__p80_bound_dependency_patch__ = True
    return module
...
def load_pulse79(
    ...
    """Return fresh exact Pulse 79 modules after exact-tree verification."""
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P79, "P80-P79-IDENTITY")
        p79 = _load_with_bound_dependencies(
            "p80_exact_p79", bound, P79.source, "P80-P79-IMPORT"
        )
        _signature(
            p79,
        ...
        if (
            tuple(getattr(p79, "P58_GATE_IDS", ())) != P58_GATE_IDS
            or getattr(p79, "P39_CALLER_AUTHORITY_PRECONDITION", None)
            != "future-authority-supplied-fresh-anonymous-exact-cutoff-root"
            or not isinstance(
                getattr(p79, "OrderedCapabilityMaterializationResult", None), type
            )
            or not isinstance(getattr(p79, "P58Failure", None), type)
            or not isinstance(getattr(p79, "IndeterminateCleanup", None), type)
        ):
            raise SealedDependencyFailure("P80-P79-API")

        try:
            p52 = p79.load_exact_p52_stage_reader(repo_root)
            p78, p51, _p56 = p79.load_exact_p78_stack(repo_root)
            p43, _p45, p47 = p51.load_terminal_dependencies(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P80-P79-STACK") from error
        ...
        for module, name, parameters in (
            ...
            _signature(module, name, parameters, "P80-P79-STACK")
        if not isinstance(getattr(p51, "TerminalPulse47Once", None), type):
            raise SealedDependencyFailure("P80-P79-STACK")

        return p79, p52, p78, p51, p43, p47
...
def release_identities() -> dict[str, object]:
    return {
        ...
        "pulse_79": {
            "commit": P79_COMMIT,
            "manifest_aggregate": P79.manifest_aggregate,
            "manifest_raw_sha256": P79.manifest_raw_sha256,
            "receipt_payload_sha256": P79.receipt_payload_sha256,
            "receipt_raw_sha256": P79.receipt_raw_sha256,
            "release_root": P79_RELEASE_ROOT,
            "seal_payload_sha256": P79.seal_payload_sha256,
            "seal_raw_sha256": P79.seal_raw_sha256,
            "source_sha256": P79.source_sha256,
            "stack": json.loads(canonical_bytes(P79_PREDECESSORS)),
        },
    }
