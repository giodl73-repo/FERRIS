...
RUN_ROOT = REPO_ROOT / "target" / "pulse-80-test-runtime"
...
_LOCAL_SEALED_MODULE_PREFIX = "_ferris_p80_local_sealed_dependencies_v1_"
...
from sealed_dependencies import P79_COMMIT, P79_RELEASE_ROOT, load_pulse79  # noqa: E402
    ...
    for name in tuple(sys.modules):
        if name.startswith(_DYNAMIC_RELEASE_PREFIXES) or name.startswith(
            (
                ...
                "ferris.pulse-80.local-sealed-dependencies.runtime",
            )
        ):
            sys.modules.pop(name, None)
...
def _load_fresh_release_module(relative: str) -> object:
    path = ROOT / relative
    name = f"pulse80_test_{path.stem}_{uuid.uuid4().hex}"
...
class WitnessPreservingCapabilityMaterializationExecutorTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.sandbox.mkdir()
        self.p79, self.p52, self.p78, self.p51, self.p43, self.p47 = load_pulse79(
            REPO_ROOT
        )
    ...
    def _sealed_stub(self, modules: tuple[object, ...] | None = None) -> object:
        ...
        bound = (
            ...
            else (self.p79, self.p52, self.p78, self.p51, self.p43, self.p47)
        )
        return types.SimpleNamespace(
            SealedDependencyFailure=StubSealedDependencyFailure,
            load_pulse79=lambda _repo_root: bound,
        )

    def _assert_exact_loaded_stack(self, modules: tuple[object, ...]) -> None:
        p79, p52, p78, p51, p43, p47 = modules
        self.assertEqual(tuple(p79.P58_GATE_IDS), executor.P58_GATE_IDS)
        self.assertTrue(callable(p79.run_ordered_capability_materialization_executor))
    ...
    def _assert_completed_execution(
        ...
        *,
        ...
        self.assertEqual(result.private_record["p79_execution_outcome"], "completed")
    ...
    def test_sealed_binding_and_production_signature_match_pulse79(self) -> None:
        self.assertEqual(P79_COMMIT, "2be90a8c88cc96387ea8e6f7e599058d92bf5459")
        self.assertEqual(
            P79_RELEASE_ROOT,
            "docs/simulations/profile-diff-held-out/pulse-79-ordered-capability-materialization-executor-stage-capture-bootstrap-argv-successor-release",
        )
        self.assertEqual(
            tuple(
                inspect.signature(
        ...
        self.assertIn("class _Pulse80LinuxLockManager", source)
        ...
        self.assertIn('_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p80"', sealed_source)
        ...
        self.assertIn("_P80_INTERNAL_LOCK_MANAGER", sealed_source)
        self.assertIn("_bind_internal_lock_manager", sealed_source)
        self.assertIn("P80-SEALED-LOCK-CROSS-INSTANCE-REENTRY", sealed_source)
        self.assertIn("_WINDOWS_WAIT_ABANDONED", sealed_source)
        self.assertNotIn("register_at_fork", sealed_source)
        self.assertNotIn("pulse-80-sealed-loader-locks", sealed_source)
    ...
    def test_executor_import_ignores_external_sealed_dependencies(self) -> None:
        ...
        attacker_file.write_text(
            "raise RuntimeError('P80 attack path resolution succeeded')\n",
            encoding="utf-8",
            newline="\n",
        )

        def hijacked(_repo_root: Path) -> object:
            raise AssertionError("P80 attack sys.modules resolution succeeded")

        malicious = types.SimpleNamespace(
            P58_GATE_IDS=("hijacked",),
            SealedDependencyFailure=RuntimeError,
            load_pulse79=hijacked,
        ...
        try:
            ...
            p79, _p52, _p78, _p51, _p43, _p47 = sealed.load_pulse79(REPO_ROOT)
        finally:
    ...
    def test_sealed_dependency_loader_ignores_cache_preseed_and_mutation(self) -> None:
        ...
        first.load_pulse79 = lambda _repo_root: (_ for _ in ()).throw(
        ...
        self.assertTrue(callable(second.load_pulse79))
        self.assertIsNot(second.SealedDependencyFailure, RuntimeError)
        self._assert_exact_loaded_stack(second.load_pulse79(REPO_ROOT))

    def test_concurrent_load_pulse79_serializes_and_restores_foreign_sentinel(self) -> None:
        ...
        def gated_exec(
            ...
            **kwargs: object,
            ...
            if name == "p80_exact_p79" and not seen_first:
                seen_first = True
                first_holding.set()
        ...
        def worker(binder: object) -> None:
            try:
                results.append(binder.load_pulse79(REPO_ROOT))
    ...
    def test_load_pulse79_import_exception_restores_generic_slot(self) -> None:
        fresh_executor = _load_fresh_release_module(
            "witness_preserving_capability_materialization_executor.py"
        )
    ...
    def test_stress_concurrent_legitimate_binder_pairs_complete_without_failure(self) -> None:
        ...
        expected_paths = tuple(
            ...
            for module in (self.p79, self.p52, self.p78, self.p51, self.p43, self.p47)
        )
        ...
        def worker(label: str, binder: object, pair_index: int) -> None:
            try:
                ...
                modules = binder.load_pulse79(REPO_ROOT)
                with results_lock:
    ...
    def test_kernel_lock_name_is_stable_across_fresh_binders(self) -> None:
        ...
        self.assertNotIn("pulse-80-sealed-loader-locks", first)
        self.assertTrue(
            first.startswith("Local\\")
            if os.name == "nt"
            else first.startswith("\0ferris-p80-")
        )
    ...
    def test_kernel_lock_rejects_unsupported_posix_platform(self) -> None:
        ...
        with (
            ...
            with self.assertRaises(binder.SealedDependencyFailure) as raised:
                binder._open_kernel_lock("\0ferris-p80-unsupported")
        self.assertEqual(str(raised.exception), "P80-SEALED-LOCK-OPEN")
    ...
    def test_kernel_lock_cross_instance_same_thread_fails_closed_quickly_and_clears_marker(
        ...
        binder_a = fresh_a._load_local_sealed_dependencies()
        binder_b = fresh_b._load_local_sealed_dependencies()
        manager_a = fresh_a._P80_INTERNAL_LOCK_MANAGER
        manager_b = fresh_b._P80_INTERNAL_LOCK_MANAGER
        ...
        with binder_a._sealed_loading_lock() as owner_state:
            ...
            elapsed = time.monotonic() - started
            self.assertEqual(
                str(raised.exception), "P80-SEALED-LOCK-CROSS-INSTANCE-REENTRY"
            )
    ...
    def test_kernel_lock_at_fork_registration_is_idempotent_per_binder(self) -> None:
        ...
        manager = fresh_executor._P80_INTERNAL_LOCK_MANAGER
        self.assertEqual(manager._fork_hook_registrations, 1)
        manager_ids: set[int] = set()
        for _ in range(25):
            binder = fresh_executor._load_local_sealed_dependencies()
            manager_ids.add(id(binder._P80_INTERNAL_LOCK_MANAGER))
            self.assertIs(binder._P80_INTERNAL_LOCK_MANAGER, manager)
            self.assertEqual(
                binder._P80_INTERNAL_LOCK_MANAGER._fork_hook_registrations, 1
            )
    ...
    def test_kernel_lock_does_not_create_path_artifacts(self) -> None:
        ...
        binder = fresh_executor._load_local_sealed_dependencies()
        artifact_root = REPO_ROOT / "target" / "pulse-80-sealed-loader-locks"
    ...
    def test_kernel_lock_acquire_failure_cleans_up(self) -> None:
        ...
        with (
            ...
            patch.object(
                ...
                side_effect=binder.SealedDependencyFailure("P80-SEALED-LOCK-ACQUIRE"),
            ),
            patch.object(binder, "_close_kernel_lock", new=close_kernel_lock),
        ):
            with self.assertRaises(binder.SealedDependencyFailure) as raised:
        ...
        self.assertEqual(str(raised.exception), "P80-SEALED-LOCK-ACQUIRE")
    ...
    def test_old_private_binder_key_is_ignored(self) -> None:
        ...
        try:
            ...
            self.assertTrue(callable(binder.load_pulse79))
            self._assert_exact_loaded_stack(binder.load_pulse79(REPO_ROOT))
        finally:
            _clean_loaded_test_modules()
    ...
    def test_local_sealed_binder_exception_cleanup_leaves_no_stale_runtime_slot(self) -> None:
        ...
        try:
            ...
            self.assertEqual(str(raised.exception), "P80-LOCAL-SEALED-IMPORT")
            self.assertFalse(
    ...
    def test_qualification_delegates_to_exact_p79_executor(self) -> None:
        with patch.object(
            self.p79,
            "_run_qualification_executor",
            wraps=self.p79._run_qualification_executor,
        ) as wrapped:
    ...
    def test_published_result_retains_verified_result_and_witness_after_p79_cleanup(self) -> None:
    ...
    def test_p79_prelaunch_failure_remains_publication_not_attempted(self) -> None:
        p39, p41 = self.p79.load_exact_p39_and_p41(REPO_ROOT)

        def fail(*_args: object) -> object:
            raise p39.PublicFailure("P39-TEST-FAILURE")

        with (
            patch.object(self.p79, "load_exact_p39_and_p41", return_value=(p39, p41)),
    ...
    def test_preexisting_terminal_root_is_rejected_before_p79_execution(self) -> None:
        ...
        def open_wsl(_repo: Path, _parent: str, _api: object) -> object:
            raise AssertionError("P79 must not build WSL capability when terminal root exists")
        ...
        with (
            ...
            patch.object(
                self.p79,
                "_run_qualification_executor",
                side_effect=AssertionError("P79 must not run"),
            ),
        ):
            result = executor._run_qualification_executor(
                REPO_ROOT,
        ...
        self.assertEqual(result.private_record["failure_code"], "P80-TERMINAL-ROOT-NOT-FRESH")
        self.assertEqual(result.publication["disposition"], "not-attempted")
        self.assertEqual(fake.launches, [])
