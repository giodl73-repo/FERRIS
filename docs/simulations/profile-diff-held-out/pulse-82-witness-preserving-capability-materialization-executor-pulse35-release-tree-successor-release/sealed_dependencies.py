"""Byte-bound exact imports for Pulse 82's witness-preserving Pulse 81 wrapper."""
...
P81_COMMIT = "bc3717d9df1ce0c3a5b724e79820e0b0a20d9c02"
P81_RELEASE_ROOT = (
    "docs/simulations/profile-diff-held-out/"
    "pulse-81-ordered-capability-materialization-executor-pulse35-release-tree-successor-release"
)
P58_GATE_IDS = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p82"
...
_P82_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p82_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 81 predecessor or callable binding failed."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P82-SEALED-LOCK-STATE")
    global _P82_INTERNAL_LOCK_MANAGER
    _P82_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P82_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P82-SEALED-LOCK-STATE")
    return _P82_INTERNAL_LOCK_MANAGER
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P82-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(
    path: Path, code: str = "P82-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P82-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P82-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P82-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P82-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P82-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P82-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P82-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P82-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P82-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P82-SEALED-LOCK-STATE")
                else:
...
def _patch_loaded_dependency_module(module: object, code: str) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise SealedDependencyFailure(code)
    if getattr(module, "__p82_bound_dependency_patch__", False):
        return module
    ...
    module.__p82_bound_dependency_patch__ = True
    return module
...
def load_pulse81(
    ...
    """Return fresh exact Pulse 81 modules after exact-tree verification."""
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P81, "P82-P81-IDENTITY")
        p81 = _load_with_bound_dependencies(
            "p82_exact_p81", bound, P81.source, "P82-P81-IMPORT"
        )
        _signature(
            p81,
        ...
        if (
            tuple(getattr(p81, "P58_GATE_IDS", ())) != P58_GATE_IDS
            or getattr(p81, "P39_CALLER_AUTHORITY_PRECONDITION", None)
            != "future-authority-supplied-fresh-anonymous-exact-cutoff-root"
            or not isinstance(
                getattr(p81, "OrderedCapabilityMaterializationResult", None), type
            )
            or not isinstance(getattr(p81, "P58Failure", None), type)
            or not isinstance(getattr(p81, "IndeterminateCleanup", None), type)
        ):
            raise SealedDependencyFailure("P82-P81-API")

        try:
            p52 = p81.load_exact_p52_stage_reader(repo_root)
            p78, p51, _p56 = p81.load_exact_p78_stack(repo_root)
            p43, _p45, p47 = p51.load_terminal_dependencies(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P82-P81-STACK") from error
        ...
        for module, name, parameters in (
            ...
            _signature(module, name, parameters, "P82-P81-STACK")
        if not isinstance(getattr(p51, "TerminalPulse47Once", None), type):
            raise SealedDependencyFailure("P82-P81-STACK")

        return p81, p52, p78, p51, p43, p47
...
def release_identities() -> dict[str, object]:
    return {
        ...
        "pulse_81": {
            "commit": P81_COMMIT,
            "manifest_aggregate": P81.manifest_aggregate,
            "manifest_raw_sha256": P81.manifest_raw_sha256,
            "receipt_payload_sha256": P81.receipt_payload_sha256,
            "receipt_raw_sha256": P81.receipt_raw_sha256,
            "release_root": P81_RELEASE_ROOT,
            "seal_payload_sha256": P81.seal_payload_sha256,
            "seal_raw_sha256": P81.seal_raw_sha256,
            "source_sha256": P81.source_sha256,
            "stack": json.loads(canonical_bytes(P81_PREDECESSORS)),
        },
    }
