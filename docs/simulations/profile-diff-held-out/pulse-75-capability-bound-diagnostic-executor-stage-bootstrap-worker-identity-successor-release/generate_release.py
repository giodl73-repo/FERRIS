#!/usr/bin/env python3
"""Generate Pulse 75's complete-tree manifest and release seal."""
...
SCHEMA_PATH = (
    "schemas/"
    "ferris.pulse-75-capability-bound-diagnostic-executor-stage-bootstrap-worker-identity-successor.v1.schema.json"
)
...
def public_files() -> list[Path]:
    ...
    if residue:
        raise RuntimeError(
            "P75 release tree contains Python cache residue: "
...
def main() -> None:
    ...
    manifest = {
        ...
        "schema": "ferris.pulse-75-capability-bound-diagnostic-executor-stage-bootstrap-worker-identity-successor-manifest/v1",
        "scope": (
            "Stage-bootstrap/worker-identity exact Pulse 72 successor preserving "
            "exact P57/P56/P51 semantics while byte-binding a local sibling sealed "
            "loader, owning every post-create stage failure cleanup inside the WSL "
            "bootstrap, and requiring the worker bootstrap plus worker dependency "
            "load path to execute only from verified bundle identity and bytes."
        ),
    ...
    payload = {
        "schema": "ferris.pulse-75-capability-bound-diagnostic-executor-stage-bootstrap-worker-identity-successor-seal-payload/v1",
        "manifest": {
    ...
    write(
        ROOT / "release-seal.json",
        {
            ...
            "schema": "ferris.pulse-75-capability-bound-diagnostic-executor-stage-bootstrap-worker-identity-successor-seal/v1",
            "seal_id": identity,
        },
    )
