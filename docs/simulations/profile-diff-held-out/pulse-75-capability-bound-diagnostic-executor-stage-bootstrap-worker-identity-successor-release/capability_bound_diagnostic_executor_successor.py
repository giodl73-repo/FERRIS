"""Pulse 75 stage-bootstrap/worker-identity successor to exact Pulse 72."""
...
LOCAL_SEALED_DEPENDENCIES_SHA256 = (
    "sha256:618018063aafca6553f2f69d46aee47c3577b3e5dbcedfdb0944e2ae4444c53b"
)
_LOCAL_SEALED_DEPENDENCIES_RUNTIME_PREFIX = (
    "ferris.pulse-75.local-sealed-dependencies.runtime"
)
_LOCAL_SLOT_MISSING = object()
_CROSS_INSTANCE_REENTRY_STATE_KEY = (
    "_ferris_p75_cross_instance_reentry_advisory_state_v1"
)


class _Pulse75LinuxLockManager:
    def __init__(self) -> None:
        ...
        if os.name == "posix" and sys.platform.startswith("linux"):
            ...
            if not callable(register_at_fork):
                raise RuntimeError("P75-LOCAL-SEALED-FORK-HOOK")
...
_P75_INTERNAL_LOCK_MANAGER = _Pulse75LinuxLockManager()
...
def _local_sealed_dependencies_path() -> Path:
    try:
        ...
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-PATH") from error
...
def _verified_local_sealed_content(path: Path) -> bytes:
    content = _safe_local_regular(path, "P75-LOCAL-SEALED-IMPORT")
    if _local_sealed_source_digest(content) != LOCAL_SEALED_DEPENDENCIES_SHA256:
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-IDENTITY")
    return content
...
def _bind_local_sealed_lock_manager_module(module: ModuleType) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-STATE")
    try:
        module_path = Path(module.__file__).resolve(strict=True)
    except (AttributeError, OSError, TypeError) as error:
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-STATE") from error
    if module_path != _local_sealed_dependencies_path():
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-STATE")
    binder = getattr(module, "_bind_internal_lock_manager", None)
    if not callable(binder):
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-STATE")
    binder(_P75_INTERNAL_LOCK_MANAGER)
    return module
...
def _exec_local_sealed_module(path: Path, content: bytes) -> ModuleType:
    ...
    try:
        ...
        if isinstance(error, _LocalSealedBootstrapFailure):
            raise
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-IMPORT") from error
    current = sys.modules.get(name)
    if current is not module:
        raise _LocalSealedBootstrapFailure("P75-LOCAL-SEALED-STATE")
    sys.modules.pop(name, None)
    if not callable(getattr(module, "load_exact_p72_stack", None)) or not isinstance(
...
_P72, _P57, _P51, _P56 = _SEALED.load_exact_p72_stack(REPO_ROOT)
...
_BUNDLE_STAGE_SCHEMA = "ferris.pulse-75-wsl-bundle-staged/v1"
_BUNDLE_REVALIDATION_SCHEMA = "ferris.pulse-75-staged-bundle-revalidation/v1"
_BUNDLE_CLEANUP_SCHEMA = "ferris.pulse-75-staged-bundle-cleanup/v1"
WSL_WORKER_SHA256 = "sha256:53d7ce9a8a02b867b0c262d4ccbefc9a8b92cc465900486aa7dd03d08f780c70"
WORKER_SEALED_DEPENDENCIES_SHA256 = "sha256:5473c2e572475192c2ebf13ea7d6570e2a6beb5049b76040db958e107498a6b6"
...
def _cleanup_created_root(parent_fd,root_fd,name,root_path,created_identity):
 try:
  ...
  try:
   ...
   pass
  return {"bundle_absent_verified":True,"bundle_root":root_path,"cleanup_posture":"removed","schema":"ferris.pulse-75-wsl-bundle-staged/v1","status":"failed"}
 except BaseException:
  return {"bundle_absent_verified":False,"bundle_root":root_path,"cleanup_posture":"indeterminate","schema":"ferris.pulse-75-wsl-bundle-staged/v1","status":"failed"}
 ...
 try:
  ...
  try:
   ...
   _emit({
    ...
    "root_type": "directory",
    "schema": "ferris.pulse-75-wsl-bundle-staged/v1",
    "status": "staged",
   })
  except BaseException:
...
if type(request) is not dict or set(request)!=required or request["schema"]!="ferris.pulse-75-staged-bundle-revalidation/v1": raise SystemExit(2)
...
try:
 ...
 try:
  ...
  os.close(root_fd)
 response={"bundle_root": expected_root, "schema": "ferris.pulse-75-staged-bundle-revalidation/v1", "verified": True}
...
if type(request) is not dict or set(request)!=required or request["schema"]!="ferris.pulse-75-staged-bundle-cleanup/v1": raise SystemExit(2)
...
try:
 ...
 response={
  ...
  "schema": "ferris.pulse-75-staged-bundle-cleanup/v1",
 }
 sys.stdout.buffer.write(json.dumps(response,allow_nan=False,ensure_ascii=True,separators=(",",":"),sort_keys=True).encode("ascii")+b"\n")
 sys.stdout.buffer.flush()
finally:
...
def _stage_owned_bundle(repo_root: Path, ubuntu_runtime_parent: str) -> _OwnedBundle:
    ...
    if (
        ...
        if (
            response["cleanup_posture"] != "removed"
            or response["bundle_absent_verified"] is not True
        ):
...
expected_root_device=int(sys.argv[6])
expected_root_inode=int(sys.argv[7])
worker_relative=sys.argv[8]
expected_worker_sha256=sys.argv[9]
flags=os.O_RDONLY|getattr(os,"O_DIRECTORY",0)|getattr(os,"O_NOFOLLOW",0)
if not parent.startswith("/") or parent.startswith("/mnt/") or "/" in name or not name.startswith(".p57-"): raise SystemExit(2)
if expected_root!=parent.rstrip("/")+"/"+name: raise SystemExit(2)
...
try:
 parent_stat=os.fstat(parent_fd)
 if not stat.S_ISDIR(parent_stat.st_mode) or (parent_stat.st_dev,parent_stat.st_ino)!=(expected_parent_device,expected_parent_inode): raise SystemExit(2)
 root_stat=os.lstat(name,dir_fd=parent_fd)
 if stat.S_ISLNK(root_stat.st_mode) or not stat.S_ISDIR(root_stat.st_mode): raise SystemExit(2)
 ...
 try:
  ...
  try:
   ...
   try:
    ...
    sys.argv=[expected_root+"/"+worker_relative,*sys.argv[10:]]
...
def _run_qualification_executor(
    ...
    """Private fake-only seam used by Pulse 75 qualification and successors."""
