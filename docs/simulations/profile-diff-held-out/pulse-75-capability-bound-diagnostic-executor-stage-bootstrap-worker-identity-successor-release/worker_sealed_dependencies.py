"""Identity-bound exact P56 loader for Pulse 75's WSL worker."""
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P75-WORKER-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result
...
def _relative_parts(path: str) -> tuple[str, ...]:
    ...
    if not parts or any(not part or part in {".", ".."} for part in parts):
        raise SealedDependencyFailure("P75-WORKER-SEALED-TREE")
    return parts
...
def _exec_bound_module(name: str, source: Path, content: bytes) -> ModuleType:
    ...
    try:
        ...
        raise SealedDependencyFailure("P75-WORKER-SEALED-IMPORT") from error
    return module
...
def _expect_signature(module: ModuleType, name: str, parameters: tuple[str, ...]) -> None:
    ...
    if not callable(value) or tuple(inspect.signature(value).parameters) != parameters:
        raise SealedDependencyFailure("P75-WORKER-SEALED-API")


def load_exact_p56(bundle_root_fd: int, proc_prefix: str) -> ModuleType:
    bound = _verified_release(bundle_root_fd, proc_prefix, P56, "P75-WORKER-SEALED-IDENTITY")
