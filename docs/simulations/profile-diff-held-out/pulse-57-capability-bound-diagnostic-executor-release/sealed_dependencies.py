"""Verify and import the complete immutable Pulse 51 and Pulse 56 releases."""
...
def load_p51_synthetic_fixture(repo_root: Path, p51: ModuleType) -> ModuleType:
    """Load P51's source-bound private fixture only for fake qualification."""

    bound = _verified_release(repo_root, P51)
    fixture = "synthetic_fixture.py"
    if sha256_bytes(bound.files[fixture]) != (
        "sha256:237da632d4447ea30f6efc41dbbecd229b181ce3c6d029a54d77b52b015063b1"
    ):
        raise SealedDependencyFailure("P57-SEALED-IDENTITY")
    previous = sys.modules.get("diagnostic_executor")
    sys.modules["diagnostic_executor"] = p51
    try:
        return _exec_bound_module(
            "p57_exact_p51_synthetic_fixture", bound.root / fixture, bound.files[fixture]
        )
    finally:
        sys.modules.pop("diagnostic_executor", None)
        if previous is not None:
            sys.modules["diagnostic_executor"] = previous
...
def release_identities() -> dict[str, dict[str, str]]:
    ...
    return {
        "pulse_51": {
            "manifest": P51.manifest_raw_sha256,
            "seal": P51.seal_payload_sha256,
        },
        "pulse_56": {
            "manifest": P56.manifest_raw_sha256,
            "seal": P56.seal_payload_sha256,
        },
    }
