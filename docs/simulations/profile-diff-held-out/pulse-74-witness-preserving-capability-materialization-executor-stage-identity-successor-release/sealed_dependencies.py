"""Byte-bound exact imports for Pulse 74's witness-preserving Pulse 73 wrapper."""
...
P73_COMMIT = "450b4b4bd79b2217bdda368b1210b4ba7c519e0e"
P73_RELEASE_ROOT = (
    "docs/simulations/profile-diff-held-out/"
    "pulse-73-ordered-capability-materialization-executor-stage-identity-successor-release"
)
P58_GATE_IDS = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p74"
...
_P74_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p74_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 73 predecessor or callable binding failed."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P74-SEALED-LOCK-STATE")
    global _P74_INTERNAL_LOCK_MANAGER
    _P74_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P74_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P74-SEALED-LOCK-STATE")
    return _P74_INTERNAL_LOCK_MANAGER


P73 = ReleaseIdentity(
    "pulse-73-ordered-capability-materialization-executor-stage-identity-successor-release",
    "ordered_capability_materialization_executor.py",
    "sha256:d239c379aa2b3cbd7a54176e603cca8e43cc1636a9a1f1b4b7fdde4c031d5650",
    ...
    13,
    15,
    "sha256:a647491e0e7ffa5c365940bb233c0a0f4d8a14ac70d838cc0ca6b3582dd3288f",
    "sha256:9d22b4a3f631dbaec0594bd6273f89104c6c1a3f65e625e91ed68e4b13e5b0b5",
    "sha256:1c94a3e8054a124090c2c790c43b2ccca8e7905c6b9fed4f9c738bc44407c303",
...
P73_PREDECESSORS = {
    ...
    "pulse_72": {
        "manifest": "sha256:ba65fa3e8b9363e3345b736d12152f9b9dd46bd5ac1ef8b718407868d9a749b9",
        "seal": "sha256:f80fdcc6b3d7b23cd2e81004c7e5243c75512234adc049805cb025c4f93ec7a3",
    },
}
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P74-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(
    path: Path, code: str = "P74-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P74-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P74-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P74-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P74-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P74-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P74-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P74-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P74-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P74-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P74-SEALED-LOCK-STATE")
                else:
...
def _patch_loaded_dependency_module(module: object, code: str) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise SealedDependencyFailure(code)
    if getattr(module, "__p74_bound_dependency_patch__", False):
        return module
    ...
    module.__p74_bound_dependency_patch__ = True
    return module
...
def load_pulse73(
    ...
    """Return fresh exact Pulse 73 modules after exact-tree verification."""
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P73, "P74-P73-IDENTITY")
        p73 = _load_with_bound_dependencies(
            "p74_exact_p73", bound, P73.source, "P74-P73-IMPORT"
        )
        _signature(
            p73,
        ...
        if (
            tuple(getattr(p73, "P58_GATE_IDS", ())) != P58_GATE_IDS
            or getattr(p73, "P39_CALLER_AUTHORITY_PRECONDITION", None)
            != "future-authority-supplied-fresh-anonymous-exact-cutoff-root"
            or not isinstance(
                getattr(p73, "OrderedCapabilityMaterializationResult", None), type
            )
            or not isinstance(getattr(p73, "P58Failure", None), type)
            or not isinstance(getattr(p73, "IndeterminateCleanup", None), type)
        ):
            raise SealedDependencyFailure("P74-P73-API")

        try:
            p52 = p73.load_exact_p52_stage_reader(repo_root)
            p72, p51, _p56 = p73.load_exact_p72_stack(repo_root)
            p43, _p45, p47 = p51.load_terminal_dependencies(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P74-P73-STACK") from error
        ...
        for module, name, parameters in (
            ...
            _signature(module, name, parameters, "P74-P73-STACK")
        if not isinstance(getattr(p51, "TerminalPulse47Once", None), type):
            raise SealedDependencyFailure("P74-P73-STACK")

        return p73, p52, p72, p51, p43, p47
...
def release_identities() -> dict[str, object]:
    return {
        ...
        "pulse_73": {
            "commit": P73_COMMIT,
            "manifest_aggregate": P73.manifest_aggregate,
            "manifest_raw_sha256": P73.manifest_raw_sha256,
            "receipt_payload_sha256": P73.receipt_payload_sha256,
            "receipt_raw_sha256": P73.receipt_raw_sha256,
            "release_root": P73_RELEASE_ROOT,
            "seal_payload_sha256": P73.seal_payload_sha256,
            "seal_raw_sha256": P73.seal_raw_sha256,
            "source_sha256": P73.source_sha256,
            "stack": json.loads(canonical_bytes(P73_PREDECESSORS)),
        },
    }
