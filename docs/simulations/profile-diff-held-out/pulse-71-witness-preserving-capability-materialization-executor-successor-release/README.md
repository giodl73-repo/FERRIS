# Pulse 71 witness-preserving capability/materialization executor
...
ubuntu_runtime_parent)` is the only production callable. It mirrors Pulse 70's
...
Before any call, Pulse 71 loads its sibling `sealed_dependencies.py` binder by
...
fresh binder byte-binds the complete exact Pulse 70 release at commit
`b33cc2fdf2b067bc2bd448402164d939e69b5515`, verifies its manifest, receipt,
seal, source, gate catalog, and production/qualification callable signatures,
then instantiates fresh exact Pulse 52 stage helpers and exact Pulse 69/Pulse
51 terminal dependencies through Pulse 70's own sealed stack on every call.
...
`\0ferris-p71-<sha256>` whose bound socket is held for the full critical
...
stable Pulse 71 executor module owns one Linux `os.register_at_fork(after_in_child=...)`
...
executor instances on the same PID/thread from self-deadlocking, Pulse 71 also
...
`P71-SEALED-LOCK-CROSS-INSTANCE-REENTRY`; it never grants entry or bypasses
...
held across the entire exact Pulse 70 transitive
verify/import/binding sequence through exact Pulse 52, Pulse 69, Pulse 51, and
...
it installed. Pulse 71 does not rebuild P39/P41 ordering, P35 materialization,
or Pulse 69 cleanup-owning launch semantics; it delegates exact Pulse 70 production or
qualification orchestration over the fixed Pulse 69 cleanup-owning layer.

Pulse 70 removes its private runtime root on every terminal path, so Pulse 71
derives one fresh sibling terminal custody root from `private_runtime_root`
using the fixed suffix `.pulse71-terminal-publication`, rejects any preexisting
path, and creates it only after exact Pulse 70 returns `completed`. The public
event list is therefore exact Pulse 70 output with no post-completion
execution event.

## Terminal classes

After Pulse 70 returns `completed`, Pulse 71 invokes the exact one-use Pulse 51
...
- `invalid-witness-publication`: failed, malformed, mismatched, unverifiable,
  or residue-bearing terminal output. Pulse 71 performs exact bounded verified
  cleanup only; unresolved cleanup raises the non-returning public-safe
  `terminal-publication-cleanup-indeterminate` posture.

Any Pulse 70 preexecution or precompletion failure remains
...
Qualification calls exact Pulse 70 fake orchestration, alternates ten
...
Realistic integrity boundary: Pulse 71 rejects ambient import resolution,
