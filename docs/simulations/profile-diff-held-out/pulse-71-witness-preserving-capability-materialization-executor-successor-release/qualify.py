#!/usr/bin/env python3
"""Run deterministic fake-only Pulse 71 qualification cycles."""
...
WORK_ROOT = REPO_ROOT / "target" / "pulse-71-qualification-runtime"
...
CONTROL_METHODS = {
    "exact-p70-binding-and-signature": "test_sealed_binding_and_production_signature_match_pulse70",
    ...
    "concurrent-p70-load-restores-foreign-sentinel": "test_concurrent_load_pulse70_serializes_and_restores_foreign_sentinel",
    "p70-import-exception-restores-generic-slot": "test_load_pulse70_import_exception_restores_generic_slot",
    ...
    "qualification-delegates-to-p70": "test_qualification_delegates_to_exact_p70_executor",
    "published-result-survives-private-cleanup": "test_published_result_retains_verified_result_and_witness_after_p70_cleanup",
    ...
    "p70-prelaunch-failure-publication-not-attempted": "test_p70_prelaunch_failure_remains_publication_not_attempted",
    "cleanup-indeterminate-precedence": "test_cleanup_indeterminate_takes_precedence",
    "preexisting-terminal-root-rejected": "test_preexisting_terminal_root_is_rejected_before_p70_execution",
...
def _p27_success(path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError("P71 fake P27 root must be absent")
    path.mkdir()
    return {
...
class _InProcessWsl:
    ...
    def launch(self, ordinal: int, arguments: tuple[str, ...]) -> object:
        if self._closed or ordinal != len(self.requests) + 1 or len(arguments) != 7:
            raise RuntimeError("P71 fake worker protocol rejection")
...
def _synthetic_p39_checkout(root: Path) -> Path:
    ...
    if completed.returncode != 0:
        raise RuntimeError("P71 synthetic P39 checkout initialization failed")
    return checkout
...
def _run_cycle(root: Path, cycle: int) -> dict[str, object]:
    ...
    _p58, _p52, _p57, p51, p43, p47 = load_pulse70(REPO_ROOT)
    fake = FakeP56(
        artifact,
    ...
    if (
        private["p70_execution_outcome"] != "completed"
        ...
        raise RuntimeError("P71 qualification topology, cleanup, or ordering failure")
    if witnessed_failure:
        if (
            ...
            raise RuntimeError("P71 qualification did not retain only the witness failure")
    else:
        if (
            ...
            raise RuntimeError("P71 qualification did not retain result and witness")
    ...
    if any(
        ...
        raise RuntimeError("P71 qualification disclosed private capability material")
    return {
        "cycle": cycle,
        "cycle_mode": mode,
...
def _run_behavioral_controls() -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("P71 qualification behavioral-control failure")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with os.scandir(WORK_ROOT) as entries:
            if next(entries, None) is not None:
                raise RuntimeError("Pulse 71 qualification retained scratch")
    finally:
        if WORK_ROOT.exists():
            cleanup_synthetic_runtime_root(WORK_ROOT)
    ...
    payload = {
        "schema": (
            "ferris.pulse-71-witness-preserving-capability-materialization-"
        ...
        "p70_bound_commit": P70_COMMIT,
        "p70_publication_invocations": 0,
        "p71_publication_invocations": args.cycles,
    ...
    if (
        ...
        raise RuntimeError("Pulse 71 qualification did not cover all terminal classes")
