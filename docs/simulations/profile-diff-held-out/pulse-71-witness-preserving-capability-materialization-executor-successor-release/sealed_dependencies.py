"""Byte-bound exact imports for Pulse 71's witness-preserving Pulse 70 wrapper."""
...
P70_COMMIT = "b33cc2fdf2b067bc2bd448402164d939e69b5515"
P70_RELEASE_ROOT = (
    "docs/simulations/profile-diff-held-out/"
    "pulse-70-ordered-capability-materialization-executor-successor-release"
)
P58_GATE_IDS = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p71"
...
_P71_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p71_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 70 predecessor or callable binding failed."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P71-SEALED-LOCK-STATE")
    global _P71_INTERNAL_LOCK_MANAGER
    _P71_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P71_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P71-SEALED-LOCK-STATE")
    return _P71_INTERNAL_LOCK_MANAGER
...
P70 = ReleaseIdentity(
    ...
    13,
    15,
    "sha256:d9333b6ee0f5f94bc127136824898ccfb627a18db0166d1b935b73e31d399894",
    "sha256:d488b0198bd8069a077e68f0ce505af3dead78d624836d2521f48c56814a21fc",
    "sha256:190283cd8696114c49cca0d35917a1cc87d14b443c11d48a492f83c7379afda4",
...
P70_PREDECESSORS = {
    ...
    "pulse_69": {
        "manifest": "sha256:dd52c64100cdcd91ae1ea92f91af112f56296faf803511832ce3db489e071463",
        "seal": "sha256:2ac37cac02569d89cc068778cd956c8b5273c086685a1296dc568d13d191885f",
    },
}
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P71-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(
    path: Path, code: str = "P71-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P71-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P71-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P71-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P71-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P71-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P71-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P71-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P71-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P71-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P71-SEALED-LOCK-STATE")
                else:
...
def _patch_loaded_dependency_module(module: object, code: str) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise SealedDependencyFailure(code)
    if getattr(module, "__p71_bound_dependency_patch__", False):
        return module
    ...
    module.__p71_bound_dependency_patch__ = True
    return module
...
def load_pulse70(
    ...
    """Return fresh exact Pulse 70 modules after exact-tree verification."""
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P70, "P71-P70-IDENTITY")
        p70 = _load_with_bound_dependencies(
            "p71_exact_p70", bound, P70.source, "P71-P70-IMPORT"
        )
        _signature(
            p70,
        ...
        if (
            tuple(getattr(p70, "P58_GATE_IDS", ())) != P58_GATE_IDS
            or getattr(p70, "P39_CALLER_AUTHORITY_PRECONDITION", None)
            != "future-authority-supplied-fresh-anonymous-exact-cutoff-root"
            or not isinstance(
                getattr(p70, "OrderedCapabilityMaterializationResult", None), type
            )
            or not isinstance(getattr(p70, "P58Failure", None), type)
            or not isinstance(getattr(p70, "IndeterminateCleanup", None), type)
        ):
            raise SealedDependencyFailure("P71-P70-API")

        try:
            p52 = p70.load_exact_p52_stage_reader(repo_root)
            p69, p51, _p56 = p70.load_exact_p69_stack(repo_root)
            p43, _p45, p47 = p51.load_terminal_dependencies(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P71-P70-STACK") from error
        ...
        for module, name, parameters in (
            ...
            _signature(module, name, parameters, "P71-P70-STACK")
        if not isinstance(getattr(p51, "TerminalPulse47Once", None), type):
            raise SealedDependencyFailure("P71-P70-STACK")

        return p70, p52, p69, p51, p43, p47
...
def release_identities() -> dict[str, object]:
    return {
        ...
        "pulse_70": {
            "commit": P70_COMMIT,
            "manifest_aggregate": P70.manifest_aggregate,
            "manifest_raw_sha256": P70.manifest_raw_sha256,
            "receipt_payload_sha256": P70.receipt_payload_sha256,
            "receipt_raw_sha256": P70.receipt_raw_sha256,
            "release_root": P70_RELEASE_ROOT,
            "seal_payload_sha256": P70.seal_payload_sha256,
            "seal_raw_sha256": P70.seal_raw_sha256,
            "source_sha256": P70.source_sha256,
            "stack": json.loads(canonical_bytes(P70_PREDECESSORS)),
        },
    }
