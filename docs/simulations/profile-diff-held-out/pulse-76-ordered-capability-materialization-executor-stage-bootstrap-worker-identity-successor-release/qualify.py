#!/usr/bin/env python3
"""Run Pulse 76's sealed fake-only qualification cycles."""
...
WORK_ROOT = REPO_ROOT / "target" / "pulse-76-qualification-runtime"
...
from sealed_dependencies import canonical_bytes, load_exact_p75_stack, sha256_bytes  # noqa: E402
...
CONTROL_METHODS = {
    ...
    "exact-p75-binding-and-local-loader": "test_exact_p75_binding_and_local_loader",
    "local-loader-fresh-modules": "test_local_loader_ignores_ambient_state_and_returns_fresh_modules",
    "concurrent-100-complete-load-graph-serialized": "test_concurrent_100_exact_p75_loads_are_serialized",
...
def _run_cycle(root: Path, cycle: int) -> dict[str, object]:
    ...
    _p75, p51, _p56 = load_exact_p75_stack(REPO_ROOT)
    fake = FakeP56(
        artifact,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if WORK_ROOT.exists():
            cleanup_synthetic_runtime_root(WORK_ROOT)
    payload = {
        "schema": "ferris.pulse-76-ordered-capability-materialization-executor-stage-bootstrap-worker-identity-successor-qualification/v1",
        "outcome": "pass",
        "cycles_required": 20,
    ...
    if args.write_receipt:
        ...
        envelope = {
            ...
            "receipt_id": digest,
            "schema": "ferris.pulse-76-ordered-capability-materialization-executor-stage-bootstrap-worker-identity-successor-qualification-envelope/v1",
        }
