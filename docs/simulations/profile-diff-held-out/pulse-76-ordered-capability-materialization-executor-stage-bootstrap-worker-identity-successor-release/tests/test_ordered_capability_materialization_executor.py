...
class OrderedCapabilityMaterializationExecutorTests(unittest.TestCase):
    def setUp(self) -> None:
        self.work = REPO_ROOT / "target" / f"pulse-76-test-{uuid.uuid4().hex}"
    ...
    def _run(
        ...
        *,
        ...
        p75, p51, p56_real = stack or sealed.load_exact_p75_stack(REPO_ROOT)
    ...
    def _load_complete_exact_graph(self, binder: object) -> tuple[object, ...]:
        ...
        materializer, verifier = binder.load_exact_p35_materializer_and_verifier(REPO_ROOT)
        p75, p51, p56 = binder.load_exact_p75_stack(REPO_ROOT)
        return p39, p41, p52, materializer, verifier, p75, p51, p56
    ...
    def test_exact_p75_binding_and_local_loader(self) -> None:
        p75, p51, p56 = sealed.load_exact_p75_stack(REPO_ROOT)
        ...
        self.assertIn("load_exact_p75_stack", source)
    ...
    def test_p27_bounded_failure_is_terminal_and_cleaned(self) -> None:
        _p69, p51, _p56 = sealed.load_exact_p75_stack(REPO_ROOT)
    ...
    def test_ordinal_69_semantic_mismatch_preserves_failure_and_windows_expiry(self) -> None:
        p75, p51, p56 = sealed.load_exact_p75_stack(REPO_ROOT)
        original = p75._normalize_result
        calls = 0
        ...
        p75._normalize_result = mismatch
        result, fake, sessions, runtime, _checkout = self._run(
            "ordinal-69-mismatch", stack=(p75, p51, p56)
        )
    ...
    def test_first_semantic_mismatch_stops_execution(self) -> None:
        ...
        result, fake, _sessions, runtime, _checkout = self._run(
            "first-mismatch", stack=(p75, p51, p56)
        )
    ...
    def test_concurrent_100_exact_p75_loads_are_serialized(self) -> None:
        ...
        def worker(index: int) -> None:
            try:
                ...
                p39, p41, p52, materializer, verifier, p75, p51, p56 = (
                ...
                if not callable(p75.run_capability_bound_diagnostic_executor):
                    raise AssertionError("missing exact Pulse 75 executor")
