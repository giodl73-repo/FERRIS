...
class OrderedCapabilityMaterializationExecutorTests(unittest.TestCase):
    def setUp(self) -> None:
        self.work = REPO_ROOT / "target" / f"pulse-87-test-{uuid.uuid4().hex}"
    ...
    def _run(
        ...
        *,
        ...
        p86, p51, p56_real = stack or sealed.load_exact_p86_stack(self.repo_root)
    ...
    def _load_complete_exact_graph(self, binder: object) -> tuple[object, ...]:
        ...
        p86, p51, p56 = binder.load_exact_p86_stack(self.repo_root)
        return p39, p41, p52, materializer, verifier, p86, p51, p56
    ...
    def test_exact_p86_binding_and_local_loader(self) -> None:
        p86, p51, p56 = sealed.load_exact_p86_stack(self.repo_root)
        ...
        self.assertIn("load_exact_p86_stack", source)
    ...
    def test_ordered_terminal_preserves_p86_stage_indeterminate(self) -> None:
        p86, p51, p56 = sealed.load_exact_p86_stack(self.repo_root)
        result, _fake, sessions, runtime, _checkout = self._run(
            "p86-stage-indeterminate",
            stack=(p86, p51, p56),
            wsl_failure=p86.ExecutorFailure("P86-INDETERMINATE-STAGE-CLEANUP"),
        )
        self.assertEqual(
    ...
    def test_p35_source_matching_historical_alternate_digest_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(sealed.SealedDependencyFailure, "P87-P35-IDENTITY"):
    ...
    def test_p27_bounded_failure_is_terminal_and_cleaned(self) -> None:
        _p69, p51, _p56 = sealed.load_exact_p86_stack(self.repo_root)
    ...
    def test_ordinal_69_semantic_mismatch_preserves_failure_and_windows_expiry(self) -> None:
        p86, p51, p56 = sealed.load_exact_p86_stack(self.repo_root)
        original = p86._normalize_result
        calls = 0
        ...
        p86._normalize_result = mismatch
        result, fake, sessions, runtime, _checkout = self._run(
            "ordinal-69-mismatch", stack=(p86, p51, p56)
        )
    ...
    def test_first_semantic_mismatch_stops_execution(self) -> None:
        ...
        result, fake, _sessions, runtime, _checkout = self._run(
            "first-mismatch", stack=(p86, p51, p56)
        )
    ...
    def test_concurrent_100_exact_p86_loads_are_serialized(self) -> None:
        ...
        def worker(index: int) -> None:
            try:
                ...
                p39, p41, p52, materializer, verifier, p86, p51, p56 = (
                ...
                if not callable(p86.run_capability_bound_diagnostic_executor):
                    raise AssertionError("missing exact Pulse 86 executor")
    ...
    def test_kernel_lock_process_stress_serializes_subprocesses(self) -> None:
        script = self._write_subprocess_script(
            ...
            with binder._sealed_loading_lock():
                ...
                p86, p51, p56 = binder.load_exact_p86_stack(repo_root)
                if not callable(p39.verify):
                    raise AssertionError("missing exact Pulse 39 verifier")
                if not callable(p41.copy_release):
                    raise AssertionError("missing exact Pulse 41 copier")
