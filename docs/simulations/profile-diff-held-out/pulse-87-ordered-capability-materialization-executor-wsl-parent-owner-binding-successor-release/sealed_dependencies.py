"""Byte-bound exact imports for Pulse 87's ordered capability successor."""
...
P86_COMMIT = "9f87d726d886ffd864c511268e5bdd0d021686d8"
P86_RELEASE_ROOT = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p87"
_WINDOWS_WAIT_OBJECT_0 = 0x00000000
_WINDOWS_WAIT_ABANDONED = 0x00000080
_P87_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p87_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A Pulse 87 predecessor was not the exact sealed release."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P87-SEALED-LOCK-STATE")
    global _P87_INTERNAL_LOCK_MANAGER
    _P87_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P87_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P87-SEALED-LOCK-STATE")
    return _P87_INTERNAL_LOCK_MANAGER
...
P86 = ReleaseIdentity(
    ...
    "ferris.pulse-86-wsl-parent-owner-binding-capability-executor-successor-manifest/v1",
    ...
    14,
    16,
    "sha256:e0dff0705c2a1eb32836df8e6c17df8ac957ca8f2f65850ebc3f6df30d20c288",
    "sha256:41aa980d400eed0ab33ed40e702b62234865907b41eb32de4de6663fc373dd12",
    "sha256:fdb49de7ff30fa9bf069468f658a6de31002e44c852b6ef16534e617ef7f5691",
...
P35_VERIFIER_SOURCE = "verify_materialization.py"
P35_VERIFIER_SOURCE_SHA256 = "sha256:352d35202c0bef1a2294daa21bc4f6151db8f86a1bc1a0465914474981c1e301"
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P87-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(path: Path, code: str = "P87-SEALED-TREE", maximum: int = 4_194_304) -> bytes:
    try:
...
def _release_root(repo_root: Path, directory: str) -> Path:
    try:
        ...
        if not root.is_absolute():
            raise SealedDependencyFailure("P87-SEALED-ROOT")
        ...
        raise SealedDependencyFailure("P87-SEALED-ROOT") from error
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P87-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P87-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P87-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P87-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P87-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P87-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P87-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P87-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P87-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P87-SEALED-LOCK-STATE")
                else:
...
def _aggregate(
    entries: list[object],
    code: str,
    algorithm: object = "sha256-path-length-path-digest-v1",
) -> str:
    ...
    for entry in entries:
        ...
        if (
            ...
            raise SealedDependencyFailure(code)
        normalized.append((path, size, digest))
    if len({path for path, _, _ in normalized}) != len(normalized):
        raise SealedDependencyFailure(code)
    legacy_file_length_aggregate = False
    if type(algorithm) is dict:
        if type(algorithm.get("id")) is not str:
            raise SealedDependencyFailure(code)
    elif type(algorithm) is str:
        if algorithm == "sha256-length-path-filedigest-v1":
            legacy_file_length_aggregate = True
        elif algorithm != "sha256-path-length-path-digest-v1":
            raise SealedDependencyFailure(code)
    else:
        raise SealedDependencyFailure(code)
    digest = hashlib.sha256()
    for path, size, value in sorted(normalized, key=lambda item: item[0].encode("utf-8")):
        try:
        ...
        if len(raw) != 32:
            raise SealedDependencyFailure(code)
        if legacy_file_length_aggregate:
...
def _manifest_file_digest(entries: list[object], path: str, code: str) -> str:
    for entry in entries:
        if type(entry) is dict and entry.get("path") == path and type(entry.get("sha256")) is str:
...
def _verified_release(
    ...
    *,
    ...
    code = code or f"P87-{identity.directory.upper().replace('-', '-')}-IDENTITY"
    ...
    if (
        ...
        or _aggregate(files, code, manifest.get("aggregate_algorithm"))
        ...
        or (
            seal.get("seal_id") != identity.seal_payload_sha256
            and seal.get("receipt_id") != identity.seal_payload_sha256
        )
    ):
        raise SealedDependencyFailure(code)
...
def load_exact_p39_and_p41(repo_root: Path) -> tuple[ModuleType, ModuleType]:
    with _sealed_loading_lock():
        ...
        p39_module = _exec_bound_module(
            "p87_exact_p39", p39.root / P39.source, p39.files[P39.source], "P87-P39-IMPORT"
        )
        p41_module = _exec_bound_module(
            "p87_exact_p41", p41.root / P41.source, p41.files[P41.source], "P87-P41-IMPORT"
        )
        for name, parameters in (
            ...
            _signature(p39_module, name, parameters, "P87-P39-API")
        if (
            ...
            raise SealedDependencyFailure("P87-P39-API")
        _signature(
            p41_module,
            "copy_release",
        ...
        _signature(p41_module, "verify_bound_tree", ("root", "phase"), "P87-P41-API")
        _signature(p41_module, "_safe_absolute", ("value",), "P87-P41-API")
        if (
            ...
            raise SealedDependencyFailure("P87-P41-API")
...
def load_exact_p52_stage_reader(repo_root: Path) -> ModuleType:
    with _sealed_loading_lock():
        ...
        module = _load_with_bound_dependencies("p87_exact_p52", bound, P52.source, "P87-P52-IMPORT")
        _signature(module, "_validate_materialization_summary", ("p51", "summary"), "P87-P52-API")
        _signature(module, "_validate_verification_summary", ("p51", "summary"), "P87-P52-API")
        _signature(module, "_validate_materialized_descriptor_root", ("p51", "descriptor_root", "runtime_root"), "P87-P52-API")
        _signature(module, "_verify_public_prelaunch_custody", ("p51", "p39", "p41", "runtime_root", "p39_checkout_root", "p41_final_root", "private_record"), "P87-P52-API")
        _signature(module, "_remove_private_tree", ("p51", "path", "code"), "P87-P52-API")
        _signature(module, "_remove_seed", ("p51", "seed_path", "private_record"), "P87-P52-API")
        return module


def load_exact_p86(repo_root: Path) -> ModuleType:
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P86)
        module = _load_with_bound_dependencies("p87_exact_p86", bound, P86.source, "P87-P86-IMPORT")
        _signature(
            module,
        ...
        for name, parameters in (
            ...
            _signature(module, name, parameters, "P87-P86-API")
        if (
            ...
            raise SealedDependencyFailure("P87-P86-API")
        return module


def load_exact_p35_materializer_and_verifier(repo_root: Path) -> tuple[ModuleType, ModuleType]:
    with _sealed_loading_lock():
        verified = _verified_release(repo_root, P35, code="P87-P35-IDENTITY")
        manifest = _sealed_json(verified.files["public-manifest.json"], "P87-P35-IDENTITY")
        files = manifest.get("files")
        if type(files) is not list:
            raise SealedDependencyFailure("P87-P35-IDENTITY")
        materializer_raw = verified.files[P35.source]
        verifier_raw = verified.files[P35_VERIFIER_SOURCE]
        if (
            manifest.get("total_bytes") != 403316
            or _manifest_file_digest(files, P35.source, "P87-P35-IDENTITY")
            != sha256_bytes(materializer_raw)
            or _manifest_file_digest(files, P35_VERIFIER_SOURCE, "P87-P35-IDENTITY")
            != P35_VERIFIER_SOURCE_SHA256
            or sha256_bytes(verifier_raw) != P35_VERIFIER_SOURCE_SHA256
        ):
            raise SealedDependencyFailure("P87-P35-IDENTITY")
        materializer = _exec_bound_module(
            "p87_exact_p35_materializer",
            verified.root / P35.source,
            materializer_raw,
            "P87-P35-IMPORT",
        )
        verifier = _exec_bound_module(
            "p87_exact_p35_verifier",
            verified.root / P35_VERIFIER_SOURCE,
            verifier_raw,
            "P87-P35-IMPORT",
        )
        for module in (materializer, verifier):
            if (
                ...
                raise SealedDependencyFailure("P87-P35-API")
        _signature(materializer, "materialize", ("seed_path", "output", "case_count"), "P87-P35-API")
        _signature(materializer, "seed_commitment", ("seed",), "P87-P35-API")
        _signature(verifier, "verify", ("output", "seed_path"), "P87-P35-API")
        return materializer, verifier


def load_exact_p86_stack(repo_root: Path) -> tuple[ModuleType, ModuleType, ModuleType]:
    """Return exact Pulse 86 plus its own byte-bound P51/P56 modules."""
    with _sealed_loading_lock():
        p86 = load_exact_p86(repo_root)
        try:
            p51 = p86.load_exact_p51(repo_root)
            p56 = p86.load_exact_p56(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P87-P86-STACK") from error
        return p86, p51, p56
...
def release_identities() -> dict[str, dict[str, str]]:
    return {
        "pulse_35": {
            "aggregate": P35.manifest_aggregate,
            "manifest": P35.manifest_raw_sha256,
            "receipt": P35.receipt_raw_sha256,
            "receipt_payload": P35.receipt_payload_sha256,
            "seal": P35.seal_raw_sha256,
            "seal_payload": P35.seal_payload_sha256,
        },
        ...
        "pulse_86": {
            "commit": P86_COMMIT,
            "manifest": P86.manifest_raw_sha256,
            "release_root": P86_RELEASE_ROOT,
            "seal": P86.seal_payload_sha256,
        },
    }
