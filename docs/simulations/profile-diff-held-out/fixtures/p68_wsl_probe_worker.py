"""Harmless exact-route probe worker for Pulse 68 WSL preflight."""
...
SCHEMA = "ferris.pulse-68-wsl-probe-session/v1"
PLATFORM = "ubuntu-24.04-x86_64"
REQUEST_COUNT = 1
MAX_REQUEST_BYTES = 16_384
MAX_RESPONSE_BYTES = 2_800_000
MAX_STREAM_BYTES = 1_048_576
SEALED_DEPENDENCIES_SHA256 = "sha256:82b35ca824347ea29ddc0801d832c1858382bd0979868d403aa40febc8f7fa16"
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, item in pairs:
        if key in value:
            raise WorkerFailure("P68-WSL-PROTOCOL")
        value[key] = item
    return value
...
def _line_value(raw: bytes, maximum: int) -> dict[str, object]:
    ...
    try:
        ...
        raise WorkerFailure("P68-WSL-PROTOCOL") from error
...
def _load_sealed_dependencies(bundle_root: Path) -> ModuleType:
    ...
    content = _safe_regular(source, "P68-WSL-BUNDLE")
    if _sha256_bytes(content) != SEALED_DEPENDENCIES_SHA256:
        raise WorkerFailure("P68-WSL-BUNDLE")
    module = ModuleType("p68_probe_sealed_dependencies")
    ...
    try:
        ...
        raise WorkerFailure("P68-WSL-BUNDLE") from error
    return module
...
class ProbeProtocol:
    ...
    def _close(self) -> None:
        ...
        try:
            ...
            if _dependency_failure(self._dependencies, error) or isinstance(error, (OSError, ValueError)):
                raise WorkerFailure("P68-WSL-CLEANUP") from error
            raise
    ...
    def _close_after_failure(self) -> None:
        try:
            ...
            raise WorkerFailure("P68-INDETERMINATE-CLEANUP") from cleanup
    ...
    def consume(self, raw: bytes) -> bytes | None:
        ...
        try:
            ...
            if (
                ...
                raise WorkerFailure("P68-WSL-OUTPUT-BOUND")
        except WorkerFailure:
            ...
            self._close_after_failure()
            if _dependency_failure(self._dependencies, error) or isinstance(error, (OSError, ValueError)):
                raise WorkerFailure("P68-WSL-PROBE-LAUNCH") from error
            raise
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        runtime_parent = _native_directory(arguments.runtime_parent, "P68-WSL-NATIVE-ROOT")
        bundle_root = _native_directory(arguments.bundle_root, "P68-WSL-BUNDLE")
        p56_root = _native_directory(arguments.p56_root, "P68-WSL-P56-ROOT")
        expected_p56 = (
            bundle_root
            / "repository"
            / "docs"
        ...
        if p56_root != expected_p56:
            raise WorkerFailure("P68-WSL-P56-ROOT")
        ...
        if type(code) is str and code.startswith(("P56-", "P57-", "P68-")):
            return 2
        raise
