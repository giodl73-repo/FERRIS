#!/usr/bin/env python3
"""Run Pulse 73's sealed fake-only qualification cycles."""
...
WORK_ROOT = REPO_ROOT / "target" / "pulse-73-qualification-runtime"
...
from sealed_dependencies import canonical_bytes, load_exact_p72_stack, sha256_bytes  # noqa: E402


CONTROL_METHODS = {
    "production-surface-rejects-injection": "test_production_surface_rejects_injection",
    "exact-p72-binding-and-local-loader": "test_exact_p72_binding_and_local_loader",
...
def _run_cycle(root: Path, cycle: int) -> dict[str, object]:
    ...
    _p72, p51, _p56 = load_exact_p72_stack(REPO_ROOT)
    fake = FakeP56(
        artifact,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if WORK_ROOT.exists():
            cleanup_synthetic_runtime_root(WORK_ROOT)
    payload = {
        "schema": "ferris.pulse-73-ordered-capability-materialization-executor-stage-identity-successor-qualification/v1",
        "outcome": "pass",
        "cycles_required": 20,
    ...
    if args.write_receipt:
        ...
        envelope = {
            ...
            "receipt_id": digest,
            "schema": "ferris.pulse-73-ordered-capability-materialization-executor-stage-identity-successor-qualification-envelope/v1",
        }
