#!/usr/bin/env python3
"""Run deterministic fake-only Pulse 72 qualification cycles."""
...
WORK_ROOT = REPO_ROOT / "target" / "pulse-72-qualification-runtime"
...
CONTROL_METHODS = {
    "exact-p69-binding-and-signature": "test_exact_p69_binding_and_production_signature_match",
...
def _p27_success(path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError("P72 fake P27 root must be absent")
    path.mkdir()
    return {
...
def _run_negative_controls() -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("P72 qualification negative-control failure")
...
def _run_cycle(root: Path, cycle: int, p51: object) -> dict[str, object]:
    ...
    if (
        ...
        raise RuntimeError("P72 qualification topology or stage-identity failure")
    return {
        "cycle": cycle,
        "cycle_mode": mode,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if WORK_ROOT.exists():
            shutil.rmtree(WORK_ROOT)
    payload = {
        "schema": "ferris.pulse-72-capability-bound-diagnostic-executor-stage-identity-successor-qualification/v1",
        "outcome": "pass",
        "cycles_required": 20,
        ...
        "exact_p69_binding_verified": True,
    ...
    if args.write_receipt:
        receipt = {
            ...
            "schema": "ferris.pulse-72-capability-bound-diagnostic-executor-stage-identity-successor-receipt/v1",
        }
