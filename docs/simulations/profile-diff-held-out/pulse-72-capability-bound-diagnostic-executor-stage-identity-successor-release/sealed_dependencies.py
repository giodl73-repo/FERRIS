"""Byte-bound exact imports for Pulse 72's stage-identity Pulse 69 wrapper."""
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p72"
_WINDOWS_WAIT_OBJECT_0 = 0x00000000
_WINDOWS_WAIT_ABANDONED = 0x00000080
_P72_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p72_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 69 predecessor or callable binding failed."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P72-SEALED-LOCK-STATE")
    global _P72_INTERNAL_LOCK_MANAGER
    _P72_INTERNAL_LOCK_MANAGER = manager



def _require_internal_lock_manager() -> object:
    if _P72_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P72-SEALED-LOCK-STATE")
    return _P72_INTERNAL_LOCK_MANAGER
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P72-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result
...
def _safe_regular(
    path: Path, code: str = "P72-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P72-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P72-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P72-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P72-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P72-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P72-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P72-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P72-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P72-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P72-SEALED-LOCK-STATE")
                else:
...
def _load_exact_p69(bound: _VerifiedRelease, code: str) -> tuple[ModuleType, ModuleType]:
    ...
    with _installed_sealed_dependencies(dependencies, code):
        module = _exec_bound_module(
            "p72_exact_p69",
            bound.root / P69.source,
            bound.files[P69.source],
            code,
        )
...
def load_exact_p69_stack(repo_root: Path) -> tuple[ModuleType, ModuleType, ModuleType, ModuleType]:
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P69, "P72-P69-IDENTITY")
        p69, dependencies = _load_exact_p69(bound, "P72-P69-IMPORT")
        _signature(
            p69,
        ...
        for name in (
            ...
            if not callable(getattr(p69, name, None)):
                raise SealedDependencyFailure("P72-P69-API")
        if (
            not isinstance(getattr(p69, "_NativeWslSession", None), type)
            or not isinstance(getattr(p69, "ExecutorFailure", None), type)
            or getattr(p69, "REQUEST_COUNT", None) != 69
            or getattr(p69, "WSL_PLATFORM", None) != "ubuntu-24.04-x86_64"
            or getattr(p69, "WSL_SCHEMA", None)
            != "ferris.pulse-57-wsl-capability-session/v1"
        ):
            raise SealedDependencyFailure("P72-P69-API")
        try:
            p57, p51, p56 = p69.load_exact_p57_stack(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P72-P69-STACK") from error
        if not callable(getattr(dependencies, "load_p51_synthetic_fixture", None)):
            raise SealedDependencyFailure("P72-P69-API")
        p69._p72_bound_sealed_dependencies = dependencies
        return p69, p57, p51, p56
...
def load_p51_synthetic_fixture(repo_root: Path, p51: ModuleType) -> ModuleType:
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P69, "P72-P69-IDENTITY")
        _p69, dependencies = _load_exact_p69(bound, "P72-P69-IMPORT")
...
def release_identities() -> dict[str, object]:
    return {
        "pulse_69": {
            "manifest_aggregate": P69.manifest_aggregate,
            "manifest_raw_sha256": P69.manifest_raw_sha256,
            "receipt_payload_sha256": P69.receipt_payload_sha256,
            "receipt_raw_sha256": P69.receipt_raw_sha256,
            "release_root": P69_RELEASE_ROOT,
            "seal_payload_sha256": P69.seal_payload_sha256,
            "seal_raw_sha256": P69.seal_raw_sha256,
            "source_sha256": P69.source_sha256,
            "stack": json.loads(canonical_bytes(P69_STACK)),
        }
    }
