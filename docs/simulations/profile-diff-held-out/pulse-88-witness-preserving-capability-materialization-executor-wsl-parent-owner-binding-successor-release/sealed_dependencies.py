"""Byte-bound exact imports for Pulse 88's witness-preserving Pulse 87 wrapper."""
...
P87_COMMIT = "efa98a4e5b2fc138458c1ead45dbb7796cf00290"
P87_RELEASE_ROOT = (
    "docs/simulations/profile-diff-held-out/"
    "pulse-87-ordered-capability-materialization-executor-wsl-parent-owner-binding-successor-release"
)
P58_GATE_IDS = (
...
_KERNEL_LOCK_NAMESPACE_PREFIX = "ferris-p88"
...
_P88_INTERNAL_LOCK_MANAGER: object | None = None
...
_ACTIVE_SEALED_LOADING_LOCK: contextvars.ContextVar[_ActiveSealedLoadingLock | None] = (
    contextvars.ContextVar("p88_active_sealed_loading_lock", default=None)
)


class SealedDependencyFailure(RuntimeError):
    """A sealed Pulse 87 predecessor or callable binding failed."""
...
def _bind_internal_lock_manager(manager: object) -> None:
    if not callable(getattr(manager, "register_active_lock_state", None)) or not callable(
        ...
        raise SealedDependencyFailure("P88-SEALED-LOCK-STATE")
    global _P88_INTERNAL_LOCK_MANAGER
    _P88_INTERNAL_LOCK_MANAGER = manager


def _require_internal_lock_manager() -> object:
    if _P88_INTERNAL_LOCK_MANAGER is None:
        raise SealedDependencyFailure("P88-SEALED-LOCK-STATE")
    return _P88_INTERNAL_LOCK_MANAGER
...
P87_PREDECESSORS = {
    "pulse_35": {
        ...
        "receipt": "sha256:4c4f4ad1d9fa437e23f655083eb74c754114c5bea43ae111d2127fc7f051a037",
        "receipt_payload": "sha256:7f1154ca94009cef966ab2f43ba74a9f017989ed5dbbdbfd8c3ce8fe64fe5cee",
        "seal": "sha256:17459123c674f2664d7d09ea03c00dcba72129bb1cf532cfe11f8cf4edeffd23",
        "seal_payload": "sha256:834781867ea008dc14a54d7b811002ee1b8fa759c0b1d7f32432ea6c0d5c5375",
    },
    "pulse_39": {
    ...
    "pulse_86": {
        "commit": "9f87d726d886ffd864c511268e5bdd0d021686d8",
        "manifest": "sha256:afda975b8e6c0672cf80e87676eba4ff4bd071ea3674babc69d39eb8ab8f1c8b",
        "release_root": "docs/simulations/profile-diff-held-out/pulse-86-wsl-parent-owner-binding-capability-executor-successor-release",
        "seal": "sha256:31b3f516aa2a94a1d9be29a7562eed3d2eda9115a038ca6739b047521cc5b187",
    },
}
...
def _duplicate_free_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SealedDependencyFailure("P88-SEALED-JSON-DUPLICATE")
        result[key] = value
    return result


def _safe_regular(
    path: Path, code: str = "P88-SEALED-TREE", maximum: int = 4_194_304
) -> bytes:
    try:
...
def _self_path() -> Path:
    try:
        ...
        raise SealedDependencyFailure("P88-SEALED-LOCK-OPEN") from error
...
def _kernel_lock_name() -> str:
    ...
    digest.update(b"\0")
    digest.update(sha256_bytes(_safe_regular(path, "P88-SEALED-LOCK-OPEN")).encode("ascii"))
...
def _open_linux_kernel_socket() -> socket.socket:
    if not _linux_socket_lock_supported() or not hasattr(socket, "AF_UNIX"):
        raise SealedDependencyFailure("P88-SEALED-LOCK-OPEN")
    try:
...
def _acquire_kernel_lock(lock_state: _KernelLockState) -> None:
    if lock_state.kind == "windows-mutex":
        ...
        if result in (_WINDOWS_WAIT_OBJECT_0, _WINDOWS_WAIT_ABANDONED):
            return
        raise SealedDependencyFailure("P88-SEALED-LOCK-ACQUIRE")
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                raise SealedDependencyFailure("P88-SEALED-LOCK-ACQUIRE") from close_error
            ...
            if error.errno == errno.EADDRINUSE and time.monotonic() < deadline:
                ...
                continue
            raise SealedDependencyFailure("P88-SEALED-LOCK-ACQUIRE") from error
...
def _release_kernel_lock(lock_state: _KernelLockState) -> None:
    ...
    if not library.ReleaseMutex(lock_state.handle):
        raise SealedDependencyFailure("P88-SEALED-LOCK-RELEASE")
...
def _close_kernel_handle(lock_state: _KernelLockState) -> None:
    ...
    try:
        ...
        raise SealedDependencyFailure("P88-SEALED-LOCK-RELEASE") from error
...
def _reject_cross_instance_reentry(lock_name: str) -> None:
    if _require_internal_lock_manager().advisory_conflict(
        ...
        raise SealedDependencyFailure("P88-SEALED-LOCK-CROSS-INSTANCE-REENTRY")
        ...
        try:
            if (
                ...
                if current is not active_state or active_state.depth != 1:
                    release_error = SealedDependencyFailure("P88-SEALED-LOCK-STATE")
                else:
...
def _patch_loaded_dependency_module(module: object, code: str) -> ModuleType:
    if not isinstance(module, ModuleType):
        raise SealedDependencyFailure(code)
    if getattr(module, "__p88_bound_dependency_patch__", False):
        return module
    ...
    module.__p88_bound_dependency_patch__ = True
    return module
...
def load_pulse87(
    ...
    """Return fresh exact Pulse 87 modules after exact-tree verification."""
    with _sealed_loading_lock():
        bound = _verified_release(repo_root, P87, "P88-P87-IDENTITY")
        p87 = _load_with_bound_dependencies(
            "p88_exact_p87", bound, P87.source, "P88-P87-IMPORT"
        )
        _signature(
            p87,
        ...
        if (
            tuple(getattr(p87, "P58_GATE_IDS", ())) != P58_GATE_IDS
            or getattr(p87, "P39_CALLER_AUTHORITY_PRECONDITION", None)
            != "future-authority-supplied-fresh-anonymous-exact-cutoff-root"
            or not isinstance(
                getattr(p87, "OrderedCapabilityMaterializationResult", None), type
            )
            or not isinstance(getattr(p87, "P58Failure", None), type)
            or not isinstance(getattr(p87, "IndeterminateCleanup", None), type)
        ):
            raise SealedDependencyFailure("P88-P87-API")

        try:
            p52 = p87.load_exact_p52_stage_reader(repo_root)
            p86, p51, _p56 = p87.load_exact_p86_stack(repo_root)
            p43, _p45, p47 = p51.load_terminal_dependencies(repo_root)
        except SealedDependencyFailure:
            raise
        except BaseException as error:
            raise SealedDependencyFailure("P88-P87-STACK") from error
        ...
        for module, name, parameters in (
            ...
            _signature(module, name, parameters, "P88-P87-STACK")
        if not isinstance(getattr(p51, "TerminalPulse47Once", None), type):
            raise SealedDependencyFailure("P88-P87-STACK")

        return p87, p52, p86, p51, p43, p47
...
def release_identities() -> dict[str, object]:
    return {
        ...
        "pulse_87": {
            "commit": P87_COMMIT,
            "manifest_aggregate": P87.manifest_aggregate,
            "manifest_raw_sha256": P87.manifest_raw_sha256,
            "receipt_payload_sha256": P87.receipt_payload_sha256,
            "receipt_raw_sha256": P87.receipt_raw_sha256,
            "release_root": P87_RELEASE_ROOT,
            "seal_payload_sha256": P87.seal_payload_sha256,
            "seal_raw_sha256": P87.seal_raw_sha256,
            "source_sha256": P87.source_sha256,
            "stack": json.loads(canonical_bytes(P87_PREDECESSORS)),
        },
    }
