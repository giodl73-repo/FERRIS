#!/usr/bin/env python3
"""Run deterministic fake-only Pulse 88 qualification cycles."""
...
P51_ROOT = (
    PROFILE_DIFF_ROOT / "pulse-51-diagnostic-executor-release"
)
WORK_ROOT = REPO_ROOT / "target" / "pulse-88-qualification-runtime"
EXACT_REPO_ROOT = REPO_ROOT / "target" / "pulse-88-exact-p35-release-tree-root"
...
CONTROL_METHODS = {
    "exact-p87-binding-and-signature": "test_sealed_binding_and_production_signature_match_pulse87",
    "p86-stage-indeterminate-publication-not-attempted": "test_p86_stage_indeterminate_remains_publication_not_attempted",
    ...
    "concurrent-p87-load-restores-foreign-sentinel": "test_concurrent_load_pulse87_serializes_and_restores_foreign_sentinel",
    "p87-import-exception-restores-generic-slot": "test_load_pulse87_import_exception_restores_generic_slot",
    ...
    "kernel-lock-cross-instance-reentry-fails-closed": "test_kernel_lock_cross_instance_same_thread_fails_closed_quickly_and_clears_marker",
    ...
    "qualification-delegates-to-p87": "test_qualification_delegates_to_exact_p87_executor",
    "published-result-survives-private-cleanup": "test_published_result_retains_verified_result_and_witness_after_p87_cleanup",
    ...
    "p87-prelaunch-failure-publication-not-attempted": "test_p87_prelaunch_failure_remains_publication_not_attempted",
    "cleanup-indeterminate-precedence": "test_cleanup_indeterminate_takes_precedence",
    "preexisting-terminal-root-rejected": "test_preexisting_terminal_root_is_rejected_before_p87_execution",
...
def _p27_success(path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError("P88 fake P27 root must be absent")
    path.mkdir()
    return {
...
class _InProcessWsl:
    ...
    def launch(self, ordinal: int, arguments: tuple[str, ...]) -> object:
        if self._closed or ordinal != len(self.requests) + 1 or len(arguments) != 7:
            raise RuntimeError("P88 fake worker protocol rejection")
...
def _synthetic_p39_checkout(root: Path) -> Path:
    ...
    if completed.returncode != 0:
        raise RuntimeError("P88 synthetic P39 checkout initialization failed")
    return checkout
...
def _normalize_exact_p35_release(root: Path) -> None:
    ...
    if manifest.get("file_count") != 8:
        raise RuntimeError("P88 exact Pulse 35 manifest file count mismatch")
    for entry in manifest["files"]:
        ...
        if (
            ...
            raise RuntimeError(
                f"P88 exact Pulse 35 normalization failed for {path.name}"
            )
        path.write_bytes(normalized)
    if sum(1 for path in root.rglob("*") if path.is_file()) != 10:
        raise RuntimeError("P88 exact Pulse 35 release tree file count mismatch")
...
def exact_repo_root() -> Path:
    ...
    pulse35_root = profile_root / "pulse-35-corpus-materializer-release"
    if (
...
def _run_cycle(root: Path, cycle: int) -> dict[str, object]:
    ...
    _p87, _p52, _p86, p51, p43, p47 = load_pulse87(repo_root)
    fake = FakeP56(
        artifact,
    ...
    if (
        private["p87_execution_outcome"] != "completed"
        ...
        raise RuntimeError("P88 qualification topology, cleanup, or ordering failure")
    if witnessed_failure:
        if (
            ...
            raise RuntimeError("P88 qualification did not retain only the witness failure")
    else:
        if (
            ...
            raise RuntimeError("P88 qualification did not retain result and witness")
    ...
    if any(
        ...
        raise RuntimeError("P88 qualification disclosed private capability material")
    return {
        "cycle": cycle,
        "cycle_mode": mode,
...
def _run_behavioral_controls() -> tuple[str, ...]:
    ...
    if (
        ...
        raise RuntimeError("P88 qualification behavioral-control failure")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with os.scandir(WORK_ROOT) as entries:
            if next(entries, None) is not None:
                raise RuntimeError("Pulse 88 qualification retained scratch")
    finally:
        if WORK_ROOT.exists():
            cleanup_synthetic_runtime_root(WORK_ROOT)
    ...
    payload = {
        "schema": (
            "ferris.pulse-88-witness-preserving-capability-materialization-"
            "executor-wsl-parent-owner-binding-successor-qualification/v1"
        ),
        "outcome": "pass",
        "cycles_required": 20,
        ...
        "p87_bound_commit": P87_COMMIT,
        "p87_publication_invocations": 0,
        "p88_publication_invocations": args.cycles,
    ...
    if (
        ...
        raise RuntimeError("Pulse 88 qualification did not cover all terminal classes")
    if args.write_receipt:
        ...
        envelope = {
            ...
            "schema": (
                "ferris.pulse-88-witness-preserving-capability-materialization-"
                "executor-wsl-parent-owner-binding-successor-qualification-envelope/v1"
            ),
        }
