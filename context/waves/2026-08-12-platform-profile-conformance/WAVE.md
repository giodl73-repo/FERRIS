...
Status: Pulse 67 authorized-unexecuted at immutable cutoff
`3ec6a36009fd34765508f729e795042fd610e5d4`, with zero calls, seeds,
descriptors, processes, publications, and transfers plus null conclusions; it
preserves the Pulse 66 withdrawal, exact final Pulse 59, and a truthful split
between dynamic route-equivalence and static exact production binding. Its
pre-call WSL qualification requires exactly two harmless bounded WSL process
spawns with no retry and no hidden third spawn before any sole Pulse 59 call:
spawn 1 is the exact Pulse 57 `subprocess.run` stage-bundle route that stages
the full twelve-file P57-shaped bundle and returns the canonical `bundle_root`
without cleanup, while spawn 2 is the exact `subprocess.Popen` worker-
bootstrap route over that same staged bundle using the separate sealed
harmless `p67_wsl_probe_worker.py`, the matching sealed harmless
`p67_wsl_probe_sealed_dependencies.py`, and the bound
`ferris.pulse-67-wsl-probe-session/v1` protocol to prove only ready/one
harmless probe/close plus bounded wait/terminate/kill and one final cleanup
after both spawns complete; static byte binding separately proves the exact
...
work during qualification; Pulse 66 permanently withdrawn
`invalid-prelaunch-wsl-probe-bundle-contract` under
`P66-WORKER-HASH-BUNDLE-LIFETIME`, with zero calls, seeds, descriptors,
processes, publications, and transfers plus null conclusions after
independent prelaunch review proved the exact worker rejects the fake sealed
dependency hash before `ready` and that the declared spawn 1 cleanup /
absence proof contradicted the required spawn 2 bundle handoff; Pulse 65
...
after independent prelaunch review proved its declared WSL qualification still
permitted a nonexistent/unbound optional
...
Pulse 63 permanently withdrawn `invalid-prelaunch-wsl-bootstrap-contract`,
with zero calls, seeds, descriptors, processes, publications, and transfers
plus null conclusions after independent prelaunch review proved its declared
WSL preflight still underbound the exact Pulse 57 `_NativeWslSession`
bootstrap path by using smaller generic bounds, a misleading Pulse 59 schema
path instead of the actual staged Pulse 56 release tree, path-bearing probe
outputs instead of exact canonical `bundle_root` stage JSON, and incomplete
worker-bootstrap / source-loader / private-parent comparison binding;
Pulse 62 permanently withdrawn `invalid-prelaunch-path-route-contract` under
...
permanently withdrawn `invalid-prelaunch-checkout-variant-integrity`,
zero-launch and null-conclusion; Pulse 50 permanently withdrawn
...
46 permanently invalid-publication-integrity, non-retryable, and
null-conclusion; Pulse 42 permanently invalid due public-result
publication/order integrity; Pulse 43 releases public
...
composition infrastructure only; Pulses 22, 24, 26, 28, 30, 32, 34, 36,
...
| 67 | Witnessed capability/materialization diagnostic authority | Authorized-unexecuted | Current one-shot authority at immutable cutoff `3ec6a36`; records the Pulse 66 withdrawal and exact final Pulse 59 while separating dynamic route-equivalence from static exact production binding: spawn 1 stages the full P57-shaped bundle and hands the same `bundle_root` to spawn 2, which launches a separate sealed harmless probe worker/dependency with the exact P57 bootstrap shape and bound ready/one-probe/close protocol, while static byte binding separately proves the exact production Pulse 57 worker/dependency identities and callable route |
...
`e01130a5c1fc5b8e58e13bbde03dfc39b8f1bf60`. A new custodian copied and
verified the nine public bundle files and ran the required two-pair harmless
preflight. All bundle and cutoff bindings verified, but the second pair failed
exact-cardinality fresh-process reload. One pair completed, four synthetic
processes ran, and no retry, generation, candidate, or category conclusion
exists. Pulses 22, 24, and 26 remain permanently invalid, non-retryable, and
unable to produce category conclusions.

Pulse 27 publishes a separate exact-two-pair adapter around the byte-for-byte
immutable Pulse 25 collector. The Pulse 26 failure came from pair-local
expected cardinality `1` being supplied to a whole-store verifier after pair
...
11. authorize and independently execute one fresh public-rule-based
    ...
    collector outcome without retry or category inference; and
12. diagnose and synthetically qualify the collector durability repair without
    executing or replaying a candidate; and
13. freeze public authority for one new independently custodied replacement
    search with exact collector verification, harmless preflight, fresh
    generation material, transactional pair collection, and no execution in
    the authority change; and
14. freeze one later new independent program against the exact public Pulse 25
    source bundle, requiring a nine-file-only isolated copy, independent
    per-file and aggregate recomputation, exactly two zero-retry preflight
...
- treating Pulse 24 as a Pulse 22 retry, resume, reseed, rescore, replay,
  continuation, or reuse;
- executing Pulse 24 without its exact cutoff, five collector qualification
  digests, two or more harmless atomic preflight pairs, and wholly new custody
  and generation material;
- classifying a Pulse 24 pair before both process records are durably synced,
  pair-sealed, and reloaded by a fresh process, or launching again after a
  package-invalidating failure;
- treating Pulse 26 as a retry, resume, reseed, rescore, reuse, or continuation
  of Pulse 22 or Pulse 24;
- executing Pulse 26 without its exact cutoff, six public release bindings,
  nine-file-only isolated copy, independent file and aggregate recomputation,
  exactly two zero-retry harmless atomic preflight pairs, and wholly new
...
- Pulse 24 remains permanently invalid-before-candidates, non-retryable, and
...
Pulse 50 historical authority commit
`48fe9fdcdda03378f68781cae342796c9f11720d` and immutable cutoff
`94d473563a1686091be94a72f491b0ff0d903800` are permanently
...
Independent custodians stopped before launch because the authority/cutoff did
not contain a sealed end-to-end executor. The public audit found the omitted
Pulse 35 CRLF/LF schema binding, no sealed descriptor/69+1/P43 runner, Pulse
27 callable-not-CLI seam, Pulse 31 schema-count drift, WSL/canonical Ubuntu
label mismatch, incomplete exact Ubuntu Pulse 33 toolchain/hash custody, and
Python resolver detail. No seed, descriptor, candidate process, private
material, P43/P47 invocation, result root, witness root, diagnostic
execution, or inference exists; execution state is zero or false.
...
release-tree residue, and keeps scratch outside the sealed tree.

Pulse 51 commit `d09c923c1e2cd2be003026597f4ad2a0e2d3764f` is the direct
...
[Pulse 55](pulses/pulse-55.md) was published as a fresh
`authorized-unexecuted` authority at self-excluding cutoff
`47113e444ef3309afec9a844f0cba62775f19f6f`. It binds
...
`core.autocrlf=false` remains the actual runtime custody posture.

At authority commit `36b3ac6b9692924af57c7c98b0a976835fe778f6`, fresh P44
custody occurred once per platform before the exact P53
`run_witness_preserving_ordered_executor` call. The one and only call returned
public publication disposition/classification `not-attempted` with one
`terminal-stop/failed` at `pulse-41-pulse-39-public-custody`: zero completed
gates, seed, descriptors, processes, no-launch dispositions,
P27/P39/P41/materializer/verifier/P47 calls, and result/witness transfers.
All category, diagnostic, product, and fix conclusions are null.

Pulse 55 is permanently `terminal-prerequisite-identity-failure` under
`P55-P33-RETAINED-IDENTITY-CONTRACT`, non-retryable, and non-resumable. The
...
identities. The structural contract issue is precise: the `build_freeze`
receipt field `retained_in_public_bundle` changes when the executable is
retained, so exact published receipt payload identity cannot equal a real
retained-custody receipt. Windows `/Brepro` plus Rust/Cargo version alone did
not reproduce the historic binary, showing the linker/SDK environment is
underbound.

No retry, resume, amendment, or successor launch is authorized from Pulse 55.
A successor requires a new sealed executor chain binding a corrected P33
...
created by this wave record. See the
