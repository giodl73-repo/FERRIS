...
- Complete PERF-Q23 with package-wide, precise-declaration, checksum,
  hidden-input, stable-output, directive, persistent-output, capability,
  `links`, and target-override controls.
- Confirm that precise rerun declarations suppress unrelated package changes
  while hidden inputs can leave generated output stale.
- Confirm that declared build-script paths remain mtime-based even under
  checksum freshness.
- Confirm that a script rerun rebuilt its owning crate and application in the
  controlled fixture even when generated bytes and effective instructions
  remained unchanged.
- Preserve write-if-changed as output hygiene rather than a promised Cargo
  freshness optimization.
- Confirm in the controlled fixture that `OUT_DIR` persisted a script-owned
  file after the script stopped producing it; reject automatic whole-directory
  cleanup.
- Consume Cargo's optional nightly build-analysis root, cascade, unit, cause,
  and timing evidence behind a versioned boundary rather than duplicating its
  schema.
...
closed.
PERF-Q23 is complete and establishes the build-script compile-identity,
run-identity, detection-mode, declared-input, hidden-input, rerun-cause,
saved-output, effective-output, output-ownership, native-metadata,
capability-boundary, and downstream-fan-out model. Precise declarations
prevented unrelated reruns and reduced the synthetic file-heavy no-op median,
while a hidden file left stale generated output. Declared path freshness
remained mtime-based under checksum freshness. A byte-identical rerun still
rebuilt the owning crate and application, and an obsolete `OUT_DIR` file
persisted after the script stopped producing it. Cargo's nightly build analysis
reported the rerun root and two-unit cascade. The adopted
opportunity is read-only declaration, output, lifecycle, native-metadata, and
